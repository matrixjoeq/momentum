...
DEFAULT_OUTPUT_JSON = (
    "src/etf_momentum/web/data/trend_r_profit_scaleout_param_search_results.json"
)

# User-provided payload baseline. Script sweeps r_multiple x reduce_fraction.
...
def _build_base_payload(raw_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    out["r_profit_scaleout_enabled"] = True
    out["r_profit_scaleout_execution_mode"] = "next_day"
    return out
...
def _run_single_case(
    *,
    ...
    payload["r_profit_scaleout_enabled"] = True
    payload["r_profit_scaleout_execution_mode"] = "next_day"
    payload["r_profit_scaleout_tiers"] = [
        {"r_multiple": float(r_multiple), "reduce_fraction": float(reduce_fraction)}
    ]
...
def run(args: argparse.Namespace) -> int:
    ...
    r_values = _grid_float_values(
        start=float(args.r_multiple_start),
        end=float(args.r_multiple_end),
        step=float(args.r_multiple_step),
    )
    reduce_values = _grid_float_values(
        start=float(args.reduce_fraction_start),
        end=float(args.reduce_fraction_end),
        step=float(args.reduce_fraction_step),
    )
    grid = [
        (r_multiple, reduce_fraction)
        for r_multiple in r_values
        for reduce_fraction in reduce_values
    ]
    print(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=int(args.workers)) as ex:
        fut_map = {
            ...
            for r_multiple, reduce_fraction in grid
        }
        done = 0
    ...
    rows.sort(
        key=lambda x: (
            float(x.get("r_multiple") or 0.0),
            float(x.get("reduce_fraction") or 0.0),
        )
    )
    ...
    payload = {
        "meta": {
            ...
            "search_space": {
                "r_multiple_start": float(args.r_multiple_start),
                "r_multiple_end": float(args.r_multiple_end),
                "r_multiple_step": float(args.r_multiple_step),
                "r_multiple_values": r_values,
                "reduce_fraction_start": float(args.reduce_fraction_start),
                "reduce_fraction_end": float(args.reduce_fraction_end),
                "reduce_fraction_step": float(args.reduce_fraction_step),
                "reduce_fraction_values": reduce_values,
            },
            "fixed_payload": {
                k: v for k, v in base_payload.items() if k != "r_profit_scaleout_tiers"
            },
    ...
    if best:
        ...
        print(
            "[INFO] best sharpe "
            f"r_multiple={best.get('r_multiple')}, "
            f"reduce_fraction={best.get('reduce_fraction')}, "
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Grid-search R scaleout tier params for trend portfolio "
            "by calling backend API."
        )
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="R tier threshold start (inclusive).",
    )
    p.add_argument(
        "--r-multiple-end",
        type=float,
        default=5.0,
        help="R tier threshold end (inclusive).",
    )
    p.add_argument(
        "--r-multiple-step",
        type=float,
        default=0.5,
        help="R tier threshold step.",
    )
    p.add_argument(
        "--reduce-fraction-start",
        type=float,
        default=0.1,
        help="Reduce fraction start (inclusive).",
    )
    p.add_argument(
        "--reduce-fraction-end",
        type=float,
        default=0.9,
        help="Reduce fraction end (inclusive).",
    )
    p.add_argument(
        "--reduce-fraction-step",
        type=float,
        default=0.1,
        help="Reduce fraction step.",
    )
