...
def _fetch_cn_from_akshare(
    tenor_label: str, start: str, end: Optional[str]
    ...
    try:
        ...
        while cur <= end_dt:
            ...
            if year_end > end_dt:
                year_end = end_dt
            try:
                df_part = ak.bond_china_yield(
                    start_date=to_str(cur), end_date=to_str(year_end)
                )
            except Exception:
                df_part = ak.bond_china_yield(
                    start_date=to_str(cur).replace("-", ""),
                    end_date=to_str(year_end).replace("-", ""),
                )
            if df_part is not None and not df_part.empty:
                frames.append(df_part)
            logging.debug(
                f"[fetch-akshare] collected year {cur.year}, part_rows={(0 if df_part is None else len(df_part))}"
            )
        ...
        tenor_candidates = [
            tenor_label,
            tenor_label.replace("年", "年期"),
            "10Y" if tenor_label.startswith("10") else "30Y",
        ]
        sel = None
        ...
        df = (
            df_all[["日期", sel]]
            .rename(columns={"日期": "date", sel: "yield_pct"})
            .copy()
        )
        df["date"] = pd.to_datetime(df["date"]).dt.date
        df["yield_pct"] = pd.to_numeric(df["yield_pct"], errors="coerce")
        df = (
            df.dropna(subset=["date", "yield_pct"])
            .sort_values("date")
...
def fetch_history(instrument: str, start: str, end: Optional[str]) -> FetchResult:
    ...
    if instrument == "CN10Y":
        ...
        try:
            ...
            logging.error(
                "fredapi not installed. Install with: python3 -m pip install fredapi"
            )
            raise
...
def upsert_rows(
    conn: sqlite3.Connection, table_name: str, df: pd.DataFrame, source: str
) -> int:
    if df.empty:
        return 0
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    rows = [
        (pd.to_datetime(d).strftime("%Y-%m-%d"), float(v), source, ts)
        for d, v in zip(df["date"], df["yield_pct"])
    ]
    conn.executemany(
        f"""
...
def plot_kline(
    ohlc: pd.DataFrame, out_path: Path, title: str
    ...
    try:
        ...
        return (
            False,
            "Missing mplfinance. Install with: python3 -m pip install mplfinance",
        )
    try:
        mpf.plot(
            ohlc,
...
def run_for_instrument(
    instrument: str, db_path: Path, do_plot: bool, window: str
) -> None:
    ...
    try:
        ...
        if instrument in ("CN10Y", "CN30Y", "US10Y") and (
            fetch_start >= today or (last is not None and fetch_start > last)
        ):
            logging.info(
                f"[run] {instrument} no new data for {fetch_start}, skipping fetch and keeping DB unchanged"
            )
            with sqlite3.connect(db_path) as conn:
                df_all = pd.read_sql_query(
                    f"SELECT date, yield_pct FROM {table} ORDER BY date",
                    conn,
                    parse_dates=["date"],
                )  # type: ignore
            ...
            pct = compute_percentile(
                latest_yield, df_win["yield_pct"]
            )  # percentage of window <= current
            print(f"=== {instrument} Yield Summary ===")
            print("Source            : (no new data)")
            print(f"History rows      : {df_all.shape[0]}")
            print(
                f"First date        : {pd.to_datetime(df_all['date'].iloc[0]).date()}"
            )
            ...
            if do_plot:
                plotted, plot_err = plot_kline(
                    ohlc,
                    _png_path_for(instrument),
                    title=f"{instrument} Yield [{window}] (up to {latest_date})",
                )
                if not plotted:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Unified bond yields tool (CN10Y, CN30Y, US10Y)"
    )
    parser.add_argument(
        "--instruments",
        default="CN10Y,CN30Y,US10Y",
        help="Comma list from CN10Y,CN30Y,US10Y or 'all'",
    )
    parser.add_argument("--db", default=str(DEFAULT_DB_PATH))
    parser.add_argument("--no-plot", action="store_true")
    parser.add_argument(
        "--window",
        choices=["all", "10y", "5y", "1y"],
        default="all",
        help="Plotting/percentile window",
    )
    parser.add_argument(
        "--full-update",
        action="store_true",
        help="Purge selected instruments' tables before refetching full history",
    )
        ...
        try:
            if args.full_update:
                ...
                with sqlite3.connect(db_path) as conn:
                    ...
                    conn.commit()
            run_for_instrument(
                t, db_path, do_plot=(not args.no_plot), window=args.window
            )
        except Exception:
            had_error = True
        ...
        with sqlite3.connect(db_path) as conn:
            for t in targets:
                ...
                df_sum = pd.read_sql_query(
                    f"SELECT MIN(date) as mind, MAX(date) as maxd, COUNT(1) as cnt FROM {table}",
                    conn,
                )
                mind = df_sum.loc[0, "mind"]
                maxd = df_sum.loc[0, "maxd"]
                cnt = (
                    int(df_sum.loc[0, "cnt"])
                    if not pd.isna(df_sum.loc[0, "cnt"])
                    else 0
                )
    ...
    if had_error:
        logging.warning(
            "One or more instruments failed. See logs above. Partial updates may have been applied."
        )
    return 0
