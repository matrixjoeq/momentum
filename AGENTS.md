...
- **SOUL binding rule (mandatory):** `SOUL.md` is the project-level quality constitution for agent behavior. Any implementation that conflicts with `SOUL.md` is non-compliant and must not be delivered.
...
- Maintenance and evolves
  ...
  - **Financial-engineering DoD gate (mandatory):** Any change affecting cash, holdings, NAV, PnL, attribution, fees, replay, or scope aggregation must pass all checks below before delivery:
    1. Identity checks: `equity = cash + market_value` (same scope, same day).
    2. Attribution rebuild checks: `daily_twr = selection + timing + position + cost_drag + cash_drag + repo_carry + repo_fee_drag`.
    3. NAV recursion checks: `nav_twr[t] = nav_twr[t-1] * (1 + daily_twr[t])`.
    4. Period checks: `period.rebuild_total = period.total_return_twr_sum`.
    5. Scope checks: account/strategy/holding/attribution must align to the same latest replay day for the queried scope.
    6. Boundary checks: add at least one negative test for invalid inputs that should be rejected (no post-hoc correction workflow).
  - **Tolerance policy (mandatory):** Define explicit tolerances in tests/code comments for each audit class (identity, recursion, attribution, aggregation). Never rely on implicit “looks close enough”.
  - **Scope additivity rule (mandatory):** For `segregated` strategies, account-vs-strategy aggregation must be additive under the defined metric. For `shared_account_cash`, non-additivity is expected and must be explicitly labeled in UI/API semantics to prevent misuse.
  - **Time-slice consistency rule (mandatory):** Holdings, performance, attribution, and related KPIs for a scope must be computed and returned on a consistent latest replay date. Returning stale holdings against newer NAV is forbidden.
  - **Funding constraint rule (mandatory):** When financing is disabled, trade entry/update must hard-reject records that would make account cash negative or (for segregated strategies with allocated budget) strategy cash negative at trade time. If financing is introduced later, allowed over-100% exposure must be controlled by explicit financing balances/limits and audited formulas.
  - **Delivery evidence rule (mandatory):** For every finance-critical change, include a concise audit note in the response/PR: formulas checked, scopes checked, tests run, and known non-additivity semantics (if any).
  ...
  - **Futures trend research price basis (mandatory):** On the futures research trend backtest, **signals, execution / strategy return compounding (`backtesting.py` and lot engine), and benchmark buy-and-hold** all use **only** the synthetic **backward-adjusted (hfq)** continuous series stored as `{root}889`. There is **no** fallback to `{root}88`, `{root}888`, or main-contract `*0` **none** rows; if `{root}889` prices are missing for the requested window, the backtest must **fail with a clear error** (per-symbol skip entries in `meta.skipped` / aggregated `missing_synthetic_hfq_continuous` when every symbol lacks 889).
  - **Futures research correlation matrix (mandatory):** The futures group correlation and correlation-select APIs compute Pearson correlation on **daily log returns** (`diff(log(close))`) using **hfq** closes from the synthetic continuous **`{root}889`** series per pool root; matrix row/column labels remain **pool codes** (e.g. `RB0`). Requires synthesized 889 data in range (same as trend research).
