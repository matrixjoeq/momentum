...
function _rollingDrawdown(nav, windowDays) {
  ...
  for (let i = w - 1; i < n; i++) {
    ...
    if (!Number.isFinite(cur) || !Number.isFinite(peak) || peak <= 0) {
      out[i] = null;
      continue;
    }
    out[i] = cur / peak - 1;
...
function pickRangeDates(dates, key) {
  ...
  if (!cfg.days)
    return { start: dates[0], end: dates[n - 1], label: cfg.label };
...
async function drawAll(page, payload) {
  ...
  const theme =
    page && page.data && page.data.theme ? page.data.theme : "light";
  ...
  const rcVals = rcRows.map((x) =>
    x.return_share == null ? NaN : Number(x.return_share),
  );
  ...
  const rkVals = rkRows.map((x) =>
    x.risk_share == null ? NaN : Number(x.risk_share),
  );
  ...
  return {
    ...
    applyTheme(nextMode) {
      const modeRaw = String(
        nextMode ||
          this.data.themeMode ||
          _safeGetStorage(THEME_MODE_KEY, "light"),
      );
    ...
    setThemeMode(e) {
      const m =
        e && e.currentTarget && e.currentTarget.dataset
          ? String(e.currentTarget.dataset.m || "")
          : "light";
      this.applyTheme(m);
    },
    ...
    switchPage(e) {
      const p =
        e && e.currentTarget && e.currentTarget.dataset
          ? String(e.currentTarget.dataset.p || "")
          : "";
    ...
    async loadRange(k) {
      try {
        ...
        const baseLitePath = isMix
          ? "/analysis/baseline/weekly5-ew-dashboard-combo-lite"
          : "/analysis/baseline/weekly5-ew-dashboard-lite";
        const baseFullPath = isMix
          ? "/analysis/baseline/weekly5-ew-dashboard-combo"
          : "/analysis/baseline/weekly5-ew-dashboard";
        const rotLitePath = isMix
          ? "/analysis/rotation/weekly5-open-combo-lite"
          : "/analysis/rotation/weekly5-open-lite";
        const rotFullPath = isMix
          ? "/analysis/rotation/weekly5-open-combo"
          : "/analysis/rotation/weekly5-open";
        ...
        if (
          !baseLite ||
          !baseLite.by_anchor ||
          !baseLite.by_anchor[String(anchor)] ||
          !baseLite.meta ||
          baseLite.meta.end !== end0 ||
          baseLite.meta.start !== start0
        ) {
        ...
        try {
          ...
          if (
            !rotBaseLite ||
            !rotBaseLite.by_anchor ||
            !rotBaseLite.by_anchor[String(anchor)] ||
            !rotBaseLite.meta ||
            rotBaseLite.meta.end !== end0 ||
            rotBaseLite.meta.start !== start0
          ) {
            rotBaseLite = await request(rotLitePath, {
              method: "POST",
              data: isMix
                ? { start: start0, end: end0 }
                : { start: start0, end: end0, anchor_weekday: Number(anchor) },
            });
            if (this.__unloaded || token !== this.__loadToken) return;
          ...
          if (picked.start && picked.end && k !== "all") {
            const rotRes = await request(rotLitePath, {
              method: "POST",
              data: isMix
                ? { start: ymd(picked.start), end: ymd(picked.end) }
                : {
                    start: ymd(picked.start),
                    end: ymd(picked.end),
                    anchor_weekday: Number(anchor),
        ...
        const yearPage = 1;
        const yearlyCells = yearlyAll.slice(
          Math.max(0, yearlyAll.length - yearPageSize),
          yearlyAll.length,
        );
        ...
        const modeLabel =
          mode === "monthly" ? "月度" : mode === "yearly" ? "年度" : "日度";
        ...
        (async () => {
          try {
            ...
            const fullRes = await request(baseFullPath, {
              method: "POST",
              data: isMix
                ? { start: s1, end: e1, rebalance_shift: "prev" }
                : {
                    start: s1,
                    end: e1,
                    rebalance_shift: "prev",
            ...
            if (dataFull) {
              ...
              const dailyCells2 = dm2.month
                ? _buildMonthCells(dm2.month, dm2.map)
                : [];
              const monthlyYears2 = _buildMonthlyYearCells(cc.monthly || {});
              const yearlyAll2 = _buildYearlyCells(cc.yearly || {});
              const myIdx2 = monthlyYears2.length
                ? monthlyYears2.length - 1
                : 0;
              const my2 = monthlyYears2[myIdx2] || { year: "", cells: [] };
              const yearPageSize2 = 12;
              const yearPage2 = 1;
              const yearlyCells2 = yearlyAll2.slice(
                Math.max(0, yearlyAll2.length - yearPageSize2),
                yearlyAll2.length,
              );
            ...
            const rotFullRes = await request(rotFullPath, {
              method: "POST",
              data: isMix
                ? { start: s1, end: e1 }
                : { start: s1, end: e1, anchor_weekday: Number(anchor) },
            });
            if (this.__unloaded || token !== this.__loadToken) return;
        ...
        try {
          const plan = isMix
            ? await request("/analysis/rotation/next-plan-auto", {
                method: "POST",
                data: { asof: end0 },
              })
            : await request("/analysis/rotation/next-plan", {
                method: "POST",
                data: { anchor_weekday: Number(anchor), asof: end0 },
              });
          if (this.__unloaded || token !== this.__loadToken) return;
    ...
    async _drawRotation(rotFull) {
      ...
      const rsiRot14 =
        (((rotFull.nav_rsi || {}).series || {}).ROTATION || {})["14"] ||
        _rsiWilder(navRot, 14);
      ...
      const ratioBbu = ratioEma.map((v, i) =>
        v != null && ratioSd[i] != null
          ? Number(v) + 2 * Number(ratioSd[i])
          : null,
      );
      const ratioBbl = ratioEma.map((v, i) =>
        v != null && ratioSd[i] != null
          ? Number(v) - 2 * Number(ratioSd[i])
          : null,
      );
      const ratioRsi14 = _rsiWilder(
        ratio.map((x) => (x == null ? NaN : x)),
        14,
      );
      ...
      const diff40 = rot40.map((v, i) =>
        v == null || ew40[i] == null ? null : Number(v) - Number(ew40[i]),
      );
      ...
      const annualTurnover =
        ms.avg_daily_turnover == null
          ? NaN
          : Number(ms.avg_daily_turnover) * 252;
      const rm = {
        ...
        max_drawdown_recovery_days:
          ms.max_drawdown_recovery_days == null
            ? "-"
            : String(ms.max_drawdown_recovery_days),
        ...
        excess_max_drawdown_recovery_days:
          mex.max_drawdown_recovery_days == null
            ? "-"
            : String(mex.max_drawdown_recovery_days),
      ...
      const monthlyYears = _buildMonthlyYearCells({
        dates: monDates,
        values: monVals,
      });
      ...
      const pageText = rows.length
        ? `第 ${pg}/${pages} 页（每页 ${ps} 期）`
        : "无逐期数据";
      ...
      const measure = async (id) => {
        const r = await new Promise((resolve) => {
          wx.createSelectorQuery()
            .in(this)
            .select(`#${id}`)
            .boundingClientRect((x) => resolve(x))
            .exec();
        });
        this.__rects[id] = r;
        return r;
      ...
      drawLineChart(wx.createCanvasContext("rNav", this), {
        width: Math.floor(r1.width || 320),
        height: Math.floor(r1.height || 220),
        x: dates,
        yMode: "linear",
        title: "轮动净值 vs 等权",
      ...
      drawLineChart(wx.createCanvasContext("rDD", this), {
        width: Math.floor(r2.width || 320),
        height: Math.floor(r2.height || 200),
        x: dates,
        yMode: "linear",
        title: "回撤对比",
        yLabelFmt: fmtPct,
      ...
      drawLineChart(wx.createCanvasContext("rRSI", this), {
        width: Math.floor(r3.width || 320),
        height: Math.floor(r3.height || 180),
        x: dates,
        yMode: "linear",
        yFixed: [0, 100],
        title: "策略 RSI14",
        theme: this.data.theme,
        series: [{ name: "RSI14", y: rsiRot14, color: "#7b61ff", width: 1.4 }],
      });

      // 4) rolling 3y return
      const r4 = await measure("rRR3Y");
      drawLineChart(wx.createCanvasContext("rRR3Y", this), {
        width: Math.floor(r4.width || 320),
        height: Math.floor(r4.height || 180),
        x: dates,
        yMode: "linear",
        title: "策略滚动三年收益率",
        yLabelFmt: fmtPct,
      ...
      drawLineChart(wx.createCanvasContext("rMDD3Y", this), {
        width: Math.floor(r5.width || 320),
        height: Math.floor(r5.height || 180),
        x: dates,
        yMode: "linear",
        title: "策略滚动三年回撤",
        yLabelFmt: fmtPct,
      ...
      drawLineChart(wx.createCanvasContext("rRatio", this), {
        width: Math.floor(r6.width || 320),
        height: Math.floor(r6.height || 220),
        x: dates,
        yMode: "linear",
        title: "净值比值 ROT/EW + EMA/BB",
        theme: this.data.theme,
        series: [
      ...
      drawLineChart(wx.createCanvasContext("rExDD", this), {
        width: Math.floor(r6b.width || 320),
        height: Math.floor(r6b.height || 180),
        x: dates,
        yMode: "linear",
        title: "超额回撤（ROT/EW）",
        yLabelFmt: fmtPct,
        theme: this.data.theme,
        series: [{ name: "EX DD", y: ddEx, color: "#ad1457", width: 1.4 }],
      });

      // 7) ratio rsi
      const r7 = await measure("rRatioRSI");
      drawLineChart(wx.createCanvasContext("rRatioRSI", this), {
        width: Math.floor(r7.width || 320),
        height: Math.floor(r7.height || 180),
        x: dates,
        yMode: "linear",
        yFixed: [0, 100],
        title: "比值 RSI14",
      ...
      drawLineChart(wx.createCanvasContext("rDiff40", this), {
        width: Math.floor(r8.width || 320),
        height: Math.floor(r8.height || 180),
        x: dates,
        yMode: "linear",
        title: "40日收益差（ROT-EW）",
        yLabelFmt: fmtPct,
        theme: this.data.theme,
        series: [{ name: "DIFF40", y: diff40, color: "#1677ff", width: 1.2 }],
      });
      ...
      drawBarChart(wx.createCanvasContext("rRC", this), {
        width: Math.floor(r10.width || 320),
        height: Math.floor(r10.height || 180),
        title: "收益贡献",
        theme: this.data.theme,
        labels: rcLabels,
        values: rcVals,
      ...
      drawBarChart(wx.createCanvasContext("rRiskC", this), {
        width: Math.floor(r11.width || 320),
        height: Math.floor(r11.height || 180),
        title: "风险贡献",
        theme: this.data.theme,
        labels: rkLabels,
        values: rkVals,
    ...
    setCalMode(e) {
      ...
      const mode =
        m === "monthly" || m === "yearly" || m === "daily" ? m : "daily";
      const modeLabel =
        mode === "monthly" ? "月度" : mode === "yearly" ? "年度" : "日度";
    ...
    setRotCalMode(e) {
      ...
      const label =
        mode === "daily" ? "日度" : mode === "monthly" ? "月度" : "年度";
    ...
    prevRotCalYear() {
      ...
      this.setData({
        rotCalMonthlyYearIdx: j,
        rotCalMonthlyYear: y.year,
        rotCalMonthlyCells: y.cells,
      });
    },
    ...
    prevRotYearPage() {
      ...
      this.setData({
        rotCalYearlyPage: next,
        rotCalYearlyCells: all.slice(start, end),
      });
    },
      ...
      const view = rows.slice(start, end).map((r, i) => {
        ...
        if (sells.length) {
          ...
          for (const leg of sells) {
            if (Number(leg.from_weight || 0) > Number(best.from_weight || 0))
              best = leg;
          }
          fromCode = best.code;
        }
        if (buys.length) {
          ...
          for (const leg of buys) {
            if (Number(leg.to_weight || 0) > Number(best.to_weight || 0))
              best = leg;
          }
          toCode = best.code;
        }
    ...
    prevCalYear() {
      ...
      this.setData({
        calMonthlyYearIdx: j,
        calMonthlyYear: y.year,
        calMonthlyCells: y.cells,
      });
    },
    ...
    prevYearPage() {
      ...
      this.setData({
        calYearlyPage: next,
        calYearlyCells: all.slice(start, end),
      });
    },
    ...
    onChartTouch(e) {
      try {
        ...
        const raw =
          id && id.startsWith("r") ? this.__rot || {} : this.__raw || {};
        ...
        if (id === "cNav") {
          ...
          lines = [
            `ROT=${fmtNum((s.ROTATION || [])[idx])}`,
            `EW=${fmtNum((s.EW_REBAL || [])[idx])}`,
          ];
        } else if (id === "rDD") {
          if (
            rotCache &&
            Array.isArray(rotCache.ddRot) &&
            Array.isArray(rotCache.ddEw)
          ) {
            lines = [
              `ROT_DD=${fmtPct(rotCache.ddRot[idx])}`,
              `EW_DD=${fmtPct(rotCache.ddEw[idx])}`,
            ];
          } else {
            const s = raw.nav && raw.nav.series ? raw.nav.series : {};
            lines = [
              `ROT_DD=${fmtPct(_drawdownFromNav(s.ROTATION || [])[idx])}`,
              `EW_DD=${fmtPct(_drawdownFromNav(s.EW_REBAL || [])[idx])}`,
            ];
          }
        } else if (id === "rRSI") {
          const rsiSeries =
            (((raw.nav_rsi || {}).series || {}).ROTATION || {})["14"] || [];
          ...
          if (rotCache && Array.isArray(rotCache.rr3y)) {
            ...
            const s = raw.nav && raw.nav.series ? raw.nav.series : {};
            lines = [
              `R3Y=${fmtPct(_rollingReturn(s.ROTATION || [], 3 * 252)[idx])}`,
            ];
          }
        } else if (id === "rMDD3Y") {
          if (rotCache && Array.isArray(rotCache.dd3y)) {
            ...
            const s = raw.nav && raw.nav.series ? raw.nav.series : {};
            lines = [
              `DD3Y=${fmtPct(_rollingDrawdown(s.ROTATION || [], 3 * 252)[idx])}`,
            ];
          }
        } else if (id === "rRatio") {
          if (
            rotCache &&
            Array.isArray(rotCache.ratio) &&
            Array.isArray(rotCache.ratioEma)
          ) {
            lines = [
            ...
            const bbu = ema.map((v, i) =>
              v != null && sd[i] != null ? Number(v) + 2 * Number(sd[i]) : null,
            );
            const bbl = ema.map((v, i) =>
              v != null && sd[i] != null ? Number(v) - 2 * Number(sd[i]) : null,
            );
            lines = [
              `RATIO=${fmtNum(ratio[idx])}`,
              `EMA252=${fmtNum(ema[idx])}`,
              `BBU=${fmtNum(bbu[idx])}`,
              `BBL=${fmtNum(bbl[idx])}`,
            ];
          }
          ...
          if (rotCache && Array.isArray(rotCache.diff40)) {
            ...
            const v =
              rot40[idx] == null || ew40[idx] == null
                ? null
                : Number(rot40[idx]) - Number(ew40[idx]);
