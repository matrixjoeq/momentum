...
def test_ingest_success_creates_batch_items_and_prices(
    ...
    with session_factory() as db:
        res = ingest_one_etf(
            db, ak=FakeAk(df), code="510300", start_date="20240101", end_date="20240131"
        )
    ...
    with session_factory() as db:
        ...
        items = list(
            db.execute(
                select(IngestionItem).where(IngestionItem.batch_id == res.batch_id)
            )
            .scalars()
            .all()
        )
        ...
        audits = list(
            db.execute(
                select(EtfPriceAudit).where(EtfPriceAudit.batch_id == res.batch_id)
            )
            .scalars()
            .all()
        )
...
def test_ingest_update_records_audit(session_factory: sessionmaker) -> None:
    ...
    with session_factory() as db:
        audits = list(
            db.execute(
                select(EtfPriceAudit).where(EtfPriceAudit.batch_id == res2.batch_id)
            )
            .scalars()
            .all()
        )
        ...
        items = list(
            db.execute(
                select(IngestionItem).where(IngestionItem.batch_id == res2.batch_id)
            )
            .scalars()
            .all()
        )
...
def test_ingest_validation_failure_keeps_data_unchanged(
    session_factory: sessionmaker,
) -> None:
    good = pd.DataFrame(
        {
    ...
    with session_factory() as db:
        ...
        batch = db.execute(
            select(IngestionBatch).where(IngestionBatch.id == res2.batch_id)
        ).scalar_one()
