...
def test_api_baseline_garch_volatility_contract_success(
    ...
    assert captured.get("include_model_comparison") is False


def test_api_baseline_garch_volatility_forwards_model_comparison_flag(
    api_client, monkeypatch
) -> None:
    import etf_momentum.api.routes as routes
    ...
    assert resp.status_code == 200
    assert captured.get("include_model_comparison") is True
    data = resp.json()
    assert data["ok"] is True
    assert data["diagnostics"]["model_comparison"]["enabled"] is True
...
def test_api_baseline_garch_volatility_rejects_invalid_payload(api_client) -> None:
    ...
    assert resp_flag.status_code == 422
