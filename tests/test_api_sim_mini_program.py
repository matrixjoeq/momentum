...
def sim_client(session_factory, engine):
    ...
    with sf() as db:
        ...
        days = [
            d.date()
            for d in __import__("pandas").date_range(start, periods=120, freq="B")
        ]
...
def test_sim_portfolio_init_generate_and_confirm_trade(sim_client):
    ...
    # 1) create portfolio
    pid = post_json(
        c, "/api/sim/portfolio", {"name": "默认账户", "initial_cash": 1000000}
    )["id"]

    # 2) init fixed strategy + 5 variants
    out = request_json(
        c, method="post", path=f"/api/sim/portfolio/{pid}/init-fixed-strategy"
    )
    ...
    assert (
        post_json(
            c,
            "/api/sim/decision/generate",
            {"portfolio_id": pid, "start": "20240301", "end": "20240430"},
        )["inserted"]
        > 0
    )
    ...
    decisions = get_json(
        c, f"/api/sim/variant/{vid0}/decisions?start=20240301&end=20240430"
    )["decisions"]
    assert len(decisions) > 0
    d0 = decisions[0]

    prev = post_json(
        c, "/api/sim/trade/preview", {"variant_id": vid0, "decision_id": d0["id"]}
    )
    assert prev["decision_id"] == d0["id"]

    assert (
        post_json(
            c, "/api/sim/trade/confirm", {"variant_id": vid0, "decision_id": d0["id"]}
        )["ok"]
        is True
    )
    ...
    request_json(
        c,
        method="post",
        path=f"/api/sim/mark-to-market?variant_id={vid0}&start=20240301&end=20240315",
    )
