...
def test_entry_momentum_rule_can_trigger_cash_when_all_blocked(session_factory):
    sf = session_factory
    with sf() as db:
        codes = ["AAA", "BBB"]
    ...
    # Should have some holding periods, and at least one should be cash (no picks).
    assert out["holdings"]
