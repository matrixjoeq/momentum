...
def backtest_rotation(
    ...
    *,
    ...
    event_study = compute_event_study(
        ...
        entry_dates=entry_dates_from_weight_membership_change(
    ...
    if need_ivol:
        ...
        metrics["excess_vs_inverse_vol_rebal"] = {
            ...
            "annualized_return_arith": float(ann_excess_ivol_arith),
            "annualized_volatility": float(ann_excess_ivol_vol),
            "information_ratio": float(ir_ivol),
    ...
    for p in holdings["periods"]:
        ...
        ivol_s = float(ivol_nav.loc[s])
        ivol_e = float(ivol_nav.loc[e])
        r_s = nav_e / nav_s - 1.0
        r_ew = ew_e / ew_s - 1.0
        r_rp = rp_e / rp_s - 1.0
        r_ivol = ivol_e / ivol_s - 1.0
        ex = float(r_s - r_ew)
        ex_rp = float(r_s - r_rp)
        ex_ivol = float(r_s - r_ivol)
        if ex > 0:
            wins += 1
            pos.append(ex)
        elif ex < 0:
        ...
        period_stats.append(
            {
                ...
                "inverse_vol_rebal_return": float(r_ivol),
                "excess_return": ex,
                "excess_return_rp": ex_rp,
                "excess_return_ivol": ex_ivol,
                "win": ex > 0,
                "win_rp": ex_rp > 0,
                "win_ivol": ex_ivol > 0,
                "buys": buys,
