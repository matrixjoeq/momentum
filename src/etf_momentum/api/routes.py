...
def baseline_analysis(
    ...
    if not bool(getattr(payload, "dynamic_universe", False)):
        close_chk = load_close_prices(
            db, codes=inp.codes, start=inp.start, end=inp.end, adjust=inp.adjust
        )
        missing = [
            c
            for c in inp.codes
            if c not in close_chk.columns or close_chk[c].dropna().empty
        ]
        if missing:
            raise HTTPException(
                status_code=400, detail=f"missing data for codes: {missing}"
            )
    try:
