...
def rotation_weekly5_open_sim(payload: RotationWeekly5OpenSimRequest, db: Session = Depends(get_session)) -> dict:
    ...
    def _build_base(*, start: dt.date, end: dt.date) -> RotationAnalysisInputs:
        ...
        return RotationAnalysisInputs(
            ...
            # fixed semantics of this endpoint
            rebalance="weekly",
            rebalance_shift="prev",
            rebalance_anchor=None,
            exec_price="open",
            # apply all rotation params from payload
            top_k=int(payload.top_k),
            position_mode=str(payload.position_mode or "adaptive"),
            entry_backfill=bool(payload.entry_backfill),
            entry_match_n=int(payload.entry_match_n),
            exit_match_n=int(payload.exit_match_n),
            lookback_days=int(payload.lookback_days),
            skip_days=int(payload.skip_days),
            risk_off=bool(payload.risk_off),
            defensive_code=payload.defensive_code,
            momentum_floor=float(payload.momentum_floor),
            score_method=str(payload.score_method or "raw_mom"),
            score_lambda=float(getattr(payload, "score_lambda", 0.0) or 0.0),
            score_vol_power=float(getattr(payload, "score_vol_power", 1.0) or 1.0),
            risk_free_rate=float(getattr(payload, "risk_free_rate", 0.025) or 0.025),
            cost_bps=float(payload.cost_bps),
            tp_sl_mode=str(payload.tp_sl_mode or "none"),
            atr_window=payload.atr_window,
            atr_mult=float(payload.atr_mult),
            atr_step=float(payload.atr_step),
            atr_min_mult=float(payload.atr_min_mult),
            corr_filter=bool(payload.corr_filter),
            corr_window=payload.corr_window,
            corr_threshold=float(payload.corr_threshold),
            inertia=bool(payload.inertia),
            inertia_min_hold_periods=int(payload.inertia_min_hold_periods),
            inertia_score_gap=float(payload.inertia_score_gap),
            inertia_min_turnover=float(payload.inertia_min_turnover),
            rr_sizing=bool(payload.rr_sizing),
            ...
            trend_exit_filter=bool(payload.trend_exit_filter),
            trend_sma_window=int(payload.trend_sma_window),
            trend_ma_type=str(payload.trend_ma_type or "sma"),
            bias_filter=bool(payload.bias_filter),
            bias_exit_filter=bool(payload.bias_exit_filter),
            bias_ma_window=int(payload.bias_ma_window),
            bias_level_window=str(payload.bias_level_window or "all"),
            bias_threshold_type=str(payload.bias_threshold_type or "quantile"),
            bias_quantile=float(payload.bias_quantile),
            bias_fixed_value=float(payload.bias_fixed_value),
            bias_min_periods=int(payload.bias_min_periods),
            ...
            rsi_block_overbought=bool(payload.rsi_block_overbought),
            rsi_block_oversold=bool(payload.rsi_block_oversold),
            vol_monitor=bool(payload.vol_monitor),
            vol_window=int(payload.vol_window),
            vol_target_ann=float(payload.vol_target_ann),
            vol_max_ann=float(payload.vol_max_ann),
            chop_filter=bool(payload.chop_filter),
            chop_mode=str(payload.chop_mode or "er"),
            chop_window=int(payload.chop_window),
            chop_er_threshold=float(payload.chop_er_threshold),
            chop_adx_window=int(payload.chop_adx_window),
            chop_adx_threshold=float(payload.chop_adx_threshold),
            asset_rc_rules=[r.model_dump() for r in payload.asset_rc_rules] if payload.asset_rc_rules else None,
            asset_vol_index_rules=asset_vol_rules,
            vol_index_close=vol_index_close,
        )
...
def rotation_next_plan(payload: dict, db: Session = Depends(get_session)) -> dict:
    ...
    try:
        ...
        bt_inp = RotationAnalysisInputs(
            ...
            position_mode=str(wk_req.position_mode or "adaptive"),
            entry_backfill=bool(wk_req.entry_backfill),
            entry_match_n=int(wk_req.entry_match_n),
            exit_match_n=int(wk_req.exit_match_n),
            ...
            trend_exit_filter=bool(wk_req.trend_exit_filter),
            trend_sma_window=int(wk_req.trend_sma_window),
            trend_ma_type=str(wk_req.trend_ma_type or "sma"),
            bias_filter=bool(wk_req.bias_filter),
            bias_exit_filter=bool(wk_req.bias_exit_filter),
            bias_ma_window=int(wk_req.bias_ma_window),
            bias_level_window=str(wk_req.bias_level_window or "all"),
            bias_threshold_type=str(wk_req.bias_threshold_type or "quantile"),
            bias_quantile=float(wk_req.bias_quantile),
            bias_fixed_value=float(wk_req.bias_fixed_value),
            bias_min_periods=int(wk_req.bias_min_periods),
            ...
            asset_bias_rules=[r.model_dump() for r in (wk_req.asset_bias_rules or [])] if wk_req.asset_bias_rules else None,
