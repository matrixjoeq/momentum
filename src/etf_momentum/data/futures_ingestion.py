...
def _normalize_futures_df(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=["trade_date", "open", "high", "low", "close", "volume", "open_interest", "amount"])
    ...
    oi_col = _pick_col(cols, ("持仓量", "持仓", "open_interest", "oi"))
    ...
    out["open_interest"] = pd.to_numeric(df[oi_col], errors="coerce") if oi_col is not None else None
...
def ingest_one_futures(
    ...
    *,
    ...
    if norm.empty:
        msg = "no futures data in requested range"
        mark_futures_fetch_status(db, code=code, status="failed", message=msg)
        db.commit()
        return IngestFuturesResult(code=code, upserted=0, status="failed", message=msg)
    ...
    rows = [
        FuturesPriceRow(
            ...
            open_interest=_to_float(row.open_interest),
            source="sina",
            adjust="none",
        )
    ...
    msg = f"none={len(rows)} source=sina"
