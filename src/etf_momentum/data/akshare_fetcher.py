...
def fetch_etf_daily_qfq(
    ...
    if missing:
        raise ValueError(
            f"akshare result missing columns: {missing}. columns={list(df.columns)}"
        )
    ...
    for _, r in df2.iterrows():
        rows.append(
            PriceRow(
                ...
                high=float(r["high"])
                if ("high" in df2.columns and pd.notna(r.get("high")))
                else None,
                low=float(r["low"])
                if ("low" in df2.columns and pd.notna(r.get("low")))
                else None,
                close=float(r["close"]) if pd.notna(r["close"]) else None,
                volume=float(r["volume"])
                if "volume" in df2.columns and pd.notna(r.get("volume"))
                else None,
                amount=float(r["amount"])
                if "amount" in df2.columns and pd.notna(r.get("amount"))
                else None,
                source="eastmoney",
                adjust=req.adjust,
            )
