...
def logical_rollback_batch(db: Session, batch: IngestionBatch) -> None:
    ...
    ins_dates = list(
        db.execute(
            select(IngestionItem.trade_date).where(
                IngestionItem.batch_id == batch.id, IngestionItem.action == "insert"
            )
        ).all()
    )
    if ins_dates:
    ...
    audits = list(
        db.execute(select(EtfPriceAudit).where(EtfPriceAudit.batch_id == batch.id))
        .scalars()
        .all()
    )
    for a in audits:
        row = db.execute(
            select(EtfPrice).where(
                EtfPrice.code == a.code,
                EtfPrice.trade_date == a.trade_date,
                EtfPrice.adjust == a.adjust,
            )
        ).scalar_one_or_none()
        if row is None:
            # should not happen: update implies existed; but keep safe by inserting
            row = EtfPrice(
                code=a.code, trade_date=a.trade_date, adjust=a.adjust, source=a.source
            )
            db.add(row)
        row.open = a.open
        row.high = a.high
    ...
    *,
    ...
    if batch is None:
        return RollbackResult(
            batch_id=batch_id, status="failed", message="batch not found"
        )
    ...
    if (
        batch.val_max_abs_return is not None
        and batch.val_max_hl_spread is not None
        and batch.val_max_gap_days is not None
    ):
        policy = ValidationPolicyParams(
        ...
        policy = ValidationPolicyParams(
            max_abs_return=0.35, max_hl_spread=0.6, max_gap_days=15
        )
    try:
        logical_rollback_batch(db, batch)
        update_ingestion_batch(
            db, batch_id=batch.id, status="rolled_back", message="logical rollback"
        )
        db.commit()
    except Exception as e:  # pylint: disable=broad-exception-caught
        db.rollback()
        update_ingestion_batch(
            db, batch_id=batch.id, status="rollback_failed", message=str(e)
        )
        db.commit()
    ...
    if pre_fp is not None:
        cur_fp = compute_price_fingerprint(
            db, code=batch.code, start_date=start_d, end_date=end_d, adjust=adj
        )
        if cur_fp != pre_fp:
            needs_snapshot = True
    ...
    if not needs_snapshot:
        ...
        db.commit()
        return RollbackResult(
            batch_id=batch.id, status="success", message="rolled back (validated)"
        )

    if validation_exc is not None:
        return RollbackResult(
            batch_id=batch.id, status="failed", message=validation_exc.to_json()
        )
    return RollbackResult(
        batch_id=batch.id,
        status="failed",
        message="rollback validation/fingerprint mismatch",
    )
