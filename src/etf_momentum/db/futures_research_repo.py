...
from .models import FuturesPool, FuturesResearchGroup, FuturesResearchGroupItem, FuturesResearchState
...
def get_futures_research_state(db: Session) -> FuturesResearchState:
    obj = db.execute(select(FuturesResearchState).where(FuturesResearchState.id == 1)).scalar_one_or_none()
    if obj is None:
...
def get_futures_group(db: Session, *, name: str) -> FuturesResearchGroup | None:
    return db.execute(select(FuturesResearchGroup).where(FuturesResearchGroup.name == name)).scalar_one_or_none()


def list_futures_groups(db: Session) -> list[FuturesGroupData]:
    groups = list(db.execute(select(FuturesResearchGroup).order_by(FuturesResearchGroup.name.asc())).scalars().all())
    if not groups:
        return []
    ...
    rows = list(
        db.execute(
            ...
            .order_by(FuturesResearchGroupItem.group_id.asc(), FuturesResearchGroupItem.sort_order.asc(), FuturesResearchGroupItem.code.asc())
        ).scalars().all()
    )
    for r in rows:
        gid_to_codes.setdefault(int(r.group_id), []).append(str(r.code))
    return [
        FuturesGroupData(name=str(g.name), codes=list(gid_to_codes.get(int(g.id), [])), is_active=bool(g.is_active))
        for g in groups
    ]
...
def get_active_futures_group(db: Session) -> FuturesGroupData | None:
    g = db.execute(select(FuturesResearchGroup).where(FuturesResearchGroup.is_active.is_(True))).scalar_one_or_none()
    if g is None:
        return None
    rows = list(
        db.execute(
            ...
            .order_by(FuturesResearchGroupItem.sort_order.asc(), FuturesResearchGroupItem.code.asc())
        ).scalars().all()
    )
    return FuturesGroupData(name=str(g.name), codes=[str(x.code) for x in rows], is_active=True)
    ...
    *,
    ...
    for c in input_codes:
        ...
        if c in valid:
            ...
            skipped.append(c)
    db.execute(delete(FuturesResearchGroupItem).where(FuturesResearchGroupItem.group_id == existing.id))
    if deduped:
        db.add_all(
            [
                FuturesResearchGroupItem(group_id=int(existing.id), code=code, sort_order=i)
    ...
    db.flush()
    return (
        FuturesGroupData(name=str(existing.name), codes=list(deduped), is_active=bool(existing.is_active)),
        skipped,
    )
...
def delete_futures_group(db: Session, *, name: str) -> bool:
    ...
    if g is None:
        return False
    db.execute(delete(FuturesResearchGroupItem).where(FuturesResearchGroupItem.group_id == g.id))
    db.delete(g)
    db.flush()
    return True
