...
def validate_price_series(
    ...
    *,
    ...
    if any(
        points[i].trade_date >= points[i + 1].trade_date for i in range(len(points) - 1)
    ):
    ...
    for p in points:
        ...
        if (
            not _is_finite_positive(p.open)
            or not _is_finite_positive(p.high)
            or not _is_finite_positive(p.low)
            or not _is_finite_positive(p.close)
        ):
        ...
        if (
            ...
            if not (p.low <= min(p.open, p.close) <= max(p.open, p.close) <= p.high):
                _raise(
                    ...
                    "OHLC invalid",
                    trade_date=p.trade_date,
                    detail="low <= min(open,close) <= max(open,close) <= high",
                )
            if p.low <= 0:
                _raise(
                    "low_nonpositive",
            ...
            if hl_spread > policy.max_hl_spread:
                _raise(
                    "hl_spread_exceeded",
                    "high/low spread too large",
                    trade_date=p.trade_date,
                    value=hl_spread,
                    threshold=policy.max_hl_spread,
                )
        ...
        if prev_date is not None:
            ...
            if gap > policy.max_gap_days:
                _raise(
                    ...
                    trade_date=p.trade_date,
                    value=gap,
                    threshold=policy.max_gap_days,
                    detail=f"{prev_date.isoformat()} -> {p.trade_date.isoformat()}",
                )
        ...
        if prev_close is not None and p.close is not None:
            ...
            if abs(r) > policy.max_abs_return:
                _raise(
                    "abs_return_exceeded",
                    "abs return too large",
                    trade_date=p.trade_date,
                    value=r,
                    threshold=policy.max_abs_return,
                )
