...
def _pool_membership_mask(
    db: Session,
    *,
    codes: list[str],
    index: pd.Index,
) -> pd.DataFrame:
    """Return the causal configured membership interval for each asset."""
    mask = pd.DataFrame(True, index=index, columns=codes, dtype=bool)
    ...
    try:
        ...
        rows = (
            db.query(EtfPool.code, EtfPool.start_date, EtfPool.end_date)
            .filter(EtfPool.code.in_(codes))
            .all()
        )
    except Exception:
        return mask
    for code, start_raw, end_raw in rows:
        key = str(code)
        if key not in mask.columns:
            continue
        if start_raw:
            start = pd.to_datetime(str(start_raw), format="%Y%m%d", errors="coerce")
            if pd.notna(start):
                mask.loc[mask.index < start, key] = False
        if end_raw:
            end = pd.to_datetime(str(end_raw), format="%Y%m%d", errors="coerce")
            if pd.notna(end):
                mask.loc[mask.index > end, key] = False
    return mask
...
DEFAULT_R_PROFIT_SCALEOUT_TIERS: list[dict[str, float]] = [
...
def _build_risk_control_compatibility_meta(
    *,
    ...
    enabled_norm = {str(k): bool(v) for k, v in (enabled_flags or {}).items()}
    violations: list[dict[str, Any]] = []
    for group in RISK_MUTEX_GROUPS:
        active = [name for name in group if bool(enabled_norm.get(name, False))]
        if len(active) > 1:
            violations.append(
                {
    ...
    return {
        "parallel_overlays": list(RISK_OVERLAY_KEYS),
        "mutually_exclusive_groups": [list(g) for g in RISK_MUTEX_GROUPS],
        "enabled": enabled_norm,
        "or_eligible_enabled": [
            key for key in RISK_OVERLAY_KEYS if bool(enabled_norm.get(key, False))
        ],
...
def _enforce_risk_control_compatibility(
    enabled_flags: dict[str, bool],
) -> dict[str, Any]:
    meta = _build_risk_control_compatibility_meta(enabled_flags=enabled_flags)
    violations = list(meta.get("violations") or [])
    if violations:
        first = violations[0]
        group = list(first.get("group") or [])
        raise ValueError(
            f"mutually exclusive risk controls cannot be enabled together: {', '.join(group)}"
        )
    return meta
...
class TrendInputs:
    ...
    ma_trailing_stop_reduce_fraction: float = 0.33
    ...
    cvar_risk_mgmt_enabled: bool = False
    cvar_window: int = 60
    ...
    vol_periodic_risk_mgmt_enabled: bool = False
    vol_periodic_rebalance_threshold_pct: float = 0.05
    ...
    ma_entry_filter_enabled: bool = False
    ma_entry_filter_type: str = "sma"
    ma_entry_filter_fast: int = 100
    ma_entry_filter_slow: int = 200
...
def _vol_regime_state_from_ratio(
    ratio: float,
    *,
    ...
    if not np.isfinite(float(ratio)):
        return "NORMAL"
    r = float(ratio)
    if r > float(extreme_threshold):
        return "EXTREME"
    if r > float(expand_threshold):
        return "REDUCED"
    if r < float(contract_threshold):
        return "INCREASED"
    return "NORMAL"


def _next_vol_regime_state(
    current_state: str,
    *,
    ratio: float,
    expand_threshold: float,
    ...
    st = str(current_state or "NORMAL").strip().upper()
    if st not in VOL_REGIME_STATES:
        st = "NORMAL"
    if not np.isfinite(float(ratio)):
        return st
    r = float(ratio)
    if st == "NORMAL":
        return _vol_regime_state_from_ratio(
            r,
            expand_threshold=float(expand_threshold),
            contract_threshold=float(contract_threshold),
...
def _risk_budget_dynamic_weights(
    ...
    *,
    ...
    - Supports four states: INCREASED / NORMAL / REDUCED / EXTREME.
    - State changes can jump across any tiers in one step.
    - Whenever state changes, rebalance once to latest base_target weight.
    """
    ...
    state = "FLAT"  # FLAT | NORMAL | REDUCED | INCREASED | EXTREME
    held_weight = 0.0
    stats = {
    ...
    for d in idx:
        ...
        if state == "FLAT":
            ...
            if dynamic_enabled:
                ...
                state = _vol_regime_state_from_ratio(
                    r,
                    expand_threshold=float(expand_threshold),
                    contract_threshold=float(contract_threshold),
                    extreme_threshold=float(extreme_threshold),
        ...
        if dynamic_enabled:
            ...
            if can_recalc and next_state != state:
                old_state = str(state)
                ...
                if state == "NORMAL" and old_state in {"REDUCED", "EXTREME"}:
                    stats["vol_risk_adjust_recover_from_expand_count"] += 1
                if state == "NORMAL" and old_state == "INCREASED":
...
def _risk_budget_periodic_weights(
    ...
    *,
    ...
    """
    Risk-budget weights with periodic ATR-based rebalance threshold.

    When in position, compare the target share proxy (risk_budget_pct / ATR)
    against the current held share proxy (held_weight / close). Rebalance only
    when relative change reaches threshold.
    """
    idx = active_signal.index
    sig = active_signal.reindex(idx).astype(float).fillna(0.0).clip(lower=0.0)
    px = close.reindex(idx).astype(float)
...
def _apply_cvar_budget_scaling(
    ...
    *,
    ...
    r = (
        asset_returns.reindex(index=w.index, columns=w.columns)
        .astype(float)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
    )
    ...
    scales = pd.Series(1.0, index=w.index, dtype=float)
    realized_hist: list[float] = []
    prev_scaled = pd.Series(0.0, index=w.columns, dtype=float)
    fallback_reasons: set[str] = set()
    fallback_last_reason: str | None = None
    fallback_count = 0
    ...
    in_trigger_episode = False
    eps = 1e-12
    for d in w.index:
        row = w.loc[d].astype(float).fillna(0.0)
        scale = 1.0
        reason: str | None = None
        if not bool(enabled):
            reason = "disabled"
        elif len(realized_hist) < win:
            reason = "insufficient_samples"
        else:
            hist = pd.Series(realized_hist[-win:], dtype=float)
            _, cvar_loss = _historical_var_cvar_loss(hist, confidence=0.95)
            if cvar_loss is None or (not np.isfinite(float(cvar_loss))):
                reason = "invalid_cvar"
            elif float(cvar_loss) <= eps:
                reason = "non_positive_cvar"
            else:
                cvar_estimate_available_count += 1
                scale = float(min(1.0, budget / max(float(cvar_loss), eps)))
        if reason is not None:
            fallback_count += 1
            fallback_reasons.add(str(reason))
            fallback_last_reason = str(reason)
            scale = 1.0
        if scale < 1.0 - eps:
            scale_applied_count += 1
            d_str = (
                pd.Timestamp(d).date().isoformat()
                if not pd.isna(pd.Timestamp(d))
                else str(d)
            )
            trigger_dates.append(d_str)
            if scale > 0.8:
                trigger_bucket_counts["mild_0.8_1.0"] += 1
            elif scale > 0.5:
                trigger_bucket_counts["medium_0.5_0.8"] += 1
            else:
                trigger_bucket_counts["strong_0.0_0.5"] += 1
            if not in_trigger_episode:
                trigger_episode_count += 1
                in_trigger_episode = True
        else:
            in_trigger_episode = False
        scales.loc[d] = float(scale)
        scaled_row = (row * float(scale)).astype(float)
        out.loc[d] = scaled_row.to_numpy(dtype=float)
        ret_row = r.loc[d].astype(float).fillna(0.0)
        realized = float((prev_scaled * ret_row).sum())
        if np.isfinite(realized):
            realized_hist.append(realized)
        prev_scaled = scaled_row
    stats = {
        ...
        "cvar_scale_trigger_count": int(scale_applied_count),
        "cvar_scale_trigger_episode_count": int(trigger_episode_count),
        "cvar_scale_trigger_count_by_bucket": {
            k: int(v) for k, v in trigger_bucket_counts.items()
        },
        "cvar_scale_trigger_days": int(len(set(trigger_dates))),
        "cvar_scale_trigger_dates": list(dict.fromkeys(trigger_dates))[:200],
        "cvar_estimate_available_count": int(cvar_estimate_available_count),
        "cvar_scale_fallback_count": int(fallback_count),
        "cvar_scale_fallback_reasons": sorted(list(fallback_reasons)),
        "cvar_scale_fallback_last_reason": fallback_last_reason,
    ...
    return out.astype(float), stats


def _append_var_cvar_metrics(
    metrics: dict[str, Any],
    ret: pd.Series,
) -> None:
    var95, cvar95 = _historical_var_cvar_loss(ret, confidence=0.95)
    metrics["var_95"] = (
        float(var95)
...
def _next_trading_day_iso(index: pd.Index, i: int) -> str | None:
    if i + 1 >= len(index):
        return None
    d = index[i + 1]
...
def _max_drawdown_from_trade_returns(values: list[float]) -> float | None:
    ...
    if arr.size == 0:
        return None
    factors = np.maximum(1.0 + arr, 1e-12)
...
def _risk_of_ruin_probability(
    *,
    p_win: float | None,
    avg_loss_abs: float | None,
    max_risk: float | None,
) -> float | None:
    p = float(p_win) if p_win is not None else float("nan")
    a = float(avg_loss_abs) if avg_loss_abs is not None else float("nan")
    mr = float(max_risk) if max_risk is not None else float("nan")
    if not (np.isfinite(p) and np.isfinite(a) and np.isfinite(mr)):
        return None
    if p <= 0.0 or p >= 1.0 or a <= 0.0 or mr < 0.0:
        return None
    base = (1.0 - p) / p
    exp = mr / a
...
def _trade_stats_from_returns(
    ...
    *,
    ...
    risk_of_ruin_maxrisk: float | None = None,
) -> dict[str, Any]:
    ...
    max_risk_trade_nav = _max_drawdown_from_trade_returns(rs)
    cfg_maxrisk = (
        float(risk_of_ruin_maxrisk)
        if risk_of_ruin_maxrisk is not None and np.isfinite(float(risk_of_ruin_maxrisk))
        else None
    )
    maxrisk_for_formula = (
        cfg_maxrisk
        if cfg_maxrisk is not None and cfg_maxrisk > 0.0
        else max_risk_trade_nav
    )
    maxrisk_basis = (
        "configured_risk_of_ruin_maxrisk"
        if cfg_maxrisk is not None and cfg_maxrisk > 0.0
        else "max_drawdown_of_trade_return_nav"
    )
    risk_of_ruin = _risk_of_ruin_probability(
        p_win=win_rate_ex_zero,
        avg_loss_abs=avg_loss_abs_ex_zero,
        max_risk=maxrisk_for_formula,
    )
    return {
        ...
        "risk_of_ruin": {
            "formula": "((1-P)/P)^(maxrisk/A)",
            "probability": risk_of_ruin,
            "P": win_rate_ex_zero,
            "A": avg_loss_abs_ex_zero,
            "maxrisk": maxrisk_for_formula,
            "P_basis": "win_rate_ex_zero",
            "A_basis": "abs(mean_loss_ex_zero)",
            "maxrisk_basis": maxrisk_basis,
            "maxrisk_configured": cfg_maxrisk,
            "maxrisk_drawdown_reference": max_risk_trade_nav,
        },
    }


def _semi_variance_run_stats_from_returns(
    values: list[float], *, flat_eps: float = 1e-12
) -> dict[str, Any]:
    rs = [float(x) for x in (values or []) if np.isfinite(float(x))]
    eps = float(abs(float(flat_eps)))
    pos_daily = np.asarray([x for x in rs if x > eps], dtype=float)
    neg_daily = np.asarray([x for x in rs if x < -eps], dtype=float)

    upside_semivariance = (
        float(np.mean(np.square(pos_daily))) if pos_daily.size > 0 else None
    )
    downside_semivariance = (
        float(np.mean(np.square(neg_daily))) if neg_daily.size > 0 else None
    )

    run_returns: list[float] = []
    profit_runs: list[float] = []
    loss_runs: list[float] = []
    profit_run_counts: list[int] = []
    loss_run_counts: list[int] = []
    run_sign = 0  # +1 profit-run, -1 loss-run, 0 no run
    run_values: list[float] = []

    def _flush_run() -> None:
        nonlocal run_sign, run_values
        if run_sign == 0 or not run_values:
            run_sign = 0
            run_values = []
            return
        cum_ret = float(np.prod(1.0 + np.asarray(run_values, dtype=float)) - 1.0)
        run_returns.append(cum_ret)
        if run_sign > 0:
            profit_runs.append(cum_ret)
            profit_run_counts.append(int(len(run_values)))
        else:
            loss_runs.append(cum_ret)
            loss_run_counts.append(int(len(run_values)))
        run_sign = 0
        run_values = []
    ...
    for r in rs:
        ...
        if sign != run_sign:
            ...
            run_values = [float(r)]
            continue
        run_values.append(float(r))
    _flush_run()

    run_trade_stats = _trade_stats_from_returns(run_returns, flat_eps=eps)
    return {
        "total_segments": int(len(run_returns)),
        "profit_segments": int(len(profit_runs)),
        "loss_segments": int(len(loss_runs)),
        "win_rate_ex_zero": run_trade_stats.get("win_rate_ex_zero"),
        "payoff_ex_zero": run_trade_stats.get("payoff_ex_zero"),
        "kelly_ex_zero": run_trade_stats.get("kelly_ex_zero"),
        "upside_semivariance": upside_semivariance,
        "downside_semivariance": downside_semivariance,
        "profit_return_stats": _dist_stats(profit_runs),
        "profit_count_stats": _dist_stats([float(x) for x in profit_run_counts]),
        "loss_return_stats": _dist_stats(loss_runs),
        "loss_count_stats": _dist_stats([float(x) for x in loss_run_counts]),
    }
...
def _trade_metric_series_for_code(
    ...
    *,
    ...
    if isinstance(series_or_df, pd.Series):
        return pd.to_numeric(series_or_df, errors="coerce").astype(float)
    if not isinstance(series_or_df, pd.DataFrame) or series_or_df.empty:
        return pd.Series(dtype=float)

    wanted: list[str] = []
    code_norm = str(code or "").strip()
    default_norm = str(default_code or "").strip()
    if code_norm:
        wanted.append(code_norm)
    if default_norm and default_norm not in wanted:
        wanted.append(default_norm)

    for key in wanted:
        for col in series_or_df.columns:
            if str(col) == key:
                return pd.to_numeric(series_or_df[col], errors="coerce").astype(float)
    if series_or_df.shape[1] == 1:
        return pd.to_numeric(series_or_df.iloc[:, 0], errors="coerce").astype(float)
    return pd.Series(dtype=float)


def _trade_extreme_records_from_trades(
    trades: list[dict[str, Any]],
    *,
    exec_price: pd.Series | pd.DataFrame,
    high_price: pd.Series | pd.DataFrame,
    low_price: pd.Series | pd.DataFrame,
    default_code: str | None = None,
) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    cache: dict[str, tuple[pd.Series, pd.Series, pd.Series] | None] = {}
    for tr in list(trades or []):
        entry_date_raw = str((tr or {}).get("entry_date") or "").strip()
        exit_date_raw = str((tr or {}).get("exit_date") or "").strip()
        if not entry_date_raw or not exit_date_raw:
            continue
        entry_ts = pd.to_datetime(entry_date_raw, errors="coerce")
        exit_ts = pd.to_datetime(exit_date_raw, errors="coerce")
        if pd.isna(entry_ts) or pd.isna(exit_ts):
            continue
        if exit_ts < entry_ts:
            continue

        code_key = str((tr or {}).get("code") or default_code or "").strip()
        if not code_key:
            continue
        if code_key not in cache:
            px = _trade_metric_series_for_code(
                exec_price,
                code=code_key,
            ...
            if px.empty:
                ...
                hi = _trade_metric_series_for_code(
                    high_price,
                    code=code_key,
                    default_code=default_code,
                ).copy()
                hi.index = pd.to_datetime(hi.index, errors="coerce")
                hi = (
                    hi[~hi.index.isna()]
                    .sort_index()
                    .replace([np.inf, -np.inf], np.nan)
                ...
                lo = _trade_metric_series_for_code(
                    low_price,
                    code=code_key,
                    default_code=default_code,
                ).copy()
                lo.index = pd.to_datetime(lo.index, errors="coerce")
                lo = (
                    lo[~lo.index.isna()]
                    .sort_index()
                    .replace([np.inf, -np.inf], np.nan)
                ...
                cache[code_key] = (px, hi, lo)
        cached = cache.get(code_key)
        if cached is None:
            continue
        px, hi, lo = cached
        seg = px[(px.index >= entry_ts) & (px.index <= exit_ts)]
        if seg.empty:
            continue
        entry_px = float(seg.iloc[0]) if np.isfinite(float(seg.iloc[0])) else np.nan
        exit_px = float(seg.iloc[-1]) if np.isfinite(float(seg.iloc[-1])) else np.nan
        if not np.isfinite(entry_px) or entry_px <= 0.0 or not np.isfinite(exit_px):
            continue
        hi_seg = hi.reindex(seg.index)
        lo_seg = lo.reindex(seg.index)
        max_high = (
            float(hi_seg.max(skipna=True))
            if bool(pd.notna(hi_seg).any())
            else float(seg.max(skipna=True))
        )
        min_low = (
            float(lo_seg.min(skipna=True))
            if bool(pd.notna(lo_seg).any())
            else float(seg.min(skipna=True))
        )
        if not np.isfinite(max_high):
            max_high = float(seg.max(skipna=True))
        if not np.isfinite(min_low):
            min_low = float(seg.min(skipna=True))

        if bool(pd.notna(hi_seg).any()):
            max_idx = pd.to_datetime(hi_seg.astype(float).idxmax(skipna=True))
        else:
            max_idx = pd.to_datetime(seg.astype(float).idxmax(skipna=True))
        if bool(pd.notna(lo_seg).any()):
            min_idx = pd.to_datetime(lo_seg.astype(float).idxmin(skipna=True))
        else:
            min_idx = pd.to_datetime(seg.astype(float).idxmin(skipna=True))
        pos_max = int(seg.index.get_indexer([max_idx])[0])
        pos_min = int(seg.index.get_indexer([min_idx])[0])
        if pos_max < 0:
            pos_max = 0
        if pos_min < 0:
            pos_min = 0
        holding_days = int(len(seg))
        entry_to_max_days = int(pos_max + 1)
        entry_to_min_days = int(pos_min + 1)
        max_to_exit_days = int(holding_days - pos_max)
        min_to_exit_days = int(holding_days - pos_min)
        final_price_ret = float(exit_px / entry_px - 1.0)
        max_possible_ret_raw = float(max_high / entry_px - 1.0)
        min_possible_ret = float(min_low / entry_px - 1.0)
        net_ret_raw = (tr or {}).get("return")
        net_ret = (
            float(net_ret_raw)
            if net_ret_raw is not None and np.isfinite(float(net_ret_raw))
            else float(final_price_ret)
        )
        sign_eps = 1e-12
        trade_sign = (
            1.0 if net_ret > sign_eps else (-1.0 if net_ret < -sign_eps else 0.0)
        )
        max_possible_ret = float(abs(max_possible_ret_raw) * trade_sign)
        out.append(
            {
                "code": code_key,
                ...
                "holding_days": holding_days,
                "entry_to_max_days": int(entry_to_max_days),
                "entry_to_min_days": int(entry_to_min_days),
                "max_to_exit_days": int(max_to_exit_days),
                "min_to_exit_days": int(min_to_exit_days),
                "final_price_return": float(final_price_ret),
                "max_possible_return": float(max_possible_ret),
                "min_possible_return": float(min_possible_ret),
...
def _trade_extreme_stats_from_records(
    ...
    *,
    ...
    hold_all: list[float] = []
    hold_profit: list[float] = []
    hold_loss: list[float] = []
    e2max_all: list[float] = []
    e2max_profit: list[float] = []
    e2max_loss: list[float] = []
    e2min_all: list[float] = []
    e2min_profit: list[float] = []
    e2min_loss: list[float] = []
    max2exit_all: list[float] = []
    max2exit_profit: list[float] = []
    max2exit_loss: list[float] = []
    min2exit_all: list[float] = []
    min2exit_profit: list[float] = []
    min2exit_loss: list[float] = []
    for rec in list(records or []):
        max_v = rec.get("max_possible_return")
        min_v = rec.get("min_possible_return")
        final_v = rec.get("final_price_return")
        hold_v = rec.get("holding_days")
        e2max_v = rec.get("entry_to_max_days")
        e2min_v = rec.get("entry_to_min_days")
        max2exit_v = rec.get("max_to_exit_days")
        min2exit_v = rec.get("min_to_exit_days")
        net_v = rec.get("net_return")
        max_f = (
            float(max_v) if max_v is not None and np.isfinite(float(max_v)) else np.nan
        )
        min_f = (
            float(min_v) if min_v is not None and np.isfinite(float(min_v)) else np.nan
        )
        final_f = (
            float(final_v)
            if final_v is not None and np.isfinite(float(final_v))
            else np.nan
        )
        hold_f = (
            float(hold_v)
            if hold_v is not None and np.isfinite(float(hold_v))
            else np.nan
        )
        e2max_f = (
            float(e2max_v)
            if e2max_v is not None and np.isfinite(float(e2max_v))
            else np.nan
        )
        e2min_f = (
            float(e2min_v)
            if e2min_v is not None and np.isfinite(float(e2min_v))
            else np.nan
        )
        max2exit_f = (
            float(max2exit_v)
            if max2exit_v is not None and np.isfinite(float(max2exit_v))
            else np.nan
        )
        min2exit_f = (
            float(min2exit_v)
            if min2exit_v is not None and np.isfinite(float(min2exit_v))
            else np.nan
        )
        net_f = (
            float(net_v) if net_v is not None and np.isfinite(float(net_v)) else np.nan
        )
        if np.isfinite(max_f):
            max_all.append(float(max_f))
        if np.isfinite(min_f):
            min_all.append(float(min_f))
        if np.isfinite(final_f):
            final_all.append(float(final_f))
        if np.isfinite(hold_f):
            hold_all.append(float(hold_f))
        if np.isfinite(e2max_f):
            e2max_all.append(float(e2max_f))
        if np.isfinite(e2min_f):
            e2min_all.append(float(e2min_f))
        if np.isfinite(max2exit_f):
            max2exit_all.append(float(max2exit_f))
        if np.isfinite(min2exit_f):
            min2exit_all.append(float(min2exit_f))
        if np.isfinite(net_f):
            if net_f > eps:
                if np.isfinite(max_f):
                    max_profit.append(float(max_f))
                if np.isfinite(min_f):
                    min_profit.append(float(min_f))
                if np.isfinite(hold_f):
                    hold_profit.append(float(hold_f))
                if np.isfinite(e2max_f):
                    e2max_profit.append(float(e2max_f))
                if np.isfinite(e2min_f):
                    e2min_profit.append(float(e2min_f))
                if np.isfinite(max2exit_f):
                    max2exit_profit.append(float(max2exit_f))
                if np.isfinite(min2exit_f):
                    min2exit_profit.append(float(min2exit_f))
            elif net_f < -eps:
                if np.isfinite(max_f):
                    max_loss.append(float(max_f))
                if np.isfinite(min_f):
                    min_loss.append(float(min_f))
                if np.isfinite(hold_f):
                    hold_loss.append(float(hold_f))
                if np.isfinite(e2max_f):
                    e2max_loss.append(float(e2max_f))
                if np.isfinite(e2min_f):
                    e2min_loss.append(float(e2min_f))
                if np.isfinite(max2exit_f):
                    max2exit_loss.append(float(max2exit_f))
                if np.isfinite(min2exit_f):
                    min2exit_loss.append(float(min2exit_f))
    return {
        "final_price_return_stats": _dist_stats(final_all),
        "max_possible_return_stats": _dist_stats(max_all),
        "max_possible_return_profit_stats": _dist_stats(max_profit),
        "max_possible_return_loss_stats": _dist_stats(max_loss),
        "min_possible_return_stats": _dist_stats(min_all),
        "min_possible_return_profit_stats": _dist_stats(min_profit),
        "min_possible_return_loss_stats": _dist_stats(min_loss),
        "holding_days_stats": {
            "overall": _dist_stats(hold_all),
            "profit_trades": _dist_stats(hold_profit),
            "loss_trades": _dist_stats(hold_loss),
        },
        "entry_to_max_days_stats": {
            "overall": _dist_stats(e2max_all),
            "profit_trades": _dist_stats(e2max_profit),
            "loss_trades": _dist_stats(e2max_loss),
        },
        "entry_to_min_days_stats": {
            "overall": _dist_stats(e2min_all),
            "profit_trades": _dist_stats(e2min_profit),
            "loss_trades": _dist_stats(e2min_loss),
        },
        "max_to_exit_days_stats": {
            "overall": _dist_stats(max2exit_all),
            "profit_trades": _dist_stats(max2exit_profit),
            "loss_trades": _dist_stats(max2exit_loss),
        },
        "min_to_exit_days_stats": {
            "overall": _dist_stats(min2exit_all),
            "profit_trades": _dist_stats(min2exit_profit),
            "loss_trades": _dist_stats(min2exit_loss),
        },
    }


def _build_trade_extreme_stats_from_trades(
    trades: list[dict[str, Any]],
    *,
    exec_price: pd.Series | pd.DataFrame,
    high_price: pd.Series | pd.DataFrame,
    ...
    records = _trade_extreme_records_from_trades(
        trades,
    ...
    by_code_records: dict[str, list[dict[str, Any]]] = {}
    for c in list(expected_codes or []):
        ck = str(c)
        if ck:
            by_code_records.setdefault(ck, [])
    for rec in records:
        ck = str((rec or {}).get("code") or "").strip()
        if not ck:
            continue
        by_code_records.setdefault(ck, []).append(rec)
    return {
        "overall": _trade_extreme_stats_from_records(records, flat_eps=flat_eps),
        "by_code": {
            str(c): _trade_extreme_stats_from_records(rows, flat_eps=flat_eps)
            for c, rows in by_code_records.items()
        },
    }
    ...
    for i in range(len(bp)):
        ...
        if pos_side == 0:
            if b > 0.0:
                ...
                entry_atr = a
                entry_i = i
                entry_base_abs = max(abs(float(b)), 1e-12)
                pos_shares = (
                    float(abs(float(b)) / float(c))
                    if np.isfinite(c) and c > 0.0
                    else 0.0
                )
                pos_cost_amount = float(abs(float(b)))
        ...
        if pos_side == 1:
            ...
            # Dynamic average-cost tracking for current holding:
            # - add position: weighted-average cost moves toward today's close
            # - reduce position: average cost stays unchanged (average-cost method)
            delta_abs = float(abs(float(b)) - abs(float(prev_base)))
            if np.isfinite(c) and c > 0.0:
                if delta_abs > 1e-12:
                    add_shares = float(delta_abs / c)
                    if add_shares > 0.0:
                        pos_shares = float(pos_shares + add_shares)
                        pos_cost_amount = float(pos_cost_amount + delta_abs)
                elif delta_abs < -1e-12 and pos_shares > 1e-12:
                    sell_notional = float(min(-delta_abs, max(0.0, pos_cost_amount)))
                    sell_shares = float(min(pos_shares, sell_notional / c))
                    if sell_shares > 0.0:
                        avg_cost_prev = (
                            float(pos_cost_amount / pos_shares)
                            if pos_shares > 1e-12
                            else float("nan")
                        )
                        pos_shares = float(max(0.0, pos_shares - sell_shares))
                        if np.isfinite(avg_cost_prev) and avg_cost_prev > 0.0:
                            pos_cost_amount = float(
                                max(0.0, pos_cost_amount - avg_cost_prev * sell_shares)
                            )
                if pos_shares > 1e-12:
                    entry_px = float(pos_cost_amount / pos_shares)

            stop_before = float(stop_px) if np.isfinite(stop_px) else None
            stop_armed = bool(same_day_stop) or (
                entry_i >= 0 and (i - int(entry_i)) >= 2
            )
            stop_triggered = False
            if execution_v == "next_day":
                stop_triggered = bool(
            ...
            if stop_triggered:
                ...
                out[i] = (
                    float(b)
                    if (execution_v == "next_day" and exec_ds is not None)
                    else 0.0
                )
            ...
            if mode_v == "static" and np.isfinite(initial_stop_price):
                stop_candidate = float(initial_stop_price)
                stop_px = float(initial_stop_price)
            elif (
                np.isfinite(entry_px) and entry_px > 0.0 and np.isfinite(a) and a > 0.0
            ):
                atr_ref = (
                    float(entry_atr)
                    if (
                ...
                if mode_v == "tightening" and np.isfinite(c):
                ...
                base_risk_pct = float(
                    max(0.0, dist_mult) * atr_ref / max(entry_px, 1e-12)
                )
                risk_budget_scale = float(
                    max(0.0, abs(float(b)) / max(float(entry_base_abs), 1e-12))
                )
                eff_risk_pct = float(base_risk_pct * max(risk_budget_scale, 1e-12))
                stop_candidate_v = float(entry_px) * (1.0 - float(eff_risk_pct))
                stop_candidate = (
                    float(stop_candidate_v) if np.isfinite(stop_candidate_v) else None
                )
                if np.isfinite(stop_candidate_v):
                    if mode_v == "trailing" or mode_v == "tightening":
                        if (not np.isfinite(stop_px)) or (stop_candidate_v > stop_px):
        ...
        stop_before = float(stop_px) if np.isfinite(stop_px) else None
        stop_armed = bool(same_day_stop) or (entry_i >= 0 and (i - int(entry_i)) >= 2)
        stop_triggered = False
        if execution_v == "next_day":
            stop_triggered = bool(
                stop_armed
        ...
        if mode_v == "static" and np.isfinite(initial_stop_price):
            stop_candidate = float(initial_stop_price)
            stop_px = float(initial_stop_price)
        elif np.isfinite(entry_px) and entry_px > 0.0 and np.isfinite(a) and a > 0.0:
            atr_ref = (
                float(entry_atr)
                if (
                    atr_basis_v == "entry"
            ...
            base_risk_pct = float(max(0.0, dist_mult) * atr_ref / max(entry_px, 1e-12))
            risk_budget_scale = float(
                max(0.0, abs(float(b)) / max(float(entry_base_abs), 1e-12))
            )
            eff_risk_pct = float(base_risk_pct * max(risk_budget_scale, 1e-12))
    ...
    *,
    ...
    if mode == "next_day":
        ...
        if (not np.isfinite(exec_fill)) or exec_fill <= 0.0:
            return 0.0
        fill_for_ret = float(exec_fill)
    else:
        if (not np.isfinite(fill_px)) or fill_px <= 0.0:
            return 0.0
        fill_for_ret = float(fill_px)
    ...
    if (not np.isfinite(base)) or base <= 0.0:
        return 0.0
    return float(fill_for_ret / base - 1.0)
...
def _apply_intraday_stop_execution_single(
    *,
    ...
    for e in events:
        ev_mode = (
            str(e.get("execution_mode") or stop_execution_mode or "intraday")
            .strip()
            .lower()
        )
        d_raw = e.get("execution_date") or e.get("date")
        try:
        ...
        cl = (
            float(close_sig.loc[d])
            if (d in close_sig.index and np.isfinite(float(close_sig.loc[d])))
            else float("nan")
        )
        pc = (
    ...
    *,
    ...
    for c in w_adj.columns:
        ...
        for e in events:
            ...
            cl = (
                float(close_sig_df.loc[d, c])
                if (
                    d in close_sig_df.index
                    and c in close_sig_df.columns
                    and np.isfinite(float(close_sig_df.loc[d, c]))
                )
                else float("nan")
            )
            pc = (
...
def _apply_intraday_scaleout_execution_single(
    *,
    weights: pd.Series,
    scale_multiplier: pd.Series,
    ...
    for e in events:
        ...
        try:
            ...
            continue
        if d not in w0.index:
            continue
        sold_frac = e.get("reduce_fraction")
        try:
            sold_frac_f = float(sold_frac)
        except (TypeError, ValueError):
            sold_frac_f = float("nan")
        if (not np.isfinite(sold_frac_f)) or sold_frac_f <= 0.0:
            continue
        ...
        ev_mode = (
            str(e.get("execution_mode") or execution_mode or "intraday").strip().lower()
        )
        day_ret = _stop_fill_return(
            exec_price=exec_price,
            execution_mode=ev_mode,
...
def _apply_intraday_scaleout_execution_portfolio(
    *,
    weights: pd.DataFrame,
    ...
    prev_close_df = close_sig_df.astype(float).shift(1)
    for c in w0.columns:
        stats = scaleout_by_asset.get(str(c)) or {}
        events = list(stats.get("trigger_events") or [])
        if not events:
            continue
        for e in events:
            ...
            ev_mode = (
                str(e.get("execution_mode") or execution_mode or "intraday")
                .strip()
                .lower()
            )
            day_ret = _stop_fill_return(
...
def _advance_overlay_market_dates(
    stats: dict[str, Any],
    index: pd.Index,
) -> dict[str, Any]:
    """Map close-execution overlay observations to their actual T+1 dates."""
    out = dict(stats or {})
    ordered = list(pd.Index(index))
    next_date = {
        pd.Timestamp(ordered[i]): pd.Timestamp(ordered[i + 1])
        for i in range(max(0, len(ordered) - 1))
    }

    def _advance(raw: Any) -> str | None:
        try:
            current = pd.Timestamp(raw)
        except (TypeError, ValueError):
            return None
        advanced = next_date.get(current)
        if advanced is None:
            return None
        return advanced.date().isoformat()

    events_out: list[dict[str, Any]] = []
    for event in list(out.get("trigger_events") or []):
        row = dict(event)
        for key in (
            "date",
            "trigger_date",
            ...
            if row.get(key) is not None:
                row[f"{key}_decision_coordinate"] = row.get(key)
                advanced = _advance(row.get(key))
                if advanced is not None:
                    row[key] = advanced
        events_out.append(row)
    out["trigger_events"] = events_out

    dates_out = [
        advanced
        for raw in list(out.get("trigger_dates") or [])
        if (advanced := _advance(raw)) is not None
    ]
    out["trigger_dates"] = dates_out
    out["first_trigger_date"] = dates_out[0] if dates_out else None
    out["last_trigger_date"] = dates_out[-1] if dates_out else None

    trace_out: list[dict[str, Any]] = []
    for trace in list(out.get("trace_last_rows") or []):
        row = dict(trace)
        raw_date = row.get("date")
        advanced = _advance(raw_date)
        if advanced is not None:
            row["decision_coordinate_date"] = raw_date
            row["date"] = advanced
        trace_out.append(row)
    out["trace_last_rows"] = trace_out
    return out


def _apply_intraday_or_arbitration_single(
    *,
    weights: pd.Series,
    ...
    w0 = weights.astype(float).copy()
    w_adj = w0.copy()
    overlays = [str(k) for k in (event_sets or {}).keys()]
    override_by_overlay = {
        name: pd.Series(0.0, index=w0.index, dtype=float) for name in overlays
    }
    prev_close = close_sig.astype(float).shift(1)
    candidates_by_day: dict[pd.Timestamp, dict[str, list[dict[str, Any]]]] = {}
    eps = 1e-12

    for overlay in overlays:
        stats = event_sets.get(overlay) or {}
        events = list((stats or {}).get("trigger_events") or [])
        default_mode = (
            str((stats or {}).get("execution_mode") or "intraday").strip().lower()
        )
        if default_mode not in STOP_EXECUTION_MODES:
        ...
        for e in events:
            ...
            if d not in w0.index:
                continue
            req_raw = e.get("requested_reduce_fraction", e.get("reduce_fraction"))
            if req_raw is None:
                req_raw = 1.0
            try:
                req = float(req_raw)
            ...
            if (not np.isfinite(req)) or req <= 0.0:
                req = 1.0
            req = float(min(1.0, max(0.0, req)))
            ev_mode = (
                str(e.get("execution_mode") or default_mode or "intraday")
                .strip()
                .lower()
            )
            if ev_mode not in STOP_EXECUTION_MODES:
                ev_mode = default_mode
            is_full_exit = bool(req >= 1.0 - eps) or (
                str(e.get("trigger_kind") or "").strip().lower() == "exit"
            )
            day_bucket = candidates_by_day.setdefault(d, {})
            day_bucket.setdefault(str(overlay), []).append(
                {
                    "overlay": overlay,
                    "event": e,
                    "requested": float(req),
                    "execution_mode": str(ev_mode),
                    "is_full_exit": bool(is_full_exit),
                }
            )

    for d, by_overlay_rows in candidates_by_day.items():
        if not by_overlay_rows:
            continue

        def _row_stop_price(row: dict[str, Any]) -> float:
            ev = row.get("event") or {}
            for key in ("trigger_price_eff", "trigger_price", "fill_price"):
                try:
                    px = float(ev.get(key))
                except (TypeError, ValueError):
                    px = float("nan")
        ...
        def _cand_full_exit_stop_price(cand: dict[str, Any]) -> float:
            rows = [
                r for r in list(cand.get("rows") or []) if bool(r.get("is_full_exit"))
            ]
            if not rows:
                return float("-inf")
            return float(max(_row_stop_price(r) for r in rows))

        overlay_candidates: list[dict[str, Any]] = []
        for overlay, rows in by_overlay_rows.items():
            if not rows:
                continue
            any_full = any(bool(row.get("is_full_exit")) for row in rows)
            if any_full:
                requested_overlay = 1.0
            else:
                requested_overlay = float(
                    min(1.0, sum(float(row.get("requested", 0.0)) for row in rows))
                )
            execution_mode_overlay = str(rows[0].get("execution_mode") or "intraday")
            overlay_candidates.append(
                {
                    "overlay": str(overlay),
                    "rows": rows,
                    "requested": float(max(0.0, requested_overlay)),
                    "execution_mode": execution_mode_overlay,
                    "is_full_exit": bool(any_full),
                }
            )
        ...
        if base_abs <= eps:
            for cand in overlay_candidates:
                for row in cand["rows"]:
                    e = row["event"]
                    e["requested_reduce_fraction"] = float(row["requested"])
                    e["reduce_fraction"] = 0.0
                    e["ignored_by_priority"] = True
                    e["ignored_reason"] = "no_position"
            continue

        full_cands = [
            row for row in overlay_candidates if bool(row.get("is_full_exit"))
        ]
        if full_cands:
            chosen = max(
                full_cands,
                key=lambda row: (
                    float(_cand_full_exit_stop_price(row)),
                    float(row.get("requested", 0.0)),
                ),
            )
            ignored_reason = "full_exit_wins"
        else:
            chosen = max(
                overlay_candidates, key=lambda row: float(row.get("requested", 0.0))
            )
            ignored_reason = "max_partial_wins"

        requested = float(chosen.get("requested", 0.0))
        sell_frac = float(min(1.0, max(0.0, requested)))
        sold_w = float(base_abs * sell_frac)
        sold_w = float(min(max(0.0, base_abs), max(0.0, sold_w)))
        executed_frac = float(sold_w / base_abs) if base_abs > eps else 0.0
        rem = float(max(0.0, base_abs - sold_w))
        w_adj.loc[d] = float(base_sign * rem)

        chosen_rows = list(chosen.get("rows") or [])
        chosen_row_for_fill: dict[str, Any] | None = None
        if chosen_rows:
            if bool(chosen.get("is_full_exit")):
                full_rows_local = [
                    row for row in chosen_rows if bool(row.get("is_full_exit"))
                ]
                chosen_row_for_fill = (
                    max(full_rows_local, key=_row_stop_price)
                    if full_rows_local
                    else chosen_rows[0]
                )
            else:
                chosen_row_for_fill = max(
                    chosen_rows, key=lambda row: float(row.get("requested", 0.0))
                )
        e_chosen = (
            (chosen_row_for_fill or chosen_rows[0]).get("event")
            if chosen_rows
            else {"fill_price": None}
        )
        fill_raw = e_chosen.get("fill_price")
        try:
            fill_px = float(fill_raw)
        except (TypeError, ValueError):
            fill_px = float("nan")
        ...
        overlay_key = str(chosen.get("overlay"))
        if overlay_key in override_by_overlay:
            override_by_overlay[overlay_key].loc[d] = float(
                override_by_overlay[overlay_key].loc[d] + sold_w * base_sign * day_ret
            )

        for cand in overlay_candidates:
            rows = list(cand.get("rows") or [])
            if cand is not chosen:
                for row in rows:
                    ...
                    e["reduce_fraction"] = 0.0
                    e["ignored_by_priority"] = True
                    e["ignored_reason"] = str(ignored_reason)
                continue
            if bool(cand.get("is_full_exit")):
                full_rows_local = [row for row in rows if bool(row.get("is_full_exit"))]
                selected_row = (
                    max(full_rows_local, key=_row_stop_price)
                    if full_rows_local
                    else rows[0]
                ...
                for row in rows:
                    ...
                    if row is selected_row:
                        e["reduce_fraction"] = float(executed_frac)
                        e["ignored_by_priority"] = False
                        e.pop("ignored_reason", None)
                    else:
                        e["reduce_fraction"] = 0.0
                        e["ignored_by_priority"] = True
                        e["ignored_reason"] = "full_exit_wins"
                continue
            remaining_exec = float(executed_frac)
            for row in rows:
                e = row["event"]
                req_row = float(row.get("requested", 0.0))
                row_exec = float(min(max(0.0, req_row), max(0.0, remaining_exec)))
                remaining_exec = float(max(0.0, remaining_exec - row_exec))
                e["requested_reduce_fraction"] = float(req_row)
                e["reduce_fraction"] = float(row_exec)
                e["ignored_by_priority"] = bool(row_exec <= eps)
                if bool(row_exec <= eps):
                    e["ignored_reason"] = "max_partial_wins"
                else:
                    e.pop("ignored_reason", None)

    return w_adj.astype(float), override_by_overlay


def _apply_intraday_or_arbitration_portfolio(
    *,
    weights: pd.DataFrame,
    event_sets_by_asset: dict[str, dict[str, dict[str, Any]]],
    exec_price: str,
    open_sig_df: pd.DataFrame,
    close_sig_df: pd.DataFrame,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    w0 = weights.astype(float).copy()
    overlays = [str(k) for k in (event_sets_by_asset or {}).keys()]
    w_adj = w0.copy()
    override_by_overlay = {
        name: pd.Series(0.0, index=w0.index, dtype=float) for name in overlays
    }
    override_by_overlay_asset = {
        name: pd.DataFrame(0.0, index=w0.index, columns=w0.columns, dtype=float)
        for name in overlays
    }
    for c in w0.columns:
        events_for_code: dict[str, dict[str, Any]] = {}
        for overlay in overlays:
            by_asset = event_sets_by_asset.get(overlay) or {}
            events_for_code[overlay] = dict(by_asset.get(str(c)) or {})
        w_col, over_col = _apply_intraday_or_arbitration_single(
            weights=w0[c].astype(float),
            event_sets=events_for_code,
            exec_price=exec_price,
            open_sig=(
                open_sig_df[c].astype(float)
                if c in open_sig_df.columns
                else pd.Series(np.nan, index=w0.index, dtype=float)
            ),
            close_sig=(
                close_sig_df[c].astype(float)
                if c in close_sig_df.columns
                else pd.Series(np.nan, index=w0.index, dtype=float)
            ),
        )
        w_adj[c] = w_col.reindex(w0.index).astype(float)
        for overlay in overlays:
            if overlay in override_by_overlay and overlay in over_col:
                code_override = (
                    over_col[overlay].reindex(w0.index).fillna(0.0).astype(float)
                )
                override_by_overlay_asset[overlay][c] = code_override
                override_by_overlay[overlay] = (
                    override_by_overlay[overlay].astype(float) + code_override
                )
    return w_adj.astype(float), {
        **override_by_overlay,
        "_by_asset": override_by_overlay_asset,
    }
...
def _normalize_r_profit_scaleout_tiers(
    tiers: list[dict[str, float]] | None,
) -> list[dict[str, float]]:
    raw = tiers if tiers is not None else DEFAULT_R_PROFIT_SCALEOUT_TIERS
    out: list[dict[str, float]] = []
    for item in raw or []:
        if not isinstance(item, dict):
            continue
        ...
        if (not np.isfinite(reduce_frac)) or reduce_frac <= 0.0 or reduce_frac > 1.0:
            continue
        out.append({"r_multiple": float(r_mult), "reduce_fraction": float(reduce_frac)})
    if not out:
        out = [dict(x) for x in DEFAULT_R_PROFIT_SCALEOUT_TIERS]
    uniq: dict[float, float] = {}
    for row in out:
        uniq[float(row["r_multiple"])] = float(row["reduce_fraction"])
    rows = [
        {"r_multiple": float(k), "reduce_fraction": float(v)} for k, v in uniq.items()
    ]
    rows.sort(key=lambda x: float(x["r_multiple"]))
    return rows
...
def _build_r_profit_scaleout_plan(
    ...
    *,
    ...
    tiers_v = _normalize_r_profit_scaleout_tiers(tiers)
    execution_v = str(execution_mode or "intraday").strip().lower()
    if execution_v not in STOP_EXECUTION_MODES:
        execution_v = "intraday"
    execution_time_v = str(execution_time or "full_day").strip().lower()
    ...
    hi = high.astype(float).reindex(bp.index).fillna(cl)
    lo = low.astype(float).reindex(bp.index).fillna(cl)
    atr = _atr_from_hlc(hi, lo, cl, window=int(atr_window)).astype(float)
    mult = pd.Series(1.0, index=bp.index, dtype=float)
    if not bool(enabled):
    ...
    breakeven_trace_last_rows: list[dict[str, Any]] = []
    pending_arm_by_idx: set[int] = set()

    def _initial_r_pct(entry_price: float, entry_atr: float) -> float:
        if bool(atr_stop_enabled):
            if (
                ...
                return float(float(atr_n) * float(entry_atr) / float(entry_price))
            return float("nan")
        if (
            np.isfinite(entry_price)
            and entry_price > 0.0
    ...
    for i in range(len(bp)):
        ...
        if i in pending_arm_by_idx:
            pending_arm_by_idx.discard(i)
            if addon_enabled and in_pos and remaining_frac > eps:
                breakeven_armed = True
                breakeven_exit_pending = False
                breakeven_arm_dates.append(ds)
        ...
        if waiting_base_reset and (not base_entry_event):
            mult.iloc[i] = 0.0
            prev_base = b
            continue

        if (not in_pos) and b > 0.0:
            in_pos = True
            waiting_base_reset = False
            entry_px = c
        ...
        tier_triggered_today = False
        breakeven_triggered_today = False
        if (
            in_pos
            ...
            for t_idx, t in enumerate(tiers_v):
                ...
                if not triggered:
                    continue
                requested_reduce = float(min(1.0, max(0.0, reduce_raw)))
                reduce_eff = float(
                    min(max(0.0, remaining_for_tier), max(0.0, requested_reduce))
                )
                ...
                if execution_v == "next_day":
                    fill_px = float("nan")
                    gap_open = False
                    trigger_source = "close_above_r_scaleout_next_day_exec"
                    ...
                    fill_px = (
                        float(o) if gap_open and np.isfinite(o) else float(trigger_px)
                    )
                    trigger_source = (
                        "gap_open_above_r_scaleout"
                        if gap_open
                ...
                trade_records.append(
                    {
                        "entry_decision_date": (
                            str(bp.index[entry_i].date())
                            if entry_i >= 0 and hasattr(bp.index[entry_i], "date")
                            else None
                        ),
                        "trigger_date": ds,
                        "execution_mode": execution_v,
                ...
                tier_triggered_today = True

        if (
            addon_enabled
            and in_pos
            and breakeven_armed
            and (not breakeven_exit_pending)
            and np.isfinite(entry_px)
            and entry_px > eps
            ...
            if triggered:
                ...
                exec_idx = i + 1 if execution_v == "next_day" else i
                reduce_eff = float(max(0.0, min(1.0, remaining_frac)))
                if exec_idx < len(bp) and exec_ds is not None and reduce_eff > eps:
                    if execution_v == "next_day":
                        pending_reduce_by_idx[exec_idx] = float(
                            pending_reduce_by_idx.get(exec_idx, 0.0) + reduce_eff
                        )
                        ...
                        fill_px = (
                            float(o) if gap_open and np.isfinite(o) else float(stop_px)
                        )
                        trigger_source = (
                            "gap_open_below_r_scaleout_breakeven"
                            if gap_open
                            else "low_touch_r_scaleout_breakeven"
                        )
                        remaining_frac = 0.0
                        breakeven_exit_pending = False
                    breakeven_armed = False
                    ...
                    breakeven_trigger_events.append(
                        {
                            ...
                            "low_price": (
                                float(lo.iloc[i])
                                if np.isfinite(float(lo.iloc[i]))
                                else None
                            ),
                            "fill_price": (
                                float(fill_px) if np.isfinite(fill_px) else None
        ...
        remaining_display = float(max(0.0, min(1.0, remaining_frac)))
        if remaining_display <= eps:
        ...
        breakeven_trace_last_rows.append(
            {
                ...
                "exit_pending": bool(breakeven_exit_pending),
                "triggered": bool(breakeven_triggered_today),
                "event_reason": (
                    "r_profit_scaleout_breakeven_triggered"
                    if breakeven_triggered_today
                    else "r_profit_scaleout_breakeven_watch"
                ),
            }
        )
        if len(breakeven_trace_last_rows) > 120:
            breakeven_trace_last_rows = breakeven_trace_last_rows[-120:]
        trace_last_rows.append(
            {
                "date": ds,
                "event_type": "hold",
                "event_reason": (
                    "r_profit_scaleout_breakeven_triggered"
                    if breakeven_triggered_today
                    else "r_profit_scaleout_triggered"
                    if tier_triggered_today
                    else "r_profit_scaleout_watch"
                ),
                "base_pos": float(b),
                "entry_price": (float(entry_px) if np.isfinite(entry_px) else None),
    ...
    stats = {
        ...
        "trigger_rule": "close_only_retrace_with_non_decreasing_trigger_line",
        "trigger_rule_v2": (
            "intraday/full_day: high_ge_r_threshold"
            if execution_v == "intraday" and execution_time_v == "full_day"
            else "intraday/open: open_ge_r_threshold"
        ...
        "fill_rule": (
            "intraday: execute_at_close_after_close_trigger"
            if execution_v == "intraday"
            else f"next_day: execute_on_next_trading_day_at_{execution_time_v}"
        ),
        "fill_rule_v2": (
        ...
        "breakeven_stop": {
            ...
            "trigger_count": int(len(breakeven_trigger_events)),
            ...
            "price_rule": "entry_price_after_first_executed_scaleout",
            "target_protection_line_only": True,
            "trace_last_rows": breakeven_trace_last_rows[-80:],
        },
    }
    return mult.astype(float), stats


def _build_ma_trailing_stop_plan(
    base_pos: pd.Series,
    *,
    open_: pd.Series | None = None,
    close: pd.Series,
    ...
    if ma_type_v not in {"sma", "ema"}:
        ma_type_v = "sma"
    reduce_w = max(2, int(reduce_window))
    exit_w = max(2, int(exit_window))
    delay_days = max(1, int(effective_delay_days))
    activation_offset = max(2, int(delay_days) + 1)
    reduce_frac = float(min(1.0, max(0.0, float(reduce_fraction))))
    bp = base_pos.fillna(0.0).astype(float).clip(lower=0.0)
    cl = close.astype(float).reindex(bp.index)
    op = (
        (open_.astype(float) if isinstance(open_, pd.Series) else cl.astype(float))
    ...
    hi = high.astype(float).reindex(bp.index).fillna(cl)
    lo = low.astype(float).reindex(bp.index).fillna(cl)
    ma_reduce = _moving_average(cl, window=reduce_w, ma_type=ma_type_v).astype(float)
    ma_exit = _moving_average(cl, window=exit_w, ma_type=ma_type_v).astype(float)
    mult = pd.Series(1.0, index=bp.index, dtype=float)
    if (not bool(enabled)) or reduce_frac <= 0.0:
        return mult, {
            "enabled": False,
            "ma_type": ma_type_v,
            "execution_mode": execution_v,
            ...
            "reduce_fraction": float(reduce_frac),
            "reduce_fraction_basis": "initial_position",
            "trigger_count": 0,
            "trigger_dates": [],
            "trigger_events": [],
    ...
    for i in range(len(bp)):
        ...
        c = float(cl.iloc[i]) if np.isfinite(float(cl.iloc[i])) else float("nan")
        h = float(hi.iloc[i]) if np.isfinite(float(hi.iloc[i])) else float("nan")
        low_px = float(lo.iloc[i]) if np.isfinite(float(lo.iloc[i])) else float("nan")
        reduce_line = (
            float(ma_reduce.iloc[i])
            if np.isfinite(float(ma_reduce.iloc[i]))
            else float("nan")
        )
        exit_line = (
            float(ma_exit.iloc[i])
            if np.isfinite(float(ma_exit.iloc[i]))
            else float("nan")
        )
        base_entry_event = bool((b > 0.0) and (prev_base <= 0.0))
        ...
        trigger_line = float("nan")
        if (
            in_pos
            and bool(entry_i >= 0 and (i - int(entry_i)) >= activation_offset)
            and remaining_frac > eps
        ):
            exit_hit = False
            ...
            if exit_hit:
                trigger_kind = "exit"
                trigger_line = float(exit_line)
            elif reduce_hit and reduce_frac > 0.0:
                trigger_kind = "reduce"
                trigger_line = float(reduce_line)

        if trigger_kind is not None:
            requested_reduce = 1.0 if trigger_kind == "exit" else float(reduce_frac)
            reduce_eff = float(
                min(max(0.0, remaining_frac), max(0.0, float(requested_reduce)))
            )
            if reduce_eff > eps:
                exec_ds = (
                    _next_trading_day_iso(bp.index, i)
                ...
                exec_idx = i + 1 if execution_v == "next_day" else i
                if exec_ds is not None and exec_idx < len(bp):
                    if execution_v == "next_day":
                        pending_reduce_by_idx[exec_idx] = float(
                            pending_reduce_by_idx.get(exec_idx, 0.0) + reduce_eff
                        )
                        ...
                        gap_open_triggered = False
                        trigger_source = (
                            "close_below_ma_trailing_exit_line_next_day_exec"
                    ...
                    trigger_events.append(
                        {
                            ...
                            "trigger_price": (
                                float(trigger_line)
                                if np.isfinite(trigger_line)
                                else None
                            ),
                            "reduce_line": (
                                float(reduce_line) if np.isfinite(reduce_line) else None
                            ),
                            "exit_line": (
                                float(exit_line) if np.isfinite(exit_line) else None
                            ),
                            "open_price": (float(o) if np.isfinite(o) else None),
                            "close_price": (float(c) if np.isfinite(c) else None),
                            "high_price": (float(h) if np.isfinite(h) else None),
                            "low_price": (
                                float(low_px) if np.isfinite(low_px) else None
                            ),
                            "fill_price": (
                                float(fill_price) if np.isfinite(fill_price) else None
                            ),
        ...
        trace_last_rows.append(
            {
                ...
                "exit_line": (float(exit_line) if np.isfinite(exit_line) else None),
                "triggered": bool(trigger_kind is not None),
...
def _apply_r_multiple_take_profit(
    ...
    *,
    ...
    initial_r_pct = float("nan")
    entry_base_pos = float("nan")
    peak_close_profit_pct = float("nan")
    pos_shares = 0.0
    pos_cost_amount = 0.0
    tp_trigger_line_px = float("nan")
    entry_i = -1
    invalid_r_entries = 0
    eps = 1e-12
    ...
    for i in range(len(bp)):
        ...
        if not in_pos:
            ...
            wait_next_entry_lock = False
            entry_px = c
            pos_shares = float(b / c) if c > eps else 0.0
            pos_cost_amount = float(b)
            entry_i = i
            initial_r_pct = (
        ...
        # Dynamic average cost of current holding:
        # add position raises/lowers cost toward today's close;
        # reduce position keeps average cost unchanged (average-cost method).
        delta_pos = float(b - float(prev_base))
        if np.isfinite(c) and c > eps:
            if delta_pos > eps:
                add_shares = float(delta_pos / c)
                if add_shares > 0.0:
                    pos_shares = float(pos_shares + add_shares)
                    pos_cost_amount = float(pos_cost_amount + delta_pos)
            elif delta_pos < -eps and pos_shares > eps:
                sell_notional = float(min(-delta_pos, max(0.0, pos_cost_amount)))
                sell_shares = float(min(pos_shares, sell_notional / c))
                if sell_shares > 0.0:
                    avg_cost_prev = (
                        float(pos_cost_amount / pos_shares)
        ...
        peak_probe_profit_pct = (
            (peak_probe_price / entry_px - 1.0)
            if (
                np.isfinite(peak_probe_price)
                and np.isfinite(entry_px)
                and entry_px > eps
            )
            else float("nan")
        )
        if np.isfinite(peak_probe_profit_pct):
            if not np.isfinite(peak_close_profit_pct):
                peak_close_profit_pct = float(peak_probe_profit_pct)
            else:
                peak_close_profit_pct = max(
                    float(peak_close_profit_pct), float(peak_probe_profit_pct)
                )
        base_pos_scale = 1.0
        if np.isfinite(entry_base_pos) and entry_base_pos > eps:
            base_pos_scale = float(max(0.0, float(b) / float(entry_base_pos)))
        risk_base_pct = (
            float(atr_n) * float(a) / max(float(entry_px), eps)
            if (
                bool(atr_stop_enabled)
                and np.isfinite(a)
                and a > 0.0
                and np.isfinite(entry_px)
                and entry_px > eps
            )
            else float(initial_r_pct)
        )
        effective_r_pct = (
            float(risk_base_pct) * float(max(base_pos_scale, eps))
            if np.isfinite(risk_base_pct) and risk_base_pct > eps
            else float("nan")
        )
        peak_r_mult = (
            (float(peak_close_profit_pct) / float(effective_r_pct))
            if (
                np.isfinite(peak_close_profit_pct)
        ...
        if np.isfinite(peak_r_mult):
            for row in tiers_v:
                if peak_r_mult >= float(row["r_multiple"]):
                    active_tier = row
        dd_from_peak_close = (
            (float(peak_close_profit_pct) - float(cur_close_profit_pct))
            / float(peak_close_profit_pct)
            if (
                np.isfinite(peak_close_profit_pct)
                and peak_close_profit_pct > eps
                and np.isfinite(cur_close_profit_pct)
            )
            else float("nan")
        )
        tp_armed = bool(entry_i >= 0 and i > int(entry_i))
        dd_eval = dd_from_peak_close
        trigger_profit_pct = (
            float(peak_close_profit_pct) * (1.0 - float(active_tier["retrace_ratio"]))
            if (
                active_tier is not None
                and np.isfinite(peak_close_profit_pct)
                and np.isfinite(entry_px)
                and entry_px > eps
            )
            else float("nan")
        )
        trigger_px_candidate = (
            float(entry_px) * (1.0 + float(trigger_profit_pct))
            if np.isfinite(trigger_profit_pct)
            else float("nan")
        )
        if np.isfinite(trigger_px_candidate):
            if (not np.isfinite(tp_trigger_line_px)) or (
                trigger_px_candidate > tp_trigger_line_px
            ):
                tp_trigger_line_px = float(trigger_px_candidate)
        gap_open_triggered = bool(
            tp_armed
            and active_tier is not None
            and np.isfinite(o)
            and np.isfinite(tp_trigger_line_px)
            and o <= float(tp_trigger_line_px)
        )
        if execution_v == "intraday" and execution_time_v == "full_day":
            tp_triggered = bool(
                gap_open_triggered
                or (
                    ...
                    and np.isfinite(l)
                    and np.isfinite(tp_trigger_line_px)
                    and l <= float(tp_trigger_line_px)
                )
            )
        elif execution_v == "intraday" and execution_time_v == "open":
            tp_triggered = gap_open_triggered
        else:
            tp_triggered = bool(
                tp_armed
                and active_tier is not None
                and np.isfinite(c)
                and np.isfinite(tp_trigger_line_px)
                and c <= float(tp_trigger_line_px)
            )
        if tp_triggered:
            in_pos = False
            trigger_px = float(tp_trigger_line_px)
            exec_ds = (
                _next_trading_day_iso(bp.index, i) if execution_v == "next_day" else ds
            )
            out[i] = (
            ...
            elif execution_time_v == "full_day":
                fill_price = (
                    float(o) if gap_open_triggered else float(tp_trigger_line_px)
                )
                trigger_source = (
                    "gap_open_below_tp"
                    if gap_open_triggered
            ...
            trace_last_rows.append(
                {
                    ...
                    "peak_profit_pct": (
                        float(peak_close_profit_pct)
                        if np.isfinite(peak_close_profit_pct)
                        else None
                    ),
                    "peak_r_multiple": (
                        float(peak_r_mult) if np.isfinite(peak_r_mult) else None
                    ...
                    "tp_trigger_line_price": (
                        float(tp_trigger_line_px)
                        if np.isfinite(tp_trigger_line_px)
                        else None
                    ),
    ...
    stats = {
        ...
        "r_reference_mode": "close_only_dynamic_avg_cost_dynamic_risk_budget",
        "r_position_scale_mode": "dynamic_base_position_scaled",
        "trigger_count": trigger_count,
        "trigger_dates": trigger_dates[:200],
        "trigger_events": trigger_events[:200],
        "first_trigger_date": (trigger_dates[0] if trigger_dates else None),
        ...
        "trigger_rule": "close_only_retrace_with_non_decreasing_trigger_line",
        "trigger_rule_v2": (
            "full_day_high_peak_then_gap_open_or_low_retrace"
            if execution_v == "intraday" and execution_time_v == "full_day"
            else "open_peak_and_open_retrace"
            if execution_v == "intraday" and execution_time_v == "open"
            else "close_retrace_with_non_decreasing_trigger_line"
        ),
        "fill_rule": (
            "intraday: execute_at_close_after_close_trigger"
            if execution_v == "intraday"
...
def _apply_bias_v_take_profit(
    ...
    *,
    ...
    breakeven_armed = False
    breakeven_exit_pending = False
    breakeven_entry_price = float("nan")
    breakeven_arm_dates: list[str] = []
    breakeven_trigger_dates: list[str] = []
    breakeven_trigger_events: list[dict[str, Any]] = []
    breakeven_trace_last_rows: list[dict[str, Any]] = []
    ...
    for i in range(len(bp)):
        ...
        if not in_pos:
            ...
            breakeven_entry_price = (
                float(c) if np.isfinite(c) and c > 0.0 else float("nan")
            )
            breakeven_armed = False
            breakeven_exit_pending = False
            first_t = tiers_v[0]
        ...
        if (
            ...
            and (not breakeven_exit_pending)
            and remaining_frac > eps
            and np.isfinite(breakeven_entry_price)
            and breakeven_entry_price > eps
        ):
                    ...
                    trade_records.append(
                        {
                            ...
                            "trigger_take_profit_price": float(stop_px),
                            "execution_take_profit_price": (
                                float(fill_price) if np.isfinite(fill_price) else None
                            ),
                            "trigger_date": ds,
        ...
        breakeven_trace_last_rows.append(
            {
                "date": ds,
                "entry_price": (
                    float(breakeven_entry_price)
                    if np.isfinite(breakeven_entry_price)
                    else None
                ),
                "base_pos": float(b),
                ...
                "triggered": bool(breakeven_triggered_today),
                "event_reason": (
                    "bias_v_take_profit_breakeven_triggered"
                    if breakeven_triggered_today
                    else "bias_v_take_profit_breakeven_watch"
                ),
            }
        )
        if len(breakeven_trace_last_rows) > 120:
        ...
        trace_last_rows.append(
            {
                ...
                "tp_triggered": bool(
                    len(tier_triggered_today) > 0 or breakeven_triggered_today
                ),
                "tp_fill_price": None,
                "tp_trigger_source": (
                    "bias_v_take_profit_breakeven"
                    if breakeven_triggered_today
                    else None
                ),
                "gap_open_triggered": None,
...
def compute_trend_backtest(db: Session, inp: TrendInputs) -> dict[str, Any]:
    ...
    if ma_trailing_stop_ma_type not in {"sma", "ema"}:
        raise ValueError("ma_trailing_stop_ma_type must be one of: sma|ema")
    ma_trailing_stop_execution_mode = (
        str(getattr(inp, "ma_trailing_stop_execution_mode", "intraday") or "intraday")
        .strip()
        .lower()
    )
    if ma_trailing_stop_execution_mode not in STOP_EXECUTION_MODES:
        raise ValueError(
    ...
    if ma_trailing_stop_effective_delay_days < 1:
        raise ValueError("ma_trailing_stop_effective_delay_days must be >= 1")
    if ma_trailing_stop_reduce_window < 2:
        raise ValueError("ma_trailing_stop_reduce_window must be >= 2")
    ma_trailing_stop_exit_window = int(
        getattr(inp, "ma_trailing_stop_exit_window", 20) or 20
    )
    if ma_trailing_stop_exit_window < 2:
        raise ValueError("ma_trailing_stop_exit_window must be >= 2")
    ma_trailing_stop_reduce_fraction = float(
        getattr(inp, "ma_trailing_stop_reduce_fraction", 0.33) or 0.33
    )
    if (
        (not np.isfinite(ma_trailing_stop_reduce_fraction))
        or ma_trailing_stop_reduce_fraction <= 0.0
        or ma_trailing_stop_reduce_fraction > 1.0
    ):
        raise ValueError("ma_trailing_stop_reduce_fraction must be in (0, 1]")
    ...
    if rtp_execution_mode not in STOP_EXECUTION_MODES:
        raise ValueError(
    ...
    if bias_v_tp_execution_mode not in STOP_EXECUTION_MODES:
        raise ValueError(
    ...
    risk_control_compatibility_meta = _enforce_risk_control_compatibility(
        {
            "atr_stop": bool(atr_mode != "none"),
            "r_take_profit": bool(rtp_enabled),
            "r_profit_scaleout": bool(rps_enabled),
            "bias_v_take_profit": bool(bias_v_tp_enabled),
            "ma_trailing_stop": bool(ma_trailing_stop_enabled),
    ...
    if (
        not np.isfinite(vol_ratio_extreme_threshold)
    ) or vol_ratio_extreme_threshold <= 0:
        raise ValueError("vol_ratio_extreme_threshold must be > 0")
    ...
    if vol_ratio_extreme_threshold <= vol_ratio_expand_threshold:
        raise ValueError(
            "vol_ratio_extreme_threshold must be > vol_ratio_expand_threshold"
        )
    if (
    ...
    h_none = _pick_series(ohlc_none, "high", c_none.combine_first(px_exec_none))
    l_none = _pick_series(ohlc_none, "low", c_none.combine_first(px_exec_none))
    o_hfq = _pick_series(ohlc_hfq, "open", px_bh)
    c_hfq = _pick_series(ohlc_hfq, "close", px_bh)
    h_hfq = _pick_series(ohlc_hfq, "high", c_hfq.combine_first(px_bh))
    l_hfq = _pick_series(ohlc_hfq, "low", c_hfq.combine_first(px_bh))
    o_qfq = _pick_series(ohlc_qfq, "open", px_sig)
    if ep == "open":
        # Open execution: execution-day intraday return (open[t] -> close[t]).
        ...
        # Close execution: execution-day close-to-close return (close[t-1] -> close[t]).
        exec_none = c_none.combine_first(px_exec_none)
        exec_hfq = c_hfq.combine_first(px_bh)
        ret_none = (
            (exec_none.div(exec_none.shift(1)) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
        ...
        # OC2: arithmetic average of open and close execution daily returns.
        exec_o_none = o_none.combine_first(px_exec_none)
        exec_c_none = c_none.combine_first(px_exec_none)
        exec_o_hfq = o_hfq.combine_first(px_bh)
        exec_c_hfq = c_hfq.combine_first(px_bh)
        ...
        ret_close_none = (
            (exec_c_none.div(exec_c_none.shift(1)) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
        ...
        ret_close_hfq = (
            (exec_c_hfq.div(exec_c_hfq.shift(1)) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
    ...
    high_exec_trade = (
        h_none.where(~ca_mask.fillna(False), other=h_hfq)
        .replace([np.inf, -np.inf], np.nan)
        .ffill()
    ...
    low_exec_trade = (
        l_none.where(~ca_mask.fillna(False), other=l_hfq)
        .replace([np.inf, -np.inf], np.nan)
        .ffill()
        .combine_first(px_exec_slip)
        .astype(float)
    ...
    if ma_entry_filter_enabled:
        raw_pos, ma_entry_filter_stats_overall = _apply_ma_cross_entry_filter(
    ...
    risk_open = o_qfq.astype(float)
    risk_close = px_sig.astype(float)
    risk_high = high_qfq.astype(float)
    risk_low = low_qfq.astype(float)
    if ep == "close":
        # Risk state is maintained in decision-date coordinates. Shift market
        # observations back one row so each decision maps to its actual T+1
        # close execution date before entry price/ATR/initial-R are frozen.
    ...
    _, r_profit_scaleout_stats = _build_r_profit_scaleout_plan(
        raw_pos.fillna(0.0).astype(float),
        open_=risk_open,
        close=risk_close,
        high=risk_high.fillna(risk_close),
        low=risk_low.fillna(risk_close),
    ...
    _, ma_trailing_stop_stats = _build_ma_trailing_stop_plan(
        raw_pos.fillna(0.0).astype(float),
        open_=risk_open,
        close=risk_close,
        high=risk_high.fillna(risk_close),
    ...
    if ep == "close":
        atr_stop_stats = _advance_overlay_market_dates(atr_stop_stats, raw_pos.index)
        bias_v_take_profit_stats = _advance_overlay_market_dates(
            bias_v_take_profit_stats, raw_pos.index
        )
        r_take_profit_stats = _advance_overlay_market_dates(
            r_take_profit_stats, raw_pos.index
        )
        r_profit_scaleout_stats = _advance_overlay_market_dates(
            r_profit_scaleout_stats, raw_pos.index
        )
        ma_trailing_stop_stats = _advance_overlay_market_dates(
            ma_trailing_stop_stats, raw_pos.index
        )
    ...
    vol_periodic_rebalance_stats_overall: dict[str, int] = {
        "periodic_rebalance_evaluated_count": 0,
        "periodic_rebalance_trigger_count": 0,
        "periodic_rebalance_skip_count": 0,
    }
    ...
    if ps == "fixed_ratio":
        ...
        periodic_enabled = bool(vol_periodic_risk_mgmt_enabled)
        ...
        if periodic_enabled:
            sizing_scale, vol_periodic_rebalance_stats_overall = (
                _risk_budget_periodic_weights(
                    ...
                    rebalance_threshold_pct=float(
                        ...
                        else vol_periodic_rebalance_threshold_pct
                    ),
                )
            )
        else:
    ...
    if bool(cvar_risk_mgmt_enabled):
        cvar_ret_asset = (
            ret_hfq.reindex(raw_pos.index)
            .astype(float)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
        )
        cvar_scaled, cvar_position_scale_stats = _apply_cvar_budget_scaling(
            raw_pos.to_frame(code),
            asset_returns=cvar_ret_asset.to_frame(code),
            enabled=True,
            window=int(cvar_window),
            budget_pct=float(cvar_budget_pct),
        )
        raw_pos = cvar_scaled[code].astype(float)
    atr_gate = (
        _atr_from_hlc(
            high_qfq.astype(float).fillna(px_sig),
            low_qfq.astype(float).fillna(px_sig),
    ...
    # Post-trade weight on each day (open mode: after open trade, close mode: after close trade).
    w_post = raw_pos.shift(1).fillna(0.0).astype(float).clip(lower=0.0, upper=1.0)
    open_sig_for_stop = o_qfq.reindex(w_post.index).astype(float).ffill()
    close_sig_for_stop = px_sig.reindex(w_post.index).astype(float).ffill()

    def _apply_all_intraday_overlays(
        weight_in: pd.Series,
        *,
        mode: str,
        scale_multiplier_mode: str = "absolute",
        ...
        w_tmp, over_by_overlay = _apply_intraday_or_arbitration_single(
            ...
            event_sets={
                ...
                "ma_trailing_stop": ma_trailing_stop_stats,
            },
            exec_price=str(mode),
        ...
        bias_over = over_by_overlay.get(
            "bias_v_take_profit", pd.Series(0.0, index=w_tmp.index, dtype=float)
        )
        rtp_over = over_by_overlay.get(
            "r_take_profit", pd.Series(0.0, index=w_tmp.index, dtype=float)
        )
        rps_over = over_by_overlay.get(
            "r_profit_scaleout", pd.Series(0.0, index=w_tmp.index, dtype=float)
        )
    ...
    cm = ca_mask.reindex(ret_exec.index).fillna(False).astype(bool)
    ret_exec_open_none_day = (
        (c_none / o_none - 1.0)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_open_hfq_day = (
        (c_hfq / o_hfq - 1.0)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_open_day = ret_exec_open_none_day.where(~cm, ret_exec_open_hfq_day).astype(
        float
    )
    ret_exec_close_none_day = (
        (c_none / c_none.shift(1) - 1.0)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_close_hfq_day = (
        (c_hfq / c_hfq.shift(1) - 1.0)
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_close_day = ret_exec_close_none_day.where(
        ~cm, ret_exec_close_hfq_day
    ).astype(float)

    w_ret = w.copy()
    ret_exec_day = ret_exec_open_day.copy().astype(float)
    atr_stop_override_ret = atr_over_post.reindex(w.index).fillna(0.0).astype(float)
    bias_v_take_profit_override_ret = (
        bias_over_post.reindex(w.index).fillna(0.0).astype(float)
    )
    r_take_profit_override_ret = (
        rtp_over_post.reindex(w.index).fillna(0.0).astype(float)
    )
    r_profit_scaleout_override_ret = (
        rps_over_post.reindex(w.index).fillna(0.0).astype(float)
    )
    ma_trailing_stop_override_ret = (
        ma_over_post.reindex(w.index).fillna(0.0).astype(float)
    )
    ...
    if has_initial_account_amount:
        ...
        w_real, account_lot_meta = simulate_lot_account_weights(
            ...
            periodic_rebalance_enabled=bool(
                ps == "risk_budget" and vol_periodic_risk_mgmt_enabled
            ),
        ...
        if bool(ps == "risk_budget" and vol_periodic_risk_mgmt_enabled):
            vol_periodic_rebalance_stats_overall = {
    ...
    if not quick_mode:
        ...
        r_profit_scaleout_stats = {
            **(r_profit_scaleout_stats or {}),
            "trade_records": _enrich_trade_records_with_engine_timeline(
                records=list(
                    (r_profit_scaleout_stats or {}).get("trade_records") or []
                ),
                effective_weight=w.astype(float),
                exec_price_series=px_exec_slip.reindex(w.index).ffill().astype(float),
                slippage_spread=float(inp.slippage_rate),
        ...
        ma_trailing_stop_stats = {
            **(ma_trailing_stop_stats or {}),
            "trade_records": _enrich_trade_records_with_engine_timeline(
                records=list((ma_trailing_stop_stats or {}).get("trade_records") or []),
                effective_weight=w.astype(float),
                exec_price_series=px_exec_slip.reindex(w.index).ffill().astype(float),
                slippage_spread=float(inp.slippage_rate),
            ),
        ...
        ma_trailing_stop_stats = {**(ma_trailing_stop_stats or {}), "trade_records": []}
    turnover_daily = ((w - w.shift(1).fillna(0.0)).abs() / 2.0).astype(float)
    return_decomposition: dict[str, Any] | None = None
    if not quick_mode:
        ret_overnight_close_none_day = (
            (o_none / c_none.shift(1) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
        ret_overnight_close_hfq_day = (
            (o_hfq / c_hfq.shift(1) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
        ret_overnight_close_day = ret_overnight_close_none_day.where(
            ~cm, ret_overnight_close_hfq_day
        ).astype(float)
        ret_intraday_close_day = (
            ret_exec_open_day.reindex(w.index).fillna(0.0).astype(float)
        )
        if ep == "open":
            decomp_overnight = pd.Series(0.0, index=w.index, dtype=float)
            decomp_intraday = (
                w_ret * ret_exec_open_day.reindex(w.index).fillna(0.0).astype(float)
            ).astype(float)
            decomp_interaction = pd.Series(0.0, index=w.index, dtype=float)
        elif ep == "close":
            decomp_overnight = (
                w_ret
                * ret_overnight_close_day.reindex(w.index).fillna(0.0).astype(float)
            ).astype(float)
            decomp_intraday = (w_ret * ret_intraday_close_day).astype(float)
            decomp_interaction = (
                w_ret
                * ret_overnight_close_day.reindex(w.index).fillna(0.0).astype(float)
                * ret_intraday_close_day
            ...
            decomp_intraday = (
                w_ret
                * (
                    0.5 * ret_exec_open_day.reindex(w.index).fillna(0.0).astype(float)
                    + 0.5 * ret_intraday_close_day
                )
            ).astype(float)
            decomp_interaction = (
                0.5
        ...
        decomp_r_profit_scaleout_override = r_profit_scaleout_override_ret.astype(float)
        decomp_ma_trailing_stop_override = ma_trailing_stop_override_ret.astype(float)
        decomp_risk_exit_override = (
            ...
            + decomp_ma_trailing_stop_override
        ).astype(float)
        ...
        return_decomposition = {
            ...
            "series": {
                ...
                "r_profit_scaleout_override": decomp_r_profit_scaleout_override.astype(
                    float
                ).tolist(),
                "ma_trailing_stop_override": decomp_ma_trailing_stop_override.astype(
                    float
                ).tolist(),
            ...
            "summary": {
                ...
                "ann_r_profit_scaleout_override": float(
                    decomp_r_profit_scaleout_override.iloc[1:].mean()
                    * TRADING_DAYS_PER_YEAR
                )
                if len(decomp_r_profit_scaleout_override) > 1
                else 0.0,
                "ann_ma_trailing_stop_override": float(
                    decomp_ma_trailing_stop_override.iloc[1:].mean()
                    * TRADING_DAYS_PER_YEAR
                )
                if len(decomp_ma_trailing_stop_override) > 1
                else 0.0,
    ...
    _append_var_cvar_metrics(m_strat, strat_ret)
    m_strat["cvar_scale_trigger_count"] = int(
        (cvar_position_scale_stats or {}).get("cvar_scale_trigger_count", 0) or 0
    )
    m_strat["cvar_scale_trigger_episode_count"] = int(
        (cvar_position_scale_stats or {}).get("cvar_scale_trigger_episode_count", 0)
        or 0
    )
    m_strat["cvar_scale_fallback_count"] = int(
        (cvar_position_scale_stats or {}).get("cvar_scale_fallback_count", 0) or 0
    )
    ...
    if not search_minimal_mode:
        ...
        _append_var_cvar_metrics(m_bh, ret_bh)
        m_ex = {
        ...
        _append_var_cvar_metrics(m_ex, active)
    ...
    trade_one = _trade_returns_from_weight_series(
        ...
        ret_exec_day,
        return_weight=w_ret,
        return_override=risk_exit_override_ret,
    ...
    trade_extreme_stats = _build_trade_extreme_stats_from_trades(
        trade_one.get("trades", []),
        exec_price=px_exec_slip.reindex(nav.index).ffill().astype(float),
        high_price=high_exec_trade.reindex(nav.index).ffill().astype(float),
        low_price=low_exec_trade.reindex(nav.index).ffill().astype(float),
        expected_codes=[str(code)],
        default_code=str(code),
    )
    trade_stats["overall"].update(dict(trade_extreme_stats.get("overall") or {}))
    trade_stats["by_code"][str(code)].update(
    ...
    ma_attempted_overall = int(
        (ma_entry_filter_stats_overall or {}).get("attempted_entry_count", 0)
    )
    ma_blocked_overall = int(
        (ma_entry_filter_stats_overall or {}).get("blocked_entry_count", 0)
    )
    ...
    ma_block_rate_overall = (
        float(ma_blocked_overall / ma_attempted_overall)
        if ma_attempted_overall > 0
        else 0.0
    )
    ...
    trade_stats["overall"]["r_profit_scaleout_trigger_count"] = int(
        (r_profit_scaleout_stats or {}).get("trigger_count", 0)
    )
    ...
    trade_stats["overall"]["r_profit_scaleout_tier_trigger_counts"] = dict(
        (r_profit_scaleout_stats or {}).get("tier_trigger_counts", {}) or {}
    )
    trade_stats["overall"]["ma_entry_filter_blocked_entry_count"] = int(
        (ma_entry_filter_stats_overall or {}).get("blocked_entry_count", 0)
    )
    trade_stats["overall"]["ma_entry_filter_attempted_entry_count"] = (
        ma_attempted_overall
    )
    trade_stats["overall"]["ma_entry_filter_allowed_entry_count"] = int(
        (ma_entry_filter_stats_overall or {}).get("allowed_entry_count", 0)
    )
    trade_stats["overall"]["ma_entry_filter_blocked_entry_rate"] = float(
        ma_block_rate_overall
    )
    ...
    trade_stats["overall"]["vol_periodic_rebalance_evaluated_count"] = int(
        (vol_periodic_rebalance_stats_overall or {}).get(
            "periodic_rebalance_evaluated_count", 0
        )
    )
    trade_stats["overall"]["vol_periodic_rebalance_trigger_count"] = int(
        (vol_periodic_rebalance_stats_overall or {}).get(
            "periodic_rebalance_trigger_count", 0
        )
    )
    trade_stats["overall"]["vol_periodic_rebalance_skip_count"] = int(
        (vol_periodic_rebalance_stats_overall or {}).get(
            "periodic_rebalance_skip_count", 0
        )
    )
    ...
    trade_stats["by_code"][str(code)]["r_profit_scaleout_trigger_count"] = int(
        (r_profit_scaleout_stats or {}).get("trigger_count", 0)
    )
    ...
    trade_stats["by_code"][str(code)]["r_profit_scaleout_tier_trigger_counts"] = dict(
        (r_profit_scaleout_stats or {}).get("tier_trigger_counts", {}) or {}
    )
    trade_stats["by_code"][str(code)]["ma_entry_filter_blocked_entry_count"] = int(
        (ma_entry_filter_stats_overall or {}).get("blocked_entry_count", 0)
    )
    trade_stats["by_code"][str(code)]["ma_entry_filter_attempted_entry_count"] = (
        ma_attempted_overall
    )
    trade_stats["by_code"][str(code)]["ma_entry_filter_allowed_entry_count"] = int(
        (ma_entry_filter_stats_overall or {}).get("allowed_entry_count", 0)
    )
    trade_stats["by_code"][str(code)]["ma_entry_filter_blocked_entry_rate"] = float(
        ma_block_rate_overall
    )
    ...
    trade_stats["by_code"][str(code)]["vol_periodic_rebalance_evaluated_count"] = int(
        (vol_periodic_rebalance_stats_overall or {}).get(
            "periodic_rebalance_evaluated_count", 0
        )
    )
    trade_stats["by_code"][str(code)]["vol_periodic_rebalance_trigger_count"] = int(
        (vol_periodic_rebalance_stats_overall or {}).get(
            "periodic_rebalance_trigger_count", 0
        )
    )
    trade_stats["by_code"][str(code)]["vol_periodic_rebalance_skip_count"] = int(
        (vol_periodic_rebalance_stats_overall or {}).get(
            "periodic_rebalance_skip_count", 0
        )
    )
    ...
    m_strat["r_profit_scaleout_tier_trigger_counts"] = dict(
        (r_profit_scaleout_stats or {}).get("tier_trigger_counts", {}) or {}
    )
    ...
    out = {
        "meta": {
            ...
            "params": {
                ...
                "r_profit_scaleout_tiers": rps_tiers,
                ...
                "ma_trailing_stop_reduce_window": int(ma_trailing_stop_reduce_window),
                "ma_trailing_stop_exit_window": int(ma_trailing_stop_exit_window),
                "ma_trailing_stop_reduce_fraction": float(
                    ma_trailing_stop_reduce_fraction
                ),
                "risk_control_compatibility_meta": dict(
                    risk_control_compatibility_meta
                ),
                ...
                "risk_budget_pct": float(risk_budget_pct),
                "cvar_risk_mgmt_enabled": bool(cvar_risk_mgmt_enabled),
                "cvar_window": int(cvar_window),
                "cvar_budget_pct": float(cvar_budget_pct),
                "risk_budget_overcap_policy": str(risk_budget_overcap_policy),
                "risk_budget_max_leverage_multiple": float(
                ...
                "vol_ratio_extreme_threshold": float(vol_ratio_extreme_threshold),
                "risk_of_ruin_maxrisk": float(risk_of_ruin_maxrisk),
                "er_filter": bool(er_filter),
                "er_window": int(er_window),
                "er_threshold": float(er_threshold),
                ...
                "ma_entry_filter_enabled": bool(ma_entry_filter_enabled),
                "ma_entry_filter_type": str(ma_entry_filter_type),
                "ma_entry_filter_fast": int(ma_entry_filter_fast),
                "ma_entry_filter_slow": int(ma_entry_filter_slow),
        ...
        "asset_nav_exec": {
            "dates": nav.index.date.astype(str).tolist(),
            "series": {str(code): nav.astype(float).tolist()},
        },
        "signals": {
        ...
        "risk_controls": {
            ...
            "ma_trailing_stop": ma_trailing_stop_stats,
            "addons": {
                "r_profit_scaleout_breakeven_stop": {
                    "enabled": bool(rps_breakeven_stop_enabled),
                    **dict((r_profit_scaleout_stats or {}).get("breakeven_stop") or {}),
                },
                "bias_v_take_profit_breakeven_stop": {
                    "enabled": bool(bias_v_tp_breakeven_stop_enabled),
                    **dict(
                        (bias_v_take_profit_stats or {}).get("breakeven_stop") or {}
                    ),
                },
            },
            ...
            "vol_periodic_risk_mgmt": {
                "enabled": bool(ps == "risk_budget" and vol_periodic_risk_mgmt_enabled),
                "atr_window": int(risk_budget_atr_window),
                "rebalance_threshold_pct": float(vol_periodic_rebalance_threshold_pct),
                "evaluated_count": int(
                    (vol_periodic_rebalance_stats_overall or {}).get(
                        "periodic_rebalance_evaluated_count", 0
                    )
        ...
        "next_plan": {
            ...
            "trace": {
                ...
                "r_profit_scaleout": {
                    "enabled": bool(
                        (r_profit_scaleout_stats or {}).get("enabled", False)
                    ),
                    "breakeven_stop_enabled": bool(rps_breakeven_stop_enabled),
                    "breakeven_stop": dict(
                        (r_profit_scaleout_stats or {}).get("breakeven_stop") or {}
                    ),
                    "initial_r_mode": (r_profit_scaleout_stats or {}).get(
                        "initial_r_mode"
                    ),
                    "fallback_mode_used": bool(
                        (r_profit_scaleout_stats or {}).get("fallback_mode_used", False)
                    ),
                    "trigger_count": int(
                        (r_profit_scaleout_stats or {}).get("trigger_count", 0)
                    ),
                    "tiers": (r_profit_scaleout_stats or {}).get("tiers", []),
                    "tier_trigger_counts": (
                        (r_profit_scaleout_stats or {}).get("tier_trigger_counts", {})
                        or {}
                    ),
                    "trace_last_rows": (r_profit_scaleout_stats or {}).get(
                        "trace_last_rows", []
                    ),
                },
                "bias_v_take_profit": {
                    ...
                    "breakeven_stop_enabled": bool(bias_v_tp_breakeven_stop_enabled),
                    "breakeven_stop": dict(
                        (bias_v_take_profit_stats or {}).get("breakeven_stop") or {}
                    ),
...
def compute_trend_portfolio_backtest(
    ...
    if data_override is not None:
        ...
        close_qfq = close_qfq_raw.ffill()
        close_hfq = (
            data_override["close_hfq"].reindex(columns=codes).reindex(dates).ffill()
        )
        high_none = close_qfq.reindex(index=dates, columns=codes).astype(float)
        low_none = close_qfq.reindex(index=dates, columns=codes).astype(float)
        high_hfq = close_hfq.reindex(index=dates, columns=codes).astype(float)
        low_hfq = close_hfq.reindex(index=dates, columns=codes).astype(float)
        asset_data_available = close_qfq_raw.notna().reindex(
            index=dates, columns=codes, fill_value=False
        )
        ...
        high_exec_trade_df = high_none.reindex(index=dates, columns=codes).astype(float)
        low_exec_trade_df = low_none.reindex(index=dates, columns=codes).astype(float)
        ...
        ext_start = inp.start - dt.timedelta(days=int(need_hist) * 2)
        close_none_raw = load_close_prices(
            db, codes=codes, start=inp.start, end=inp.end, adjust="none"
        ).sort_index()
        close_none = close_none_raw.ffill()
        close_qfq = load_close_prices(
        ...
        low_hfq = close_hfq.reindex(columns=codes).astype(float)
        if close_none.empty:
            raise ValueError("no execution price data for given range (none)")
        if not bool(getattr(inp, "dynamic_universe", False)):
            ...
            close_none_raw = close_none_raw.loc[common_start:]
            close_qfq = close_qfq.loc[common_start:]
            close_hfq = close_hfq.loc[common_start:]
        dates = close_none.index
        close_none_raw = close_none_raw.reindex(index=dates).reindex(columns=codes)
        asset_data_available = close_none_raw.notna().reindex(
            index=dates, columns=codes, fill_value=False
        )
        du_b = bool(getattr(inp, "dynamic_universe", False))
        ...
        high_none = (
            ...
            .reindex(columns=codes)
            .ffill()
            .combine_first(close_none.reindex(columns=codes).astype(float))
            .astype(float)
        )
        low_none = (
        ...
        high_hfq = (
            ...
            .reindex(columns=codes)
            .ffill()
            .combine_first(close_hfq.reindex(columns=codes).astype(float))
            .astype(float)
        )
        low_hfq = (
        ...
        if ep == "open":
            ...
            ret_none = (
                (px_none_exec.div(px_none_exec.shift(1)) - 1.0)
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0.0)
                .astype(float)
            )
            ret_hfq_exec = (
                (px_hfq_exec.div(px_hfq_exec.shift(1)) - 1.0)
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0.0)
                .astype(float)
            )
        ...
        high_exec_trade_df = (
            high_none.where(~ca_mask.fillna(False), other=high_hfq)
            .replace([np.inf, -np.inf], np.nan)
            .ffill()
            .combine_first(px_exec_slip)
            .astype(float)
        ...
        low_exec_trade_df = (
            low_none.where(~ca_mask.fillna(False), other=low_hfq)
            .replace([np.inf, -np.inf], np.nan)
            .ffill()
            .combine_first(px_exec_slip)
            .astype(float)
        ...
        ret_overnight_none_close = (
            (open_none.div(close_none_exec.shift(1)) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
        ...
        ret_overnight_hfq_close = (
            (open_hfq.div(close_hfq_exec.shift(1)) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .astype(float)
        )
        ...
        ret_overnight_none_open = pd.DataFrame(
            0.0,
            index=ret_intraday_none_open.index,
            columns=ret_intraday_none_open.columns,
        ).astype(float)
        ...
        ret_overnight_hfq_open = pd.DataFrame(
            0.0,
            index=ret_intraday_hfq_open.index,
            columns=ret_intraday_hfq_open.columns,
        ).astype(float)
    ...
    r_profit_scaleout_by_asset: dict[str, dict[str, Any]] = {}
    bias_v_tp_by_asset: dict[str, dict[str, Any]] = {}
    ma_trailing_stop_by_asset: dict[str, dict[str, Any]] = {}
    ma_entry_filter_by_asset: dict[str, dict[str, int]] = {}
    ...
    for c in codes:
        ...
        if ma_entry_filter_enabled:
            pos, one_ma_entry_stats = _apply_ma_cross_entry_filter(
            ...
            ma_entry_filter_by_asset[str(c)] = dict(one_ma_entry_stats or {})
        else:
            ma_entry_filter_by_asset[str(c)] = {
                "blocked_entry_count": 0,
                "attempted_entry_count": 0,
                "allowed_entry_count": 0,
            }
        ...
        low_px = low_qfq_df[c] if (c in low_qfq_df.columns) else px
        risk_open = (
            open_qfq_df[c].astype(float).fillna(px) if c in open_qfq_df.columns else px
        )
        risk_close = px.astype(float)
        risk_high = h.astype(float).fillna(px)
        risk_low = low_px.astype(float).fillna(px)
        if c in asset_data_available.columns:
            market_observed = (
                asset_data_available[c].reindex(px.index, fill_value=False).astype(bool)
            )
            risk_open = risk_open.where(market_observed)
            risk_close = risk_close.where(market_observed)
            risk_high = risk_high.where(market_observed)
            risk_low = risk_low.where(market_observed)
        if ep == "close":
            risk_open = risk_open.shift(-1)
            risk_close = risk_close.shift(-1)
            risk_high = risk_high.shift(-1)
        ...
        _, one_rps_stats = _build_r_profit_scaleout_plan(
            pos.fillna(0.0).astype(float),
            open_=risk_open,
            close=risk_close,
            high=risk_high,
        ...
        _, one_ma_trailing_stop_stats = _build_ma_trailing_stop_plan(
            pos.fillna(0.0).astype(float),
            open_=risk_open,
            close=risk_close,
            high=risk_high,
        ...
        if ep == "close":
            one_stop_stats = _advance_overlay_market_dates(one_stop_stats, dates)
            one_rtp_stats = _advance_overlay_market_dates(one_rtp_stats, dates)
            one_rps_stats = _advance_overlay_market_dates(one_rps_stats, dates)
            one_bias_v_tp_stats = _advance_overlay_market_dates(
                one_bias_v_tp_stats, dates
            )
            one_ma_trailing_stop_stats = _advance_overlay_market_dates(
                one_ma_trailing_stop_stats, dates
            )
        atr_stop_by_asset[str(c)] = one_stop_stats
        rtp_by_asset[str(c)] = one_rtp_stats
        r_profit_scaleout_by_asset[str(c)] = one_rps_stats
        bias_v_tp_by_asset[str(c)] = one_bias_v_tp_stats
        ma_trailing_stop_by_asset[str(c)] = one_ma_trailing_stop_stats
    ...
    vol_periodic_rebalance_by_asset: dict[str, dict[str, int]] = {
        str(c): {
            "periodic_rebalance_evaluated_count": 0,
            "periodic_rebalance_trigger_count": 0,
            "periodic_rebalance_skip_count": 0,
    ...
    periodic_enabled = bool(ps == "risk_budget" and vol_periodic_risk_mgmt_enabled)
    periodic_uses_lot_threshold = bool(periodic_enabled and has_initial_account_amount)
    for d in dates:
        day_seq += 1
        ...
        if ps == "fixed_ratio":
            ...
            def _set_new_risk_budget_entry(key: str, base_target: float) -> None:
                ...
                if bool(vol_regime_risk_mgmt_enabled):
                    ...
                    entry_state = _vol_regime_state_from_ratio(
                        ratio,
                        expand_threshold=float(vol_ratio_expand_threshold),
                        contract_threshold=float(vol_ratio_contract_threshold),
                        extreme_threshold=float(vol_ratio_extreme_threshold),
                    )
                    rb_state_by_code[key] = str(entry_state)
                    if entry_state == "REDUCED":
                        vol_risk_adjust_by_asset[key][
                            "vol_risk_entry_state_reduce_on_expand_count"
                        ] += 1
                    elif entry_state == "INCREASED":
                        vol_risk_adjust_by_asset[key][
                            "vol_risk_entry_state_increase_on_contract_count"
                        ] += 1
                else:
    ...
    if bool(cvar_risk_mgmt_enabled):
        cvar_asset_ret_df = (
            ret_hfq.reindex(index=dates, columns=codes)
            .astype(float)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
        )
        w_decision, cvar_position_scale_stats = _apply_cvar_budget_scaling(
            w_decision.reindex(index=dates, columns=codes).astype(float).fillna(0.0),
            asset_returns=cvar_asset_ret_df,
            enabled=True,
            window=int(cvar_window),
            budget_pct=float(cvar_budget_pct),
        )
    ...
    # Post-trade weight on each day (open mode: after open trade, close mode: after close trade).
    membership_mask = _pool_membership_mask(db, codes=codes, index=w_decision.index)
    w_decision = w_decision.where(membership_mask, 0.0).astype(float)
    w_post = w_decision.shift(1).fillna(0.0).astype(float)
    open_sig_df_for_stop = (
        open_qfq_df.reindex(index=w_post.index, columns=codes).astype(float).ffill()
    )
    close_sig_df_for_stop = (
        close_qfq.reindex(index=w_post.index, columns=codes).astype(float).ffill()
    )
    ...
    def _apply_all_intraday_overlays_df(
        ...
        *,
        ...
        w_tmp, over_by_overlay = _apply_intraday_or_arbitration_portfolio(
            weights=weight_in.astype(float).copy(),
            event_sets_by_asset={
                "atr_stop": atr_stop_by_asset,
                "bias_v_take_profit": bias_v_tp_by_asset,
                "r_take_profit": rtp_by_asset,
                "r_profit_scaleout": r_profit_scaleout_by_asset,
                "ma_trailing_stop": ma_trailing_stop_by_asset,
            },
            exec_price=str(mode),
    ...
    if data_override is None:
        cm = (
            ca_mask.reindex(index=w.index, columns=codes).fillna(False).astype(bool)
            if isinstance(ca_mask, pd.DataFrame)
            else pd.DataFrame(False, index=w.index, columns=codes)
        )
        ret_exec_open_none_day = (
            (close_none_exec / open_none - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .reindex(index=w.index, columns=codes)
            .astype(float)
        )
        ret_exec_open_hfq_day = (
        ...
        ret_exec_open_day = ret_exec_open_none_day.where(
            ~cm, ret_exec_open_hfq_day
        ).astype(float)
        ret_exec_close_none_day = (
            (close_none_exec / close_none_exec.shift(1) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .reindex(index=w.index, columns=codes)
            .astype(float)
        ...
        ret_exec_close_hfq_day = (
            (close_hfq / close_hfq.shift(1) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .reindex(index=w.index, columns=codes)
            .astype(float)
        ...
        ret_overnight_close_none_day = (
            (open_none / close_none_exec.shift(1) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .reindex(index=w.index, columns=codes)
            .astype(float)
        ...
        ret_overnight_close_hfq_day = (
            (open_hfq / close_hfq.shift(1) - 1.0)
            .replace([np.inf, -np.inf], np.nan)
            .fillna(0.0)
            .reindex(index=w.index, columns=codes)
            .astype(float)
        ...
        ret_intraday_close_day = ret_exec_open_day.astype(float)
    else:
        ret_exec_open_day = (
            ret_intraday.reindex(index=w.index, columns=codes).fillna(0.0).astype(float)
        )
        ret_overnight_close_day = (
            ret_overnight.reindex(index=w.index, columns=codes)
            .fillna(0.0)
            .astype(float)
        )
        ret_intraday_close_day = (
        ...
        ret_exec_close_day = (
            ret_overnight_close_day
            + ret_intraday_close_day
            + ret_overnight_close_day * ret_intraday_close_day
        ).astype(float)
    ...
    risk_exit_override_by_asset = (
        risk_over_by_asset_post.reindex(index=w.index, columns=codes)
        .fillna(0.0)
        .astype(float)
    )
    ...
    if not quick_mode:
        for c in codes:
            ...
            one_rps = r_profit_scaleout_by_asset.get(key) or {}
            r_profit_scaleout_by_asset[key] = {
                **one_rps,
                "trade_records": _enrich_trade_records_with_engine_timeline(
                    records=list(one_rps.get("trade_records") or []),
                    effective_weight=w[key].astype(float),
                    exec_price_series=px_exec_slip[key]
                    .reindex(w.index)
                    .ffill()
            ...
            one_ma = ma_trailing_stop_by_asset.get(key) or {}
            ma_trailing_stop_by_asset[key] = {
                **one_ma,
                "trade_records": _enrich_trade_records_with_engine_timeline(
                    records=list(one_ma.get("trade_records") or []),
                    effective_weight=w[key].astype(float),
                    exec_price_series=px_exec_slip[key]
                    .reindex(w.index)
                    .ffill()
        ...
        for c in codes:
            ...
            one_rps = r_profit_scaleout_by_asset.get(key) or {}
            r_profit_scaleout_by_asset[key] = {**one_rps, "trade_records": []}
            one_ma = ma_trailing_stop_by_asset.get(key) or {}
            ma_trailing_stop_by_asset[key] = {**one_ma, "trade_records": []}
    decomp_atr_stop_override = (
    ...
    decomp_r_profit_scaleout_override = (
        r_profit_scaleout_override_ret.reindex(w.index).fillna(0.0).astype(float)
    )
    decomp_ma_trailing_stop_override = (
        ma_trailing_stop_override_ret.reindex(w.index).fillna(0.0).astype(float)
    )
    decomp_risk_exit_override = (
        decomp_atr_stop_override
        + decomp_bias_v_take_profit_override
    ...
    if not quick_mode:
        if ep == "open":
            comp_overnight = pd.Series(0.0, index=w.index, dtype=float)
            comp_intraday = (
                (
                    w_ret
                    * ret_exec_open_day.reindex(index=w.index, columns=codes).fillna(
                        0.0
                    )
                )
                .sum(axis=1)
            ...
            comp_interaction = pd.Series(0.0, index=w.index, dtype=float)
        elif ep == "close":
            comp_overnight = (
                (
                    w_ret
                    * ret_overnight_close_day.reindex(
            ...
            comp_intraday = (
                (
                    w_ret
                    * ret_intraday_close_day.reindex(
                        index=w.index, columns=codes
                    ).fillna(0.0)
                )
                .sum(axis=1)
    ...
    _append_var_cvar_metrics(m_strat, port_ret)
    m_strat["cvar_scale_trigger_count"] = int(
        (cvar_position_scale_stats or {}).get("cvar_scale_trigger_count", 0) or 0
    )
    m_strat["cvar_scale_trigger_episode_count"] = int(
    ...
    if not search_minimal_mode:
        ...
        _append_var_cvar_metrics(m_bench, bench_ret)
        m_ex = {
    ...
    trade_pack = _trade_returns_from_weight_df(
        ...
        return_weight=w_ret.reindex(index=nav.index, columns=codes)
        .astype(float)
        .fillna(0.0),
        return_override=risk_exit_override_by_asset.reindex(
            index=nav.index, columns=codes
        )
        .astype(float)
        .fillna(0.0),
        cost_bps=float(inp.cost_bps),
    ...
    portfolio_semi_variance_by_code: dict[str, dict[str, Any]] = {}
    for c in codes:
        code_key = str(c)
        code_ret = ret_exec_for_runs.get(c)
        if not isinstance(code_ret, pd.Series):
            code_ret = pd.Series(0.0, index=ret_exec_for_runs.index, dtype=float)
        portfolio_semi_variance_by_code[code_key] = (
    ...
    trade_extreme_stats = _build_trade_extreme_stats_from_trades(
        ...
        .ffill()
        .astype(float),
        high_price=high_exec_trade_df.reindex(index=nav.index, columns=codes)
        .ffill()
        .astype(float),
        low_price=low_exec_trade_df.reindex(index=nav.index, columns=codes)
        .ffill()
        .astype(float),
    ...
    trade_stats["overall"].update(dict(trade_extreme_stats.get("overall") or {}))
    by_code_extreme = dict(trade_extreme_stats.get("by_code") or {})
    for c in codes:
        ck = str(c)
    ...
    for c in codes:
        ...
        if isinstance(one, dict):
            one["semi_variance"] = dict(
                portfolio_semi_variance_by_code.get(code_key) or {}
            )
    event_study = None
    if not quick_mode:
        event_study = compute_event_study(
    ...
    total_r_profit_scaleout_triggers = int(
        sum(
            int((v or {}).get("trigger_count", 0))
            for v in r_profit_scaleout_by_asset.values()
        )
    )
    total_bias_v_tp_triggers = int(
        sum(int((v or {}).get("trigger_count", 0)) for v in bias_v_tp_by_asset.values())
    )
    r_profit_scaleout_breakeven_by_asset = {
        str(k): dict(((v or {}).get("breakeven_stop") or {}))
        for k, v in r_profit_scaleout_by_asset.items()
    }
    bias_v_tp_breakeven_by_asset = {
        str(k): dict(((v or {}).get("breakeven_stop") or {}))
        for k, v in bias_v_tp_by_asset.items()
    }
    total_r_profit_scaleout_breakeven_triggers = int(
        sum(
            int((v or {}).get("trigger_count", 0))
            for v in r_profit_scaleout_breakeven_by_asset.values()
        )
    )
    total_bias_v_tp_breakeven_triggers = int(
        sum(
            int((v or {}).get("trigger_count", 0))
            for v in bias_v_tp_breakeven_by_asset.values()
        )
    )
    total_ma_trailing_stop_triggers = int(
        sum(
            int((v or {}).get("trigger_count", 0))
            for v in ma_trailing_stop_by_asset.values()
        )
    )
    total_ma_entry_filter_blocked_entries = int(
        sum(
            int((v or {}).get("blocked_entry_count", 0))
            for v in ma_entry_filter_by_asset.values()
        )
    )
    total_ma_entry_filter_attempted_entries = int(
        sum(
            int((v or {}).get("attempted_entry_count", 0))
            for v in ma_entry_filter_by_asset.values()
        )
    )
    total_ma_entry_filter_allowed_entries = int(
        sum(
    ...
    total_vol_periodic_rebalance_evaluated = int(
        sum(
            int((v or {}).get("periodic_rebalance_evaluated_count", 0))
            for v in vol_periodic_rebalance_by_asset.values()
        )
    )
    total_vol_periodic_rebalance_trigger = int(
        sum(
            int((v or {}).get("periodic_rebalance_trigger_count", 0))
            for v in vol_periodic_rebalance_by_asset.values()
        )
    )
    total_vol_periodic_rebalance_skip = int(
        sum(
    ...
    total_ma_entry_filter_blocked_rate = (
        float(
            total_ma_entry_filter_blocked_entries
            / total_ma_entry_filter_attempted_entries
        )
        if total_ma_entry_filter_attempted_entries > 0
        else 0.0
    )
    ...
    total_r_profit_scaleout_tier_counts: dict[str, int] = {}
    for v in r_profit_scaleout_by_asset.values():
        one = dict(((v or {}).get("tier_trigger_counts") or {}))
        for k, n in one.items():
            kk = str(k).strip()
            ...
            try:
                ...
                nn = 0
            total_r_profit_scaleout_tier_counts[kk] = int(
                total_r_profit_scaleout_tier_counts.get(kk, 0) + nn
            )
    ...
    uniq_r_profit_scaleout_trigger_dates = sorted(
        {
            str(d)
            for v in r_profit_scaleout_by_asset.values()
            for d in (v or {}).get("trigger_dates", [])
    ...
    uniq_r_profit_scaleout_breakeven_trigger_dates = sorted(
        {
            str(d)
            for v in r_profit_scaleout_breakeven_by_asset.values()
            for d in (v or {}).get("trigger_dates", [])
    ...
    uniq_bias_v_tp_breakeven_trigger_dates = sorted(
        {
            str(d)
            for v in bias_v_tp_breakeven_by_asset.values()
            for d in (v or {}).get("trigger_dates", [])
    ...
    uniq_ma_trailing_stop_trigger_dates = sorted(
        {
            str(d)
            for v in ma_trailing_stop_by_asset.values()
            for d in (v or {}).get("trigger_dates", [])
    ...
    trade_stats["overall"]["r_profit_scaleout_trigger_count"] = int(
        total_r_profit_scaleout_triggers
    )
    trade_stats["overall"]["bias_v_take_profit_trigger_count"] = int(
    ...
    trade_stats["overall"]["r_profit_scaleout_tier_trigger_counts"] = dict(
        total_r_profit_scaleout_tier_counts
    )
    trade_stats["overall"]["ma_entry_filter_blocked_entry_count"] = int(
        total_ma_entry_filter_blocked_entries
    )
    trade_stats["overall"]["ma_entry_filter_attempted_entry_count"] = int(
        total_ma_entry_filter_attempted_entries
    )
    trade_stats["overall"]["ma_entry_filter_allowed_entry_count"] = int(
        total_ma_entry_filter_allowed_entries
    )
    trade_stats["overall"]["ma_entry_filter_blocked_entry_rate"] = float(
        total_ma_entry_filter_blocked_rate
    )
    ...
    trade_stats["overall"]["vol_periodic_rebalance_evaluated_count"] = int(
        total_vol_periodic_rebalance_evaluated
    )
    trade_stats["overall"]["vol_periodic_rebalance_trigger_count"] = int(
        total_vol_periodic_rebalance_trigger
    )
    trade_stats["overall"]["vol_periodic_rebalance_skip_count"] = int(
        total_vol_periodic_rebalance_skip
    )
    ...
    for c in codes:
        ...
        one["r_profit_scaleout_trigger_count"] = int(
            (r_profit_scaleout_by_asset.get(code_key) or {}).get("trigger_count", 0)
        )
        ...
        one["r_profit_scaleout_tier_trigger_counts"] = dict(
            (
                (r_profit_scaleout_by_asset.get(code_key) or {}).get(
                    "tier_trigger_counts"
                )
                or {}
            )
        )
        one["ma_entry_filter_blocked_entry_count"] = int(
            (ma_entry_filter_by_asset.get(code_key) or {}).get("blocked_entry_count", 0)
        )
        one["ma_entry_filter_attempted_entry_count"] = int(
            (ma_entry_filter_by_asset.get(code_key) or {}).get(
                "attempted_entry_count", 0
            )
        )
        one["ma_entry_filter_allowed_entry_count"] = int(
            (ma_entry_filter_by_asset.get(code_key) or {}).get("allowed_entry_count", 0)
        )
        one["ma_entry_filter_blocked_entry_rate"] = (
            float(
                one["ma_entry_filter_blocked_entry_count"]
                / one["ma_entry_filter_attempted_entry_count"]
            )
            if int(one["ma_entry_filter_attempted_entry_count"]) > 0
            else 0.0
        )
        ...
        one["vol_periodic_rebalance_evaluated_count"] = int(
            (vol_periodic_rebalance_by_asset.get(code_key) or {}).get(
                "periodic_rebalance_evaluated_count", 0
            )
        )
        one["vol_periodic_rebalance_trigger_count"] = int(
            (vol_periodic_rebalance_by_asset.get(code_key) or {}).get(
                "periodic_rebalance_trigger_count", 0
            )
        )
        one["vol_periodic_rebalance_skip_count"] = int(
            (vol_periodic_rebalance_by_asset.get(code_key) or {}).get(
                "periodic_rebalance_skip_count", 0
            )
        )
    ...
    m_strat["ma_trailing_stop_trigger_count"] = int(total_ma_trailing_stop_triggers)
    ...
    close_corr_df = close_qfq.reindex(index=nav.index, columns=codes).where(
        asset_data_available.reindex(index=nav.index, columns=codes, fill_value=False)
    )
    group_daily_ret_df = (
        close_corr_df.astype(float)
        .pct_change(fill_method=None)
        .replace([np.inf, -np.inf], np.nan)
    )
    ewma_corr_60 = _ewma_corr_matrix_latest(group_daily_ret_df, window=60, decay=0.94)

    asset_nav_exec_series: dict[str, list[float | None]] = {}
    for c in codes:
        nav_c = (1.0 + ret_exec_day[c].astype(float)).cumprod().astype(float)
        avail_c = (
            asset_data_available[c].astype(bool)
            if c in asset_data_available.columns
            else pd.Series(False, index=ret_exec_day.index)
        )
        if len(avail_c.index) != len(nav_c.index):
            avail_c = avail_c.reindex(nav_c.index, fill_value=False).astype(bool)
        if bool(avail_c.any()):
            first_dt = avail_c[avail_c].index.min()
            last_dt = avail_c[avail_c].index.max()
    ...
    return {
        ...
        "risk_controls": {
            ...
            "r_profit_scaleout": {
                "enabled": bool(rps_enabled),
                "tiers": rps_tiers,
                "trigger_count": int(total_r_profit_scaleout_triggers),
                "tier_trigger_counts": dict(total_r_profit_scaleout_tier_counts),
                "trigger_days": int(len(uniq_r_profit_scaleout_trigger_dates)),
                "first_trigger_date": (
                    uniq_r_profit_scaleout_trigger_dates[0]
                    if uniq_r_profit_scaleout_trigger_dates
                    else None
                ),
                "last_trigger_date": (
                    uniq_r_profit_scaleout_trigger_dates[-1]
                    if uniq_r_profit_scaleout_trigger_dates
                    else None
                ),
                "trigger_dates": uniq_r_profit_scaleout_trigger_dates[:200],
                "fallback_mode_used": bool((atr_mode == "none") and rps_enabled),
                "initial_r_mode": (
                    "atr_stop" if atr_mode != "none" else "virtual_atr_fallback"
                ),
                "execution_mode": str(rps_execution_mode),
                "by_asset": r_profit_scaleout_by_asset,
            },
            "bias_v_take_profit": {
            ...
            "addons": {
                "r_profit_scaleout_breakeven_stop": {
                    "enabled": bool(rps_breakeven_stop_enabled),
                    "trigger_count": int(total_r_profit_scaleout_breakeven_triggers),
                    "trigger_days": int(
                        len(uniq_r_profit_scaleout_breakeven_trigger_dates)
                    ),
                    "first_trigger_date": (
                        uniq_r_profit_scaleout_breakeven_trigger_dates[0]
                        if uniq_r_profit_scaleout_breakeven_trigger_dates
                        else None
                    ),
                    "last_trigger_date": (
                        uniq_r_profit_scaleout_breakeven_trigger_dates[-1]
                        if uniq_r_profit_scaleout_breakeven_trigger_dates
                        else None
                    ),
                    "trigger_dates": uniq_r_profit_scaleout_breakeven_trigger_dates[
                        :200
                    ],
                    "by_asset": r_profit_scaleout_breakeven_by_asset,
                },
                "bias_v_take_profit_breakeven_stop": {
                    "enabled": bool(bias_v_tp_breakeven_stop_enabled),
                    "trigger_count": int(total_bias_v_tp_breakeven_triggers),
                    "trigger_days": int(len(uniq_bias_v_tp_breakeven_trigger_dates)),
                    "first_trigger_date": (
                        uniq_bias_v_tp_breakeven_trigger_dates[0]
                        if uniq_bias_v_tp_breakeven_trigger_dates
                        else None
                    ),
                    "last_trigger_date": (
                        uniq_bias_v_tp_breakeven_trigger_dates[-1]
                        if uniq_bias_v_tp_breakeven_trigger_dates
                        else None
                    ),
                    "trigger_dates": uniq_bias_v_tp_breakeven_trigger_dates[:200],
                    "by_asset": bias_v_tp_breakeven_by_asset,
                },
            },
            "ma_trailing_stop": {
                "enabled": bool(ma_trailing_stop_enabled),
                ...
                "exit_window": int(ma_trailing_stop_exit_window),
                "reduce_fraction": float(ma_trailing_stop_reduce_fraction),
                "trigger_count": int(total_ma_trailing_stop_triggers),
                "trigger_days": int(len(uniq_ma_trailing_stop_trigger_dates)),
                "first_trigger_date": (
                    uniq_ma_trailing_stop_trigger_dates[0]
                    if uniq_ma_trailing_stop_trigger_dates
                    else None
                ),
                "last_trigger_date": (
                    uniq_ma_trailing_stop_trigger_dates[-1]
                    if uniq_ma_trailing_stop_trigger_dates
                    else None
                ),
                "trigger_dates": uniq_ma_trailing_stop_trigger_dates[:200],
                "by_asset": ma_trailing_stop_by_asset,
            },
            "er_exit_filter": {
                "enabled": bool(er_exit_filter),
                "window": int(er_exit_window),
            ...
            "vol_periodic_risk_mgmt": {
                ...
                "atr_window": int(risk_budget_atr_window),
                "rebalance_threshold_pct": float(vol_periodic_rebalance_threshold_pct),
                "evaluated_count": int(total_vol_periodic_rebalance_evaluated),
                "trigger_count": int(total_vol_periodic_rebalance_trigger),
                "skip_count": int(total_vol_periodic_rebalance_skip),
                "by_asset": vol_periodic_rebalance_by_asset,
            },
            "cvar_risk_mgmt": cvar_position_scale_stats,
            "monthly_risk_budget": monthly_risk_budget_gate_stats,
            "position_extension": {
