...
_build_ma_trailing_stop_plan_shared = (
    _trend_semantic_helpers._build_ma_trailing_stop_plan
)
...
_apply_intraday_or_arbitration_single = (
    _trend_semantic_helpers._apply_intraday_or_arbitration_single
)
_apply_intraday_or_arbitration_portfolio = (
    _trend_semantic_helpers._apply_intraday_or_arbitration_portfolio
)
...
def _align_ca_mask_like(
    target: pd.Series | pd.DataFrame,
    ca_mask: pd.Series | pd.DataFrame | None,
) -> pd.Series | pd.DataFrame:
    if isinstance(target, pd.DataFrame):
        if isinstance(ca_mask, pd.DataFrame):
            return (
                ca_mask.reindex(index=target.index, columns=target.columns)
                .fillna(False)
                .astype(bool)
            )
        if isinstance(ca_mask, pd.Series):
            s = ca_mask.reindex(target.index).fillna(False).astype(bool)
            return pd.DataFrame({c: s for c in target.columns}, index=target.index)
        return pd.DataFrame(False, index=target.index, columns=target.columns)
    if isinstance(ca_mask, pd.Series):
        return ca_mask.reindex(target.index).fillna(False).astype(bool)
    if isinstance(ca_mask, pd.DataFrame):
        if ca_mask.shape[1] == 0:
            return pd.Series(False, index=target.index, dtype=bool)
        return ca_mask.iloc[:, 0].reindex(target.index).fillna(False).astype(bool)
    return pd.Series(False, index=target.index, dtype=bool)


def _build_execution_day_components(
    *,
    exec_price: str,
    open_none: pd.Series | pd.DataFrame,
    ...
    ret_exec_close_none = _ratio_simple_return(c_none, c_none.shift(1)).astype(float)
    ret_exec_close_hfq = _ratio_simple_return(c_hfq, c_hfq.shift(1)).astype(float)

    if ep == "open":
        ret_none = ret_exec_open_none.astype(float)
        ret_hfq = ret_exec_open_hfq.astype(float)
        px_none_base = _as_float_like(o_none)
        px_hfq_base = _as_float_like(o_hfq)
    elif ep == "close":
        ret_none = ret_exec_close_none.astype(float)
        ret_hfq = ret_exec_close_hfq.astype(float)
    ...
    _, ca_mask_raw = corporate_action_mask(
        ...
        (1.0 + ret_hfq).astype(float),
    )
    ca_mask = _align_ca_mask_like(ret_none, ca_mask_raw)

    ret_exec_day = ret_none.where(~ca_mask, other=ret_hfq).astype(float)
    ret_exec_open_day = ret_exec_open_none.where(
        ~ca_mask, other=ret_exec_open_hfq
    ).astype(float)
    ret_exec_close_day = ret_exec_close_none.where(
        ~ca_mask, other=ret_exec_close_hfq
    ).astype(float)
    ret_overnight_close_none = _ratio_simple_return(o_none, c_none.shift(1)).astype(
        float
    )
    ret_overnight_close_hfq = _ratio_simple_return(o_hfq, c_hfq.shift(1)).astype(float)
    ret_overnight_close_day = ret_overnight_close_none.where(
        ~ca_mask, other=ret_overnight_close_hfq
    ).astype(float)
    ret_intraday_close_day = ret_exec_open_day.astype(float)
    px_exec_slip = (
        px_none_base.where(~ca_mask, other=px_hfq_base)
        .replace([np.inf, -np.inf], np.nan)
        .ffill()
        .astype(float)
    )
    ...
    return {
        ...
        "ret_exec_day": ret_exec_day.astype(float),
        "ret_exec_open_day": ret_exec_open_day.astype(float),
        "ret_exec_close_day": ret_exec_close_day.astype(float),
...
def _validate_bt_single_inputs(inp: Any) -> None:
    ...
    if bool(getattr(inp, "vol_regime_risk_mgmt_enabled", False)):
        ...
        vt_extreme = float(getattr(inp, "vol_ratio_extreme_threshold", 2.20) or 2.20)
    ...
    if rtp_exec_mode not in STOP_EXECUTION_MODES:
        raise ValueError(
    ...
    if bv_exec_mode not in STOP_EXECUTION_MODES:
        raise ValueError(
    ...
    if ma_exec_mode not in STOP_EXECUTION_MODES:
        raise ValueError(
    ...
    ma_reduce_window = int(getattr(inp, "ma_trailing_stop_reduce_window", 10) or 10)
    ma_exit_window = int(getattr(inp, "ma_trailing_stop_exit_window", 20) or 20)
    ma_effective_delay_days = int(
        getattr(inp, "ma_trailing_stop_effective_delay_days", 3) or 3
    )
    ma_reduce_fraction = float(
        getattr(inp, "ma_trailing_stop_reduce_fraction", 0.33) or 0.33
    )
    if ma_reduce_window < 2:
        raise ValueError("ma_trailing_stop_reduce_window must be >= 2")
    if ma_exit_window < 2:
        raise ValueError("ma_trailing_stop_exit_window must be >= 2")
    if ma_effective_delay_days < 1:
        raise ValueError("ma_trailing_stop_effective_delay_days must be >= 1")
    if (
        (not np.isfinite(ma_reduce_fraction))
        or ma_reduce_fraction <= 0.0
        or ma_reduce_fraction > 1.0
    ):
    ...
    _enforce_risk_control_compatibility(
        {
            ...
            "r_take_profit": bool(getattr(inp, "r_take_profit_enabled", False)),
            "r_profit_scaleout": bool(getattr(inp, "r_profit_scaleout_enabled", False)),
            "bias_v_take_profit": bool(
                getattr(inp, "bias_v_take_profit_enabled", False)
            ),
            "ma_trailing_stop": bool(getattr(inp, "ma_trailing_stop_enabled", False)),
...
def _build_meta_params(inp: Any) -> dict[str, Any]:
    out = {
        ...
        "risk_of_ruin_maxrisk": float(
            getattr(inp, "risk_of_ruin_maxrisk", 0.30) or 0.30
        ),
        ...
        "ma_trailing_stop_enabled": bool(
            getattr(inp, "ma_trailing_stop_enabled", False)
        ),
        ...
        "risk_control_compatibility_meta": _trend_semantic_helpers._build_risk_control_compatibility_meta(
            enabled_flags={
                "atr_stop": bool(
                    str(getattr(inp, "atr_stop_mode", "none") or "none").strip().lower()
                    != "none"
                ...
                "r_take_profit": bool(getattr(inp, "r_take_profit_enabled", False)),
                "r_profit_scaleout": bool(
                    getattr(inp, "r_profit_scaleout_enabled", False)
                ),
                "bias_v_take_profit": bool(
                    getattr(inp, "bias_v_take_profit_enabled", False)
                ),
                "ma_trailing_stop": bool(
                    getattr(inp, "ma_trailing_stop_enabled", False)
                ),
                "vol_regime_risk_mgmt_enabled": bool(
                    getattr(inp, "vol_regime_risk_mgmt_enabled", False)
...
def _run_single_backtesting(
    ...
    *,
    ...
    exec_comp = _build_execution_day_components(
        exec_price=ep,
        open_none=open_none,
    ...
    ret_exec_open_day = exec_comp["ret_exec_open_day"].astype(float)
    ret_exec_close_day = exec_comp["ret_exec_close_day"].astype(float)
    ret_overnight_close_day = exec_comp["ret_overnight_close_day"].astype(float)
    ret_intraday_close_day = exec_comp["ret_intraday_close_day"].astype(float)
    ca_m = exec_comp["ca_mask"]
    ret_exec = exec_comp["ret_exec_day"].astype(float)
    ret_exec_raw = ret_exec.copy().astype(float)
    px_exec_slip = exec_comp["px_exec_slip"].astype(float)
    ...
    risk_control_compatibility_meta = _enforce_risk_control_compatibility(
        {
            "atr_stop": bool(str(atr_mode) != "none"),
    ...
    ma_trailing_stop_stats: dict[str, Any] = {
        "enabled": False,
        "trigger_count": 0,
        "trigger_events": [],
        "trace_last_rows": [],
    ...
    if not simple_backtesting_mode:
        ...
        w_ret = w.copy().astype(float)
        ret_exec_day = ret_exec_open_day.reindex(w.index).fillna(0.0).astype(float)
        ...
        ma_override_ret = ma_over_post.reindex(w.index).fillna(0.0).astype(float)
        if ep == "close":
        ...
        ret_exec = ret_exec_day.astype(float)
        stats = {
    ...
    ret_hfq_cc = _ratio_simple_return(
        bt_df["HfqClose"].astype(float),
        bt_df["HfqClose"].astype(float).shift(1),
    ).astype(float)
    ...
    ret_bh_close = ret_hfq_cc.astype(float).reindex(ret_hfq_cc.index).fillna(0.0)
    if ep == "open":
        bh_ret = ret_bh_open.astype(float)
    elif ep == "close":
        bh_ret = ret_bh_close.astype(float)
    else:
        bh_ret = (0.5 * (ret_bh_open + ret_bh_close)).astype(float)
    ...
    return {
        ...
        "effective_weight": pos_eff.reindex(nav.index).fillna(0.0).astype(float),
        "return_weight": w_ret.reindex(nav.index).fillna(0.0).astype(float),
        ...
        "ret_exec_open_day": ret_exec_open_day.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
        "ret_exec_close_day": ret_exec_close_day.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
        "ret_overnight_close_day": ret_overnight_close_day.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
        "ret_intraday_close_day": ret_intraday_close_day.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
        "atr_override_ret": atr_override_ret.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
        "bias_override_ret": bias_override_ret.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
        ...
        .fillna(0.0)
        .astype(float),
        "ma_trailing_stop_override_ret": ma_override_ret.reindex(nav.index)
        .fillna(0.0)
        .astype(float),
...
def compute_trend_backtest_bt(db: Session, inp: Any) -> dict[str, Any]:
    ...
    w_eff = (
        single.get("effective_weight", single["desired_pos"].shift(1).fillna(0.0))
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
        .clip(lower=0.0)
    ...
    w_ret = (
        single.get("return_weight", w_eff)
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
        .clip(lower=0.0)
    ...
    ret_overnight_close_day = (
        single.get(
            "ret_overnight_close_day",
            _ratio_simple_return(sig_open, sig_close.shift(1)),
        )
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
    ...
    ret_intraday_close_day = (
        single.get("ret_intraday_close_day", _ratio_simple_return(sig_close, sig_open))
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
    )
    ...
    atr_over = (
        single.get("atr_override_ret", pd.Series(0.0, index=nav.index, dtype=float))
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
    )
    bv_over = (
        single.get("bias_override_ret", pd.Series(0.0, index=nav.index, dtype=float))
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
    )
    rtp_over = (
        single.get("rtp_override_ret", pd.Series(0.0, index=nav.index, dtype=float))
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
    )
    ...
    ma_over = (
        single.get(
            ...
            pd.Series(0.0, index=nav.index, dtype=float),
        )
        .astype(float)
        .reindex(nav.index)
        .fillna(0.0)
    ...
    if has_initial_account_amount:
        ...
        if not w_real.empty and code in w_real.columns:
            ...
            ma_over = (ma_over * risk_scale).astype(float)
            turnover_one_way = (w_eff - w_eff.shift(1).fillna(0.0)).abs() / 2.0
    ...
    nav = _as_nav(strat_ret)
    bench_ret = bench_ret.reindex(nav.index).astype(float).fillna(0.0)
    bh_nav = _as_nav(bench_ret)
    excess_nav = (nav / bh_nav.replace(0.0, np.nan)).fillna(1.0)
    excess_ret = _tsmom_rocp(excess_nav, 1).fillna(0.0).astype(float)
    active_ret = (
        strat_ret.reindex(nav.index).astype(float)
    ...
    if not quick_mode:
        if ep == "open":
            decomp_overnight = pd.Series(0.0, index=nav.index, dtype=float)
            decomp_intraday = (w_ret * ret_exec_open_day).astype(float)
            decomp_interaction = pd.Series(0.0, index=nav.index, dtype=float)
        elif ep == "close":
            decomp_overnight = (w_ret * ret_overnight_close_day).astype(float)
            decomp_intraday = (w_ret * ret_intraday_close_day).astype(float)
            decomp_interaction = (
                w_ret * ret_overnight_close_day * ret_intraday_close_day
            ).astype(float)
        else:
            decomp_overnight = (0.5 * w_ret * ret_overnight_close_day).astype(float)
            decomp_intraday = (
                w_ret * (0.5 * ret_exec_open_day + 0.5 * ret_intraday_close_day)
            ).astype(float)
            decomp_interaction = (
                0.5 * w_ret * ret_overnight_close_day * ret_intraday_close_day
            ).astype(float)
        decomp_cash_management = cash_management_ret.reindex(nav.index).astype(float)
        decomp_risk = (atr_over + bv_over + rtp_over + rps_over + ma_over).astype(float)
        ...
        return_decomposition = {
            ...
            "series": {
                ...
                "cash_management": decomp_cash_management.tolist(),
                ...
                "ma_trailing_stop_override": ma_over.tolist(),
            ...
            "summary": {
                ...
                "ann_cash_management": float(
                    decomp_cash_management.iloc[1:].mean() * 252.0
                )
                ...
                "ann_ma_trailing_stop_override": float(ma_over.iloc[1:].mean() * 252.0)
                if len(ma_over) > 1
                else 0.0,
    ...
    asset_ret_for_attribution = (
        ret_exec_s.to_frame(code).reindex(nav.index).astype(float).fillna(0.0)
    )
    weight_for_attribution = (
        w_eff.to_frame(code).reindex(nav.index).astype(float).fillna(0.0)
    )
    ...
    asset_ret_for_attribution[RISK_EXIT_PROXY_CODE] = (
        (atr_over + bv_over + rtp_over + rps_over + ma_over)
        .reindex(nav.index)
        .astype(float)
        .fillna(0.0)
    )
    ...
    overall_stats = {
        ...
        **dict(trade_extreme_stats.get("overall") or {}),
        ...
        "ma_trailing_stop_trigger_count": int(
            (sem_dbg.get("ma_trailing_stop") or {}).get("trigger_count", 0)
        ),
    ...
    out = {
        ...
        "risk_controls": {
            ...
            "ma_trailing_stop": dict(sem_dbg.get("ma_trailing_stop") or {}),
            "addons": {
                "r_profit_scaleout_breakeven_stop": {
                    ...
                    **dict(
                        ((sem_dbg.get("r_profit_scaleout") or {}).get("breakeven_stop"))
                        or {}
                    ),
                },
                "bias_v_take_profit_breakeven_stop": {
                    ...
                    **dict(
                        (
                            (sem_dbg.get("bias_v_take_profit") or {}).get(
                                "breakeven_stop"
                            )
                        )
                        or {}
        ...
        "next_plan": {
            ...
            "trace": {
                ...
                "r_profit_scaleout": {
                    ...
                    "breakeven_stop": dict(
                        (rps_stats or {}).get("breakeven_stop") or {}
                    ),
                ...
                "bias_v_take_profit": {
                    **dict(bv_stats or {}),
                    ...
                    "breakeven_stop": dict(
                        (bv_stats or {}).get("breakeven_stop") or {}
                    ),
                },
                "er_exit_filter": {
                    "enabled": bool(getattr(inp, "er_exit_filter", False)),
...
def compute_trend_portfolio_backtest_bt(db: Session, inp: Any) -> dict[str, Any]:
    ...
    risk_control_compatibility_meta = _enforce_risk_control_compatibility(
        {
            ...
            "atr_stop": str(getattr(inp, "atr_stop_mode", "none") or "none")
            .strip()
            .lower()
            != "none",
            "r_take_profit": bool(getattr(inp, "r_take_profit_enabled", False)),
    ...
    ret_exec_open_day_map: dict[str, pd.Series] = {}
    ret_exec_close_day_map: dict[str, pd.Series] = {}
    ret_overnight_close_day_map: dict[str, pd.Series] = {}
    ...
    for c in codes:
        ...
        single_inp = SimpleNamespace(
            ...
            ma_trailing_stop_enabled=getattr(inp, "ma_trailing_stop_enabled", False),
            ma_trailing_stop_ma_type=getattr(inp, "ma_trailing_stop_ma_type", "sma"),
            ma_trailing_stop_execution_mode=getattr(
                inp, "ma_trailing_stop_execution_mode", "intraday"
            ),
            ...
            ma_trailing_stop_reduce_window=getattr(
                inp, "ma_trailing_stop_reduce_window", 10
            ),
            ma_trailing_stop_exit_window=getattr(
                inp, "ma_trailing_stop_exit_window", 20
            ),
            ma_trailing_stop_reduce_fraction=getattr(
                inp, "ma_trailing_stop_reduce_fraction", 0.33
            ),
            ...
            vol_ratio_contract_threshold=inp.vol_ratio_contract_threshold,
            vol_ratio_normal_threshold=inp.vol_ratio_normal_threshold,
            vol_ratio_extreme_threshold=getattr(
                inp, "vol_ratio_extreme_threshold", 2.20
            ),
        ...
        one = {
            **one,
            ...
            "effective_weight": one.get("effective_weight", one["desired_pos"])
            .reindex(trim_ix)
            .fillna(0.0)
            .astype(float),
            "return_weight": one.get(
                "return_weight", one.get("effective_weight", one["desired_pos"])
            )
            .reindex(trim_ix)
            .fillna(0.0)
            .astype(float),
            "ret_exec": one["ret_exec"].reindex(trim_ix).fillna(0.0).astype(float),
            "ret_exec_open_day": one.get("ret_exec_open_day", one["ret_exec"])
            .reindex(trim_ix)
            .fillna(0.0)
            .astype(float),
            "ret_exec_close_day": one.get("ret_exec_close_day", one["ret_exec"])
            .reindex(trim_ix)
            .fillna(0.0)
            .astype(float),
            "ret_overnight_close_day": one.get(
                "ret_overnight_close_day", pd.Series(0.0, index=one["ret_exec"].index)
            )
            .reindex(trim_ix)
            .fillna(0.0)
            .astype(float),
            "ret_intraday_close_day": one.get("ret_intraday_close_day", one["ret_exec"])
            .reindex(trim_ix)
            .fillna(0.0)
            .astype(float),
        ...
        ret_exec_open_day_map[c] = one["ret_exec_open_day"].astype(float)
        ret_exec_close_day_map[c] = one["ret_exec_close_day"].astype(float)
        ...
        for d in wdf.index:
            ...
            def _set_new_risk_budget_entry(key: str, base_target: float) -> None:
                ...
                if bool(vol_regime_risk_mgmt_enabled):
                    ...
                    rb_state_by_code[key] = _vol_regime_state_from_ratio(
                        ratio,
    ...
    ret_exec_close_day_df = (
        pd.DataFrame(ret_exec_close_day_map)
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_overnight_close_day_df = (
        pd.DataFrame(ret_overnight_close_day_map)
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_intraday_close_day_df = (
        pd.DataFrame(ret_intraday_close_day_map)
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ...
    idx = wdf.index
    close_none_df = (
        load_close_prices(db, codes=ccodes, start=inp.start, end=inp.end, adjust="none")
        .reindex(index=idx, columns=ccodes)
        .ffill()
        .astype(float)
    )
    close_hfq_df = (
        load_close_prices(db, codes=ccodes, start=inp.start, end=inp.end, adjust="hfq")
        .reindex(index=idx, columns=ccodes)
        .ffill()
        .astype(float)
    )
    ...
    exec_comp_df = _build_execution_day_components(
        exec_price=ep_port,
        open_none=open_none_exec,
    ...
    ret_exec_open_none_day_df = (
        _ratio_simple_return(close_none_exec, open_none_raw)
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_open_hfq_day_df = (
        _ratio_simple_return(close_hfq_exec, open_hfq_raw)
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_df = (
        exec_comp_df["ret_exec_day"]
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_exec_open_day_df = ret_exec_open_none_day_df.where(
        ~cm_exec, ret_exec_open_hfq_day_df
    ).astype(float)
    ret_exec_close_day_df = (
        exec_comp_df["ret_exec_close_day"]
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_overnight_close_none_day_df = (
        _ratio_simple_return(open_none_raw, close_none_exec.shift(1))
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_overnight_close_hfq_day_df = (
        _ratio_simple_return(open_hfq_raw, close_hfq_exec.shift(1))
        .reindex(index=wdf.index, columns=wdf.columns)
        .fillna(0.0)
        .astype(float)
    )
    ret_overnight_close_day_df = ret_overnight_close_none_day_df.where(
        ~cm_exec, ret_overnight_close_hfq_day_df
    ).astype(float)
    ret_intraday_close_day_df = ret_exec_open_day_df.astype(float)
    px_exec_slip_df = (
        exec_comp_df["px_exec_slip"]
        .reindex(index=wdf.index, columns=wdf.columns)
        .ffill()
        .astype(float)
    )
    ...
    rps_breakeven_by_asset = {
        str(c): dict((rps_by_asset.get(str(c), {}) or {}).get("breakeven_stop") or {})
        for c in wdf.columns
    }
    bias_v_tp_breakeven_by_asset = {
        str(c): dict(
            (bias_v_tp_by_asset.get(str(c), {}) or {}).get("breakeven_stop") or {}
        )
        for c in wdf.columns
    }
    ma_trailing_stop_by_asset = {
        str(c): dict(
            (semantic_debug_by_code.get(str(c), {}) or {}).get("ma_trailing_stop") or {}
        )
        for c in wdf.columns
    }
        ...
        *,
        ...
        w_tmp, over_by_overlay = _apply_intraday_or_arbitration_portfolio(
            ...
            event_sets_by_asset={
                ...
                "r_profit_scaleout": rps_by_asset,
    ...
    atr_stop_override_ret = atr_over_post.reindex(w_eff.index).fillna(0.0).astype(float)
    bias_v_take_profit_override_ret = (
        bias_over_post.reindex(w_eff.index).fillna(0.0).astype(float)
    )
    ...
    decomp_risk = (
        ...
        + ma_trailing_stop_override_ret.reindex(w_eff.index).fillna(0.0).astype(float)
    ).astype(float)
    ...
    if not bool(getattr(inp, "quick_mode", False)):
        if ep_port == "open":
            comp_overnight = pd.Series(0.0, index=w_eff.index, dtype=float)
            comp_intraday = (w_ret * ret_exec_open_day_df).sum(axis=1).astype(float)
            comp_interaction = pd.Series(0.0, index=w_eff.index, dtype=float)
        elif ep_port == "close":
            comp_overnight = (
                (w_ret * ret_overnight_close_day_df).sum(axis=1).astype(float)
            )
            comp_intraday = (
                (w_ret * ret_intraday_close_day_df).sum(axis=1).astype(float)
            )
            comp_interaction = (
                (w_ret * ret_overnight_close_day_df * ret_intraday_close_day_df)
                .sum(axis=1)
                .astype(float)
            )
        else:
            comp_overnight = (
                (0.5 * w_ret * ret_overnight_close_day_df).sum(axis=1).astype(float)
            )
            comp_intraday = (
                (w_ret * (0.5 * ret_exec_open_day_df + 0.5 * ret_intraday_close_day_df))
                .sum(axis=1)
                .astype(float)
            )
            comp_interaction = (
                (0.5 * w_ret * ret_overnight_close_day_df * ret_intraday_close_day_df)
                .sum(axis=1)
                .astype(float)
            )
        decomp_gross = (
        ...
        return_decomposition = {
            ...
            "series": {
                ...
                .astype(float)
                .tolist(),
                "ma_trailing_stop_override": ma_trailing_stop_override_ret.reindex(
                    w_eff.index
                )
                .fillna(0.0)
                .astype(float)
            ...
            "summary": {
                ...
                "ann_ma_trailing_stop_override": float(
                    ma_trailing_stop_override_ret.iloc[1:].mean() * 252.0
                )
                if len(ma_trailing_stop_override_ret) > 1
                else 0.0,
                "ann_risk_exit_override": float(decomp_risk.iloc[1:].mean() * 252.0)
                if len(decomp_risk) > 1
                else 0.0,
    ...
    ma_trailing_stop_trigger_total = sum(
        int(
            (semantic_debug_by_code.get(c, {}).get("ma_trailing_stop") or {}).get(
                "trigger_count", 0
            )
        )
        for c in semantic_debug_by_code
    ...
    ret_exec_for_runs = ret_exec_for_runs.fillna(0.0)
    ...
    overall_stats = {
        ...
        **dict(trade_extreme_stats.get("overall") or {}),
        ...
        "ma_trailing_stop_trigger_count": int(ma_trailing_stop_trigger_total),
    ...
    for c in wdf.columns:
        ...
        by_code_stats[str(c)] = {
            ...
            **dict((trade_extreme_stats.get("by_code") or {}).get(str(c)) or {}),
            ...
            "ma_trailing_stop_trigger_count": int(
                (sem.get("ma_trailing_stop") or {}).get("trigger_count", 0)
            ),
    ...
    rps_breakeven_trigger_dates = sorted(
        {
            str(d)
            for v in rps_breakeven_by_asset.values()
            for d in list((v or {}).get("trigger_dates") or [])
            if str(d).strip()
        }
    )
    bias_v_tp_breakeven_trigger_dates = sorted(
        {
            str(d)
    ...
    ma_trailing_stop_trigger_dates = sorted(
        {
            str(d)
        ...
        "risk_controls": {
            ...
            "addons": {
                "r_profit_scaleout_breakeven_stop": {
                    ...
                    "trigger_count": int(
                        sum(
                            int((v or {}).get("trigger_count", 0) or 0)
                            for v in rps_breakeven_by_asset.values()
                        )
                    ),
                    "trigger_days": int(len(rps_breakeven_trigger_dates)),
                    "first_trigger_date": (
                        rps_breakeven_trigger_dates[0]
                        if rps_breakeven_trigger_dates
                        else None
                    ),
                    "last_trigger_date": (
                        rps_breakeven_trigger_dates[-1]
                        if rps_breakeven_trigger_dates
                        else None
                    ),
                    "trigger_dates": rps_breakeven_trigger_dates[:200],
                    "by_asset": rps_breakeven_by_asset,
                },
                "bias_v_take_profit_breakeven_stop": {
                    "enabled": bool(
                        getattr(inp, "bias_v_take_profit_breakeven_stop_enabled", True)
                    ...
                    "trigger_days": int(len(bias_v_tp_breakeven_trigger_dates)),
                    "first_trigger_date": (
                        bias_v_tp_breakeven_trigger_dates[0]
                        if bias_v_tp_breakeven_trigger_dates
                        else None
                    ),
                    "last_trigger_date": (
                        bias_v_tp_breakeven_trigger_dates[-1]
                        if bias_v_tp_breakeven_trigger_dates
                        else None
                    ),
                    "trigger_dates": bias_v_tp_breakeven_trigger_dates[:200],
                    "by_asset": bias_v_tp_breakeven_by_asset,
                },
            },
            "ma_trailing_stop": {
                "enabled": bool(getattr(inp, "ma_trailing_stop_enabled", False)),
                "ma_type": str(
                    getattr(inp, "ma_trailing_stop_ma_type", "sma") or "sma"
                ),
                "execution_mode": str(
                ...
                "trigger_count": int(ma_trailing_stop_trigger_total),
                "trigger_days": int(len(ma_trailing_stop_trigger_dates)),
                "first_trigger_date": (
                    ma_trailing_stop_trigger_dates[0]
                    if ma_trailing_stop_trigger_dates
                    else None
                ),
                "last_trigger_date": (
                    ma_trailing_stop_trigger_dates[-1]
                    if ma_trailing_stop_trigger_dates
                    else None
                ),
                "trigger_dates": ma_trailing_stop_trigger_dates[:200],
