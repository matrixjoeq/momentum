...
def backtest_tiered_exposure_by_level(
    ...
    *,
    ...
    th = sorted(th)
    exps = [
        float(x) for x in (exposures or []) if x is not None and np.isfinite(float(x))
    ]
    ...
    r_etf = np.expm1(
        pd.to_numeric(df["etf_ret"], errors="coerce").to_numpy(dtype=float)
    )
    ...
    r_etf_ok = r_etf[ok2]
    nav_strat = (
        np.cumprod(1.0 + r_strat_ok) if r_strat_ok.size else np.array([], dtype=float)
    )
...
def backtest_tiered_exposure_by_level_rolling_quantiles(
    ...
    *,
    ...
    """
    qs = [
        float(q) for q in (quantiles or []) if q is not None and np.isfinite(float(q))
    ]
    ...
    thr_df = pd.DataFrame(
        {
            f"q{int(q * 100)}": lvl.rolling(w, min_periods=mp).quantile(q).shift(1)
            for q in qs
        }
    )
    ...
    for i in range(len(df)):
        ...
        j = (
            int(np.searchsorted(th_row_sorted, lv[i], side="left"))
            if th_row_sorted.size
            else 0
        )
        j = int(max(0, min(len(exps) - 1, j)))
        exp[i] = float(exps[j])
...
def backtest_tiered_exposure_by_level_expanding_quantiles(
    ...
    *,
    ...
    thr_df = pd.DataFrame(
        {
            f"q{int(q * 100)}": lvl.expanding(min_periods=mp).quantile(q).shift(1)
            for q in qs
        }
    )
