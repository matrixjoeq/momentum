...
def hfq_close_daily_equal_weight_returns(
    close_hfq: pd.DataFrame, *, dynamic_universe: bool
) -> pd.Series:
    """
    ...
    *,
    ...
    if reb == "daily":
        for i, t in enumerate(r.index):
            ...
            if len(hist) < win:
                ...
                w_df.loc[t, :] = (
                    _inv_vol_weights(hist.std(ddof=1)).reindex(cols).fillna(0.0)
                )
    ...
    for i, lab in enumerate(labels):
        if prev_label is None or lab != prev_label:
            ...
            if len(hist) >= win:
                ww = (
                    _inv_vol_weights(hist.std(ddof=1))
                    .reindex(cols)
                    .fillna(0.0)
    ...
    *,
    ...
    if reb == "daily":
        for i, t in enumerate(r.index):
            ...
            if len(hist) < win:
                ...
                w_df.loc[t, :] = (
                    _erc_weight_series_from_hist(hist, cols).reindex(cols).fillna(0.0)
                )
        port_ret = (w_df * r).sum(axis=1)
        nav = (1.0 + port_ret).cumprod()
        if len(nav) > 0:
    ...
    for i, lab in enumerate(labels):
        if prev_label is None or lab != prev_label:
            ...
            if len(hist) >= win:
                ww = (
                    _erc_weight_series_from_hist(hist, cols)
                    .reindex(cols)
                    .fillna(0.0)
                    .to_numpy(dtype=float)
                )
...
def _compute_equal_weight_nav_and_weights_dynamic(
    ...
    *,
    ...
    if daily_ret.empty:
        idx = daily_ret.index
        return (
            pd.Series(dtype=float, name="EW"),
            pd.DataFrame(index=idx),
            pd.Series(dtype=float, name="active_count"),
        )
    ...
    labels = (
        _rebalance_labels(idx, reb, weekly_anchor=weekly_anchor)
        if reb != "none"
        else None
    )
    prev_label = None
    ...
    for i, d in enumerate(idx):
        ...
        if len(active) >= int(min_active):
            gross = (
                (1.0 + rr.loc[active].astype(float))
...
def _annualized_return(
    nav: pd.Series, ann_factor: int = TRADING_DAYS_PER_YEAR
) -> float:
    if nav.empty:
        return float("nan")
    n = len(nav) - 1
...
def _annualized_vol(
    daily_ret: pd.Series, ann_factor: int = TRADING_DAYS_PER_YEAR
) -> float:
    if daily_ret.empty:
        return float("nan")
...
def _sharpe(
    daily_ret: pd.Series, rf: float = 0.0, ann_factor: int = TRADING_DAYS_PER_YEAR
) -> float:
    if daily_ret.empty:
        return float("nan")
...
def _information_ratio(
    active_daily: pd.Series, ann_factor: int = TRADING_DAYS_PER_YEAR
) -> float:
...
def _rolling_max_drawdown(nav: pd.Series, window: int) -> pd.Series:
    ...
    return nav.rolling(window=window, min_periods=window).apply(
        lambda x: f(x.to_numpy()), raw=False
    )
...
def _quantiles_from_samples(
    samples: np.ndarray, quantiles: list[float] | None = None
) -> dict[str, float]:
    """
    ...
    for code in codes:
        ...
        if not daily_vol.empty:
            ...
            if not lv.empty:
                ...
                code_result["daily_log_volatility"] = {
                    ...
                    "std": float(np.std(lv_vals, ddof=1))
                    if len(lv_vals) >= 2
                    else float("nan"),
                ...
                lv_dev = (
                    lv_log - lv_log.rolling(window=20, min_periods=5).mean()
                ).dropna()
                if not lv_dev.empty:
                    ...
                    code_result["daily_log_vol_dev"] = {
                        ...
                        "current_date": pd.to_datetime(lv_dev.index[-1])
                        .date()
                        .isoformat(),
                    }
        ...
        if not weekly_ret.empty:
            ...
            if not wlr.empty:
                ...
                if not wlr_dev.empty:
                    ...
                    code_result["weekly_log_return_dev"] = {
                        ...
                        "current_date": pd.to_datetime(wlr_dev.index[-1])
                        .date()
                        .isoformat(),
                    }
            # Weekly volatility (rolling 4-week window)
            weekly_vol = (
                weekly_ret.rolling(window=4, min_periods=2).std(ddof=1).dropna()
            )
            if not weekly_vol.empty:
                ...
                code_result["weekly_volatility"] = {
                    ...
                    "current_date": pd.to_datetime(weekly_vol.index[-1])
                    .date()
                    .isoformat(),
                }
                # Log-vol deviation: log(vol) - MA20(log(vol))
                ...
                if not wlv.empty:
                    wlv_log = np.log(wlv)
                    wlv_dev = (
                        wlv_log - wlv_log.rolling(window=20, min_periods=5).mean()
                    ).dropna()
                    if not wlv_dev.empty:
                        ...
                        code_result["weekly_log_vol_dev"] = {
                            ...
                            "current_date": pd.to_datetime(wlv_dev.index[-1])
                            .date()
                            .isoformat(),
                        }
                ...
                if not wv.empty:
                    ...
                    code_result["weekly_log_volatility"] = {
                        ...
                        "std": float(np.std(wv_vals, ddof=1))
                        if len(wv_vals) >= 2
                        else float("nan"),
        ...
        if not monthly_ret.empty:
            ...
            code_result["monthly_return"] = {
                ...
                "current_date": pd.to_datetime(monthly_ret.index[-1])
                .date()
                .isoformat(),
            }
            ...
            if not mlr.empty:
                ...
                if not mlr_dev.empty:
                    ...
                    code_result["monthly_log_return_dev"] = {
                        ...
                        "current_date": pd.to_datetime(mlr_dev.index[-1])
                        .date()
                        .isoformat(),
                    }
            # Monthly volatility (rolling 3-month window)
            monthly_vol = (
                monthly_ret.rolling(window=3, min_periods=2).std(ddof=1).dropna()
            )
            if not monthly_vol.empty:
                ...
                code_result["monthly_volatility"] = {
                    ...
                    "current_date": pd.to_datetime(monthly_vol.index[-1])
                    .date()
                    .isoformat(),
                }
                ...
                if not mlv.empty:
                    mlv_log = np.log(mlv)
                    mlv_dev = (
                        mlv_log - mlv_log.rolling(window=20, min_periods=5).mean()
                    ).dropna()
                    if not mlv_dev.empty:
                        ...
                        code_result["monthly_log_vol_dev"] = {
                            ...
                            "current_date": pd.to_datetime(mlv_dev.index[-1])
                            .date()
                            .isoformat(),
                        }
                ...
                if not mv.empty:
                    ...
                    code_result["monthly_log_volatility"] = {
                        ...
                        "std": float(np.std(mv_vals, ddof=1))
                        if len(mv_vals) >= 2
                        else float("nan"),
        ...
        if not quarterly_ret.empty:
            ...
            code_result["quarterly_return"] = {
                ...
                "current_date": pd.to_datetime(quarterly_ret.index[-1])
                .date()
                .isoformat(),
            }
            ...
            if not qlr.empty:
                ...
                if not qlr_dev.empty:
                    ...
                    code_result["quarterly_log_return_dev"] = {
                        ...
                        "current_date": pd.to_datetime(qlr_dev.index[-1])
                        .date()
                        .isoformat(),
                    }
            # Quarterly volatility (rolling 2-quarter window)
            quarterly_vol = (
                quarterly_ret.rolling(window=2, min_periods=2).std(ddof=1).dropna()
            )
            if not quarterly_vol.empty:
                ...
                code_result["quarterly_volatility"] = {
                    ...
                    "current_date": pd.to_datetime(quarterly_vol.index[-1])
                    .date()
                    .isoformat(),
                }
                ...
                if not qlv.empty:
                    qlv_log = np.log(qlv)
                    qlv_dev = (
                        qlv_log - qlv_log.rolling(window=20, min_periods=5).mean()
                    ).dropna()
                    if not qlv_dev.empty:
                        ...
                        code_result["quarterly_log_vol_dev"] = {
                            ...
                            "current_date": pd.to_datetime(qlv_dev.index[-1])
                            .date()
                            .isoformat(),
                        }
                ...
                if not qv.empty:
                    ...
                    code_result["quarterly_log_volatility"] = {
                        ...
                        "std": float(np.std(qv_vals, ddof=1))
                        if len(qv_vals) >= 2
                        else float("nan"),
        ...
        if not yearly_ret.empty:
            ...
            if len(yearly_ret) >= 2:
                ...
                code_result["yearly_volatility"] = {
                    ...
                    "current": float(yearly_ret.iloc[-1]),
                    "current_date": pd.to_datetime(yearly_ret.index[-1])
                    .date()
                    .isoformat(),
                }
        ...
        if close_df is not None and code in close_df.columns:
            ...
            if not px.empty:
                ...
                def _log_dev(s: pd.Series, *, n: int) -> pd.Series:
                    ...
                    lp = np.log(s2)
                    ema = s2.ewm(
                        span=int(n), adjust=False, min_periods=max(3, min(20, int(n)))
                    ).mean()
                ...
                def _add_bias_v(
                    ...
                    low_v = (
                        pd.to_numeric(low_s, errors="coerce")
                        .astype(float)
                        .reindex(c.index)
                        .combine_first(c)
                    )
                    ...
                    code_result[f"{kind}_bias_v"] = {
                        ...
                        "current_date": pd.to_datetime(bias_v.index[-1])
                        .date()
                        .isoformat(),
                    }
                ...
                hi_raw = (
                    pd.to_numeric(high_df[code], errors="coerce").astype(float)
                    if (high_df is not None and code in high_df.columns)
                    else px.copy()
                )
                lo_raw = (
                    pd.to_numeric(low_df[code], errors="coerce").astype(float)
                ...
                def _add_macd_v(
                    ...
                    code_result[f"{kind}_macd_v_dif"] = {
                        ...
                        "std": float(np.std(dif_vals, ddof=1))
                        if len(dif_vals) >= 2
                        else float("nan"),
                        "count": int(len(dif_vals)),
                        "current": float(dif.iloc[-1]),
                        "current_date": pd.to_datetime(dif.index[-1])
                        .date()
                        .isoformat(),
                    }
                    code_result[f"{kind}_macd_v_dea"] = {
                        ...
                        "std": float(np.std(dea_vals, ddof=1))
                        if len(dea_vals) >= 2
                        else float("nan"),
                        "count": int(len(dea_vals)),
                        "current": float(dea.iloc[-1]),
                        "current_date": pd.to_datetime(dea.index[-1])
                        .date()
                        .isoformat(),
                    }
        ...
        if act is not None and act_src is not None:
            ...
            lv = (
                np.log(act.replace(0.0, np.nan))
                .replace([np.inf, -np.inf], np.nan)
                .dropna()
            )
            if not lv.empty:
                dev = (
                    (lv - lv.rolling(window=20, min_periods=5).mean())
                    .replace([np.inf, -np.inf], np.nan)
                    .dropna()
                )
                for kind, ser in [
                    ...
                    code_result[f"{kind}_vol_dev"] = {
                        ...
                        "source": act_src,
                        "current": float(ser.iloc[-1]),
                        "current_date": pd.to_datetime(ser.index[-1])
                        .date()
                        .isoformat(),
                    }
...
def _fft_roll_timeseries_from_returns(
    ...
    *,
    ...
    for wi in win_clean:
        if n < wi:
            out["series"][f"last_{wi}"] = {
                "ok": False,
                "reason": f"not enough samples (<{wi})",
                "dates": [],
                "spectral_entropy": [],
                "high_band_energy": [],
            }
...
def load_volume_amount(
    ...
    *,
    ...
    df["date"] = pd.to_datetime(df["date"])
    volume = df.pivot_table(
        index="date", columns="code", values="volume", aggfunc="last"
    ).sort_index()
    amount = df.pivot_table(
        index="date", columns="code", values="amount", aggfunc="last"
    ).sort_index()
...
def load_ohlc_prices(
    ...
    *,
    ...
    for k in ["open", "high", "low", "close"]:
        out[k] = df.pivot_table(
            index="date", columns="code", values=k, aggfunc="last"
        ).sort_index()
    return out
...
def _compute_equal_weight_nav_and_weights(
    ...
    *,
    ...
    if len(nav_s) > 0:
        nav_s.iloc[0] = 1.0
    w_df = pd.DataFrame(
        np.vstack(w_out), index=daily_ret.index, columns=cols, dtype=float
    )
    return nav_s, w_df
    ...
    *,
        ...
        try:
            if np.isfinite(float(total_return)) and float(total_return) > -1.0:
                ann_total_return = float(
                    np.power(1.0 + float(total_return), 252.0 / float(max(1, n_obs)))
                    - 1.0
                )
        ...
        return_rows.append(
            {
                "code": str(c),
                "return_contribution": (
                    None if pd.isna(contrib.get(c)) else float(contrib.get(c))
                ),
                ...
                "return_share": (
                    None if pd.isna(share.get(c)) else float(share.get(c))
                ),
            }
        )
    ...
    if port_var == 0.0 or np.isnan(port_var):
        ...
        if ann_applicable and np.isfinite(port_var) and port_var > 0.0:
            rc_vol = rc / np.sqrt(
                port_var
            )  # contribution to daily vol, sums to daily vol
...
def compute_baseline(db: Session, inp: BaselineInputs) -> dict[str, Any]:
    ...
    first_valid = {
        c: close[c].first_valid_index() for c in codes_eff if c in close.columns
    }
    ...
    def _cash_portfolio(
        index: pd.DatetimeIndex, columns: list[str], name: str
    ) -> tuple[pd.Series, pd.DataFrame]:
    ...
    def _single_asset_portfolio(
        ret_df: pd.DataFrame, code: str, name: str
    ) -> tuple[pd.Series, pd.DataFrame]:
    ...
    if dynamic_universe:
        if len(codes_eff) == 1:
            ...
            ew_nav, ew_w, ew_active_count = (
                _compute_equal_weight_nav_and_weights_dynamic(
        ...
        if len(codes_eff) <= 1:
            ...
            ew_nav, ew_w = _compute_equal_weight_nav_and_weights(
                ret_common[codes_eff], rebalance=inp.rebalance
            )
    ...
    ret_fwd = forward_align_returns(
        ret_common[codes_eff].fillna(0.0).astype(float)
    ).astype(float)
    if ep in {"open", "oc2"}:
        ohlc = load_ohlc_prices(
            db, codes=codes_eff, start=inp.start, end=inp.end, adjust=inp.adjust
        )
        ...
        if (
            not oo.empty
            and not cc.empty
            and set(codes_eff).issubset(oo.columns)
            and set(codes_eff).issubset(cc.columns)
        ):
            ...
            if reb == "none":
                ...
                exec_mask = (
                    ...
                    .reindex(ret_common.index)
                    .fillna(True)
                )
            if ep == "open":
                ret_fwd.loc[exec_mask, codes_eff] = same_day.loc[exec_mask, codes_eff]
            else:
                ret_fwd.loc[exec_mask, codes_eff] = (
                    0.5 * same_day.loc[exec_mask, codes_eff]
                    + 0.5 * ret_fwd.loc[exec_mask, codes_eff]
                ).astype(float)
    ...
    if reb_mode == "none":
        ...
        ew_ret = (
            (
                ew_w.reindex(index=ret_common.index, columns=codes_eff).fillna(0.0)
                * ret_fwd
            )
            .sum(axis=1)
            .astype(float)
    ...
    if dynamic_universe:
        ...
        if len(codes_eff) <= 1:
            ...
            rp_nav, rp_w = _compute_erc_nav_and_weights(
                ret_common[codes_eff], rebalance=inp.rebalance, window=rp_win
            )
            ivol_nav, ivol_w = _compute_inv_vol_nav_and_weights(
                ret_common[codes_eff], rebalance=inp.rebalance, window=rp_win
            )
    if reb_mode == "none":
        ...
        rp_ret = (
            (
                rp_w.reindex(index=ret_common.index, columns=codes_eff).fillna(0.0)
                * ret_fwd
            )
            .sum(axis=1)
            .astype(float)
        ...
        ivol_ret = (
            (
                ivol_w.reindex(index=ret_common.index, columns=codes_eff).fillna(0.0)
                * ret_fwd
            )
            .sum(axis=1)
            .astype(float)
    ...
    if mode == "CUSTOM" and float(cw.sum()) <= 0.0:
        raise ValueError(
            "holding_mode=CUSTOM requires at least one positive custom weight"
        )
    if float(cw.sum()) <= 0.0:
    ...
    if reb_mode == "none":
        ...
        custom_ret = (
            (
                custom_w.reindex(index=ret_common.index, columns=codes_eff).fillna(0.0)
                * ret_fwd
            )
            .sum(axis=1)
            .astype(float)
    ...
    bench_df = load_close_prices(
        db, codes=[bench_code], start=inp.start, end=inp.end, adjust="hfq"
    )
    ...
    if bench_df.empty or bench_code not in bench_df.columns:
        ...
        bench_ret = (
            hfq_close_buy_hold_returns(bench_s).reindex(ret_common.index).fillna(0.0)
        )
    ...
    def period_returns(series: pd.Series, freq: str) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {"period_end": r.index.date.astype(str), "return": r.values}
        )
    ...
    def _trade_freq_from_weights(weight_df: pd.DataFrame) -> dict[str, float]:
        ...
        avg_daily_trade_count = (
            float(complete_trade_count_equiv / len(daily_turnover))
            if len(daily_turnover)
            else 0.0
        )
    ...
    def _rolling_pack(nav_s: pd.Series) -> dict[str, dict[str, Any]]:
        ...
        for months in inp.rolling_months or []:
            ...
            rolling["drawdown"][f"{months}m"] = _rolling_drawdown(
                nav_s, window
            ).dropna()
    ...
    series = {
        c: nav_common[c].astype(float).tolist()
        for c in codes
        if c in nav_common.columns
    }
    ...
    nav_rsi = {
        ...
        "series": {
            "EW": {
                str(w): _rsi_wilder(ew_nav, window=int(w)).astype(float).tolist()
                for w in rsi_windows
            },
            "RP": {
                str(w): _rsi_wilder(rp_nav, window=int(w)).astype(float).tolist()
                for w in rsi_windows
            },
            "IVOL": {
                str(w): _rsi_wilder(ivol_nav, window=int(w)).astype(float).tolist()
                for w in rsi_windows
            },
            "CUSTOM": {
                str(w): _rsi_wilder(custom_nav, window=int(w)).astype(float).tolist()
                for w in rsi_windows
            },
            f"BENCH:{bench_code}": {
                str(w): _rsi_wilder(bench_nav, window=int(w)).astype(float).tolist()
                for w in rsi_windows
            },
        },
    }
    ...
    fft = _fft_analysis(
        close_common.ffill()[codes_eff], ew_nav=ew_nav, windows=inp.fft_windows
    )
    fft_roll = {
        "ok": False,
        "reason": "disabled",
        "windows": fft.get("windows", []),
    ...
    vol_m = (
        volume_common.copy()
        if (volume_common is not None and not volume_common.empty)
        else pd.DataFrame()
    )
    amt_m = (
        amount_common.copy()
        if (amount_common is not None and not amount_common.empty)
        else pd.DataFrame()
    )
    if not vol_m.empty:
    ...
    for code in codes_eff:
        if code not in close_m.columns:
            continue
        px = (
            pd.to_numeric(close_m[code], errors="coerce")
            .astype(float)
            .replace([np.inf, -np.inf], np.nan)
            .dropna()
        )
        ...
        if (not vol_m.empty) and (code in vol_m.columns):
            act = act.combine_first(
                pd.to_numeric(vol_m[code], errors="coerce").astype(float)
            )
        if (not amt_m.empty) and (code in amt_m.columns):
            act = act.combine_first(
                pd.to_numeric(amt_m[code], errors="coerce").astype(float)
            )
        act = act.replace([np.inf, -np.inf], np.nan)
        la = np.log(act.replace(0.0, np.nan)).replace([np.inf, -np.inf], np.nan)
        la_dev_daily = (la - la.rolling(window=20, min_periods=5).mean()).replace(
        ...
        for per, freq in freq_by_period.items():
            ...
            lr_dev_p = (
                (lr_p - lr_p.rolling(window=20, min_periods=5).mean())
                .replace([np.inf, -np.inf], np.nan)
                .dropna()
            )
            if lr_dev_p.empty:
            ...
            wv, mp = vol_win[per]
            vol_p = (
                ret_p.rolling(window=int(wv), min_periods=int(mp))
                .std(ddof=1)
                .replace([np.inf, -np.inf], np.nan)
                .dropna()
            )
            ...
            lv_dev_p = (
                (lv_p - lv_p.rolling(window=20, min_periods=5).mean())
                .replace([np.inf, -np.inf], np.nan)
                .dropna()
            )
            if lv_dev_p.empty:
            ...
            la_dev_p = (
                pd.to_numeric(la_dev_p, errors="coerce")
                .astype(float)
                .replace([np.inf, -np.inf], np.nan)
                .dropna()
            )
