...
def fetch_crud_oil_data(
    db, codes: List[str], start_date: dt.date, end_date: dt.date
) -> pd.DataFrame:
    """获取原油ETF的日度行情数据"""
...
def compute_multi_factor_score(
    ...
    multi_factor = (
        mom_weight * mom_norm + trend_weight * trend_norm + vol_weight * vol_norm
    )
    return multi_factor
...
def apply_trend_filter(
    ranks: pd.DataFrame, close: pd.DataFrame, sma_period: int = 50
) -> pd.DataFrame:
    """趋势过滤：只选择价格高于均线的标的"""
    sma = close.rolling(window=sma_period, min_periods=sma_period).mean()
    above_sma = close > sma
...
def backtest_strategy(
    ...
    if score_method == "multi_factor":
        ...
        scores = compute_factor_score(
            close, score_method, lookback_days, vol_lookback, sma_period
        )
...
def run_trend_filter_tests(close: pd.DataFrame) -> List[BacktestResult]:
    ...
    for score_method, lookback, top_k, rebalance in base_configs:
        for sma_period in sma_periods:
            logger.info(
                f"Testing trend filter: {score_method}_{lookback}d_sma{sma_period}"
            )
        ...
        for lookback in lookback_days_list:
            for top_k in top_k_list:
                for rebalance in rebalance_list:
                    count += 1
                    logger.info(
                        f"[{count}/{total}] Multi-factor: w=({mom_w},{trend_w},{vol_w})_{lookback}d_top{top_k}_{rebalance}"
                    )
...
def analyze_results(results: List[BacktestResult]) -> Dict[str, Any]:
    ...
    best_calmar = (
        df.sort_values("metrics.calmar_ratio", ascending=False).iloc[0]
        if len(df) > 0
        else None
    )
    ...
    if "params.lookback_days" in df.columns:
        lookback_analysis = df.groupby("params.lookback_days")[
...
def main():
    ...
    try:
        ...
        if best:
            ...
            logger.info(
                "  Return: %.1f%%", best.get("metrics.annualized_return", 0) * 100
            )
