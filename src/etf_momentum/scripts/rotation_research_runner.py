...
def fetch_price_data(
    db, codes: List[str], start_date: dt.date, end_date: dt.date, adjust: str = "qfq"
) -> pd.DataFrame:
    """获取价格数据"""
    ...
    logger.info(
        "Loaded price data: %d rows, %d codes, date range: %s to %s",
        len(pivot),
        len(pivot.columns),
        pivot.index.min().date(),
        pivot.index.max().date(),
    )
...
def get_date_range(
    db, codes: List[str], adjust: str = "qfq"
...
def compute_sharpe_momentum(
    close: pd.DataFrame, mom_lookback: int = 90, vol_lookback: int = 20
) -> pd.DataFrame:
...
def get_factor_score(
    close: pd.DataFrame, method: str, lookback: int, vol_lookback: int, sma_period: int
) -> pd.DataFrame:
    if method == "raw_mom":
        ...
        return compute_multi_factor(
            close, 0.4, 0.3, 0.3, lookback, vol_lookback, sma_period
        )
    else:
        return compute_raw_momentum(close, lookback)
...
def execute_rotation(
    ...
    portfolio_ret = (portfolio * daily_ret).sum(axis=1) - weight_change * (
        cost_bps / 10000
    ) / 2
...
def backtest_strategy(
    close: pd.DataFrame, config: RotationStrategyConfig, cost_bps: float = 3.0
) -> BacktestResult:
    """回测单个策略"""
...
def run_grid_search(
    close: pd.DataFrame, universe: UniverseConfig, cost_bps: float = 3.0
) -> List[BacktestResult]:
    """运行网格搜索"""
    results = []
        ...
        for lookback in lookback_days_list:
            for top_k in top_k_list:
                for rebalance in rebalance_list:
                    count += 1
                    logger.info(
                        f"[{count}/{total}] Testing: {score_method}_{lookback}d_top{top_k}_{rebalance}"
                    )
...
def run_parameter_search(
    ...
    for lookback in lookback_days_list:
        for top_k in top_k_list:
            count += 1
            logger.info(
                f"[{count}/{total}] Param search: lookback={lookback}d, top_k={top_k}"
            )
...
def analyze_results(results: List[BacktestResult]) -> Dict[str, Any]:
    ...
    best_return = (
        df.sort_values("metrics.annualized_return", ascending=False).iloc[0]
        if len(df) > 0
        else None
    )
    ...
    if "params.lookback_days" in df.columns:
        sensitivity["lookback"] = (
            df.groupby("params.lookback_days")["metrics.sharpe_ratio"].mean().to_dict()
        )
    if "params.top_k" in df.columns:
        sensitivity["top_k"] = (
            df.groupby("params.top_k")["metrics.sharpe_ratio"].mean().to_dict()
        )
...
def generate_report(
    analysis: Dict[str, Any], universe: UniverseConfig, output_dir: Path
):
    """生成研究报告"""
    report = f"""# 轮动策略研究报告
...
def main():
    ...
    parser.add_argument(
        "--universe", "-u", default="crude_oil", help="Universe name or JSON config"
    )
    parser.add_argument(
        "--codes", "-c", help="Comma-separated codes (overrides universe)"
    )
    ...
    parser.add_argument(
        "--quick", "-q", action="store_true", help="Quick parameter search only"
    )
    ...
    try:
        ...
        logger.info(
            "\nResults saved to: %s",
            OUTPUT_DIR / universe.name.replace(" ", "_").lower(),
        )
